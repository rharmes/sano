// Course data for the Nepali study app. Generated from the original index.html tables.
// NOTE: `dev` (Devanagari) fields were AI-drafted 2026-06-19 from the romanized `np`
// strings and are PENDING Ross's verification — treat as drafts, not authoritative.
// The per-unit `goal` (can-do) strings are likewise AI-drafted and Ross's to refine.
const COURSE = [
	{
		id: 'basics',
		title: 'Basics',
		kind: 'phrases',
		goal: 'Greet people and be polite',
		items: [
			{
				id: 'namaste-hello-goodbye',
				dev: 'नमस्ते',
				en: 'Hello / Goodbye',
				enEither: true,
				usage: 'This is the most common greeting in Nepal, used at any time of day. It\'s accompanied by pressing your palms together in front of your chest. The word literally means "I bow to the divine in you" and is appropriate in both formal and informal settings.',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'नमस्ते, म सानो हुँ', en: "Hello, I'm Sano" },
					{ dev: 'सबैलाई नमस्ते', en: 'Hello, everyone' },
				],
			},
			{
				id: 'namaskaar-hello-formal',
				dev: 'नमस्कार',
				en: 'Hello (formal)',
				usage: 'More formal version of "namaste." Both mean essentially the same thing and are used as greetings. "Namaskaar" might be used in very formal or official settings.',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'शिक्षकलाई नमस्कार', en: 'Namaskar to the teacher' },
					{ dev: 'नमस्कार, कस्तो हुनुहुन्छ?', en: 'Greetings, how are you?' },
				],
			},
			{
				id: 'hajur-yes-polite-pardon',
				dev: 'हजुर',
				en: 'Yes (polite)',
				enAlt: ['Yes', 'You', 'Pardon'],
				usage: 'This versatile word shows respect and can mean "yes," "I agree," or "pardon me?" depending on context and tone. It\'s considered more polite than simply saying "cha" (yes) and is commonly used when speaking to elders or in formal situations.',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'हजुर, भन्नुस्', en: 'Yes, go ahead' },
					{ dev: 'हजुर? फेरि भन्नुस्', en: 'Pardon? Say that again' },
				],
			},
			{
				id: 'ho-yes-informal',
				dev: 'हो',
				en: 'Yes (informal)',
				usage: 'Nepalis may also shake their head side-to-side for yes, which can confuse visitors at first!',
			},
			{
				id: 'hoina-no-informal',
				dev: 'होइन',
				en: 'No (informal)',
				usage: 'The casual "no," the counterpart to "ho" (yes). Fine with friends and family; in formal settings soften it with context. "Hoina, dhanyabad" (no, thank you).',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'यो मेरो होइन', en: "This isn't mine" },
					{ dev: 'राम्रो छ, होइन?', en: "It's good, isn't it?" },
				],
			},
			{
				id: 'hola-maybe',
				dev: 'होला',
				en: 'Maybe',
				usage: 'Used to express possibility or uncertainty. This is a casual, conversational way to say "maybe" or "perhaps." It can be used when you\'re not sure about something or when you want to leave options open. For example, if someone asks if you\'ll come to dinner, you might say "hola" if you\'re uncertain.',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'पसल बन्द होला', en: 'The shop is probably closed' },
					{ dev: 'त्यो महँगो होला', en: "That's probably expensive" },
				],
			},
			{
				id: 'maaf-garnuhos-excuse-me-i-m-sorry',
				dev: 'माफ गर्नुहोस्',
				en: "Excuse me / I'm sorry",
				enEither: true,
				usage: "This phrase is used to apologize or to get someone's attention politely. It's appropriate in both casual and formal settings. You can use it when you need to pass by someone, when you've made a mistake, or when you need someone's attention.",
			},
			{
				id: 'kripaya-please',
				dev: 'कृपया',
				en: 'Please',
				usage: 'This formal word for "please" is used to make polite requests. It\'s especially common in written communication and formal speech. In casual conversation, people often omit "kripaya" and rely on polite verb forms instead.',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'कृपया पर्खनुस्', en: 'Please wait' },
					{ dev: 'कृपया बिस्तारै बोल्नुस्', en: 'Please speak slowly' },
				],
			},
			{
				id: 'dhanyabad-thank-you',
				dev: 'धन्यवाद',
				en: 'Thank you',
				usage: "This is the standard way to express gratitude. It's used in both casual and formal situations. While Nepali culture doesn't emphasize saying thank you as much as Western cultures, it's still appreciated and understood universally.",
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'धेरै धन्यवाद', en: 'Thank you very much' },
					{ dev: 'मद्दतको लागि धन्यवाद', en: 'Thanks for the help' },
				],
			},
			{
				id: 'swagat-cha-you-re-welcome',
				dev: 'स्वागत छ',
				en: "You're welcome",
				usage: 'Standard response to "dhanyabad." "Swagat" means welcome. Shows politeness and acknowledges the thanks. You might also hear "kei pardaina" (it\'s nothing/no problem).',
			},
			{
				id: 'subha-prabhat-good-morning',
				dev: 'शुभ प्रभात',
				en: 'Good morning',
				usage: 'A formal morning greeting. "Prabhat" means morning or dawn. However, "namaste" is more commonly used throughout the day, including mornings. "Subha prabhat" is more formal and literary.',
			},
			{
				id: 'subha-ratri-good-night',
				dev: 'शुभ रात्रि',
				en: 'Good night',
				usage: 'Used when saying goodbye at night or before going to sleep. "Subha" means good or auspicious, and "ratri" means night. It\'s a warm way to wish someone well before parting in the evening.',
			},
		],
	},
	{
		id: 'basics-replies',
		title: 'Everyday Replies',
		kind: 'phrases',
		goal: 'Say yes or no and react to news',
		items: [
			{
				id: 'huncha-okay-it-will-be-done',
				dev: 'हुन्छ',
				en: 'Yes',
				enAlt: ['Yes', 'Okay', 'It will be done'],
				usage: 'This is one of the most useful words in Nepali, expressing agreement, acceptance, or acknowledgment. You\'ll hear it constantly in everyday conversation. It can mean "okay," "alright," "it\'s possible," or "it will happen."',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'भोलि आउनुस्, हुन्छ?', en: 'Come tomorrow, okay?' },
					{ dev: 'हुन्छ, म आउँछु', en: "Okay, I'll come" },
				],
			},
			{
				id: 'hudaina-not-okay-it-won-t-work',
				dev: 'हुँदैन',
				en: 'No',
				enAlt: ['No', "It won't work"],
				usage: 'This is the negative form of "huncha," used to express that something is not acceptable, not possible, or won\'t work. It\'s a polite way to decline or say no without being too direct.',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'त्यो हुँदैन', en: "That won't do" },
					{ dev: 'आज हुँदैन, भोलि हुन्छ', en: "Today doesn't work — tomorrow does" },
				],
			},
			{
				id: 'ke-bhayo-what-happened',
				dev: 'के भयो?',
				en: 'What happened?',
				usage: 'Used to ask what\'s wrong or what occurred. If someone seems upset or if something unusual happened, this is an appropriate question. "Ke" means what, and "bhayo" means happened.',
			},
			{
				id: 'kehi-chhaina-nothing-it-s-nothing',
				dev: 'केही छैन',
				en: "It's nothing",
				usage: 'A polite response when asked what\'s wrong or if you need anything. "Kehi" means something/anything, and "chhaina" means "is not." It\'s a way to reassure someone that everything is fine.',
			},
			{
				id: 'timi-you-informal',
				dev: 'तिमी',
				en: 'You (informal)',
				usage: 'The informal "you" used with friends, family, children, or people younger than you. Unlike "tapai" (formal you), "timi" shows familiarity and closeness. Use it only with people you know well or who are younger.',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'तिमी कहाँ छौ?', en: 'Where are you?' },
					{ dev: 'तिमी मेरो साथी हौ', en: 'You are my friend' },
				],
			},
			{
				id: 'cha-yes-is-has',
				dev: 'छ',
				en: 'Yes / Is / Has',
				enEither: true,
				usage: 'A simple affirmative word meaning "yes" or serving as the verb "to be" or "to have." It\'s one of the most frequently used words in Nepali. For example, "ramro cha" (it is good) or "paisa cha?" (do you have money?).',
			},
			{
				id: 'chhaina-no-is-not-don-t-have',
				dev: 'छैन',
				en: "No / Is not / Don't have",
				enEither: true,
				usage: 'The negative form of "cha." It means "no," "is not," or "don\'t have." Used frequently in daily conversation. For example, "paisa chhaina" means "I don\'t have money" or "there is no money."',
			},
			{
				id: 'dherai-ramro-very-good-excellent',
				dev: 'धेरै राम्रो',
				en: 'Very good',
				usage: 'An emphatic way to say something is very good or excellent. "Dherai" intensifies "ramro." Use this to express strong approval or satisfaction with food, experiences, or outcomes.',
			},
			{
				id: 'ekdum-ramro-very-good-excellent',
				dev: 'एकदम राम्रो',
				en: 'Very good',
				usage: 'Strong expression of approval or satisfaction. "Ekdum" is an intensifier meaning "very" or "extremely." More emphatic than just "ramro" alone.',
			},
			{
				id: 'sarai-ramro-very-good-really-good',
				dev: 'सारै राम्रो',
				en: 'Very good',
				usage: 'Another way to emphasize something is very good. "Sarai" means "very" or "really." Interchangeable with "ekdum ramro" to express strong positive feelings.',
			},
		],
	},
	{
		id: 'numbers',
		title: 'Numbers 1–10',
		kind: 'vocab',
		goal: 'Count from one to ten',
		items: [
			{
				id: 'ek-one',
				dev: 'एक',
				emoji: '1️⃣',
				en: 'One',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'एक कप चिया दिनुस्', en: 'One cup of tea, please' },
					{ dev: 'एक छिन बस्नुस्', en: 'Sit for a moment' },
				],
			},
			{
				id: 'dui-two',
				dev: 'दुई',
				emoji: '2️⃣',
				en: 'Two',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'दुई मिनेट पर्खनुस्', en: 'Wait two minutes' },
					{ dev: 'दुईवटा समोसा दिनुस्', en: 'Two samosas, please' },
				],
			},
			{
				id: 'tin-three',
				dev: 'तीन',
				emoji: '3️⃣',
				en: 'Three',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'तीन बजे आउनुस्', en: "Come at three o'clock" },
					{ dev: 'तीन दिन भयो', en: "It's been three days" },
				],
			},
			{
				id: 'chaar-four',
				dev: 'चार',
				emoji: '4️⃣',
				en: 'Four',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'हामी चार जना छौं', en: 'There are four of us' },
					{ dev: 'चार कोठा छन्', en: 'There are four rooms' },
				],
			},
			{
				id: 'paanch-five',
				dev: 'पाँच',
				emoji: '5️⃣',
				en: 'Five',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'पाँच बज्यो', en: "It's five o'clock" },
					{ dev: 'पाँच रुपैयाँ मात्र', en: 'Only five rupees' },
				],
			},
			{
				id: 'chha-six',
				dev: 'छ',
				emoji: '6️⃣',
				en: 'Six',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'बेलुका छ बजे खाना खान्छु', en: 'I eat at six in the evening' },
					{ dev: 'छ महिना भयो', en: "It's been six months" },
				],
			},
			{
				id: 'saat-seven',
				dev: 'सात',
				emoji: '7️⃣',
				en: 'Seven',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'हप्तामा सात दिन छन्', en: 'There are seven days in a week' },
					{ dev: 'म सात बजे उठ्छु', en: 'I get up at seven' },
				],
			},
			{
				id: 'aath-eight',
				dev: 'आठ',
				emoji: '8️⃣',
				en: 'Eight',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'म आठ बजे घर आउँछु', en: 'I come home at eight' },
					{ dev: 'हामी आठ बजे खाना खान्छौं', en: 'We eat at eight' },
				],
			},
			{
				id: 'nau-nine',
				dev: 'नौ',
				emoji: '9️⃣',
				en: 'Nine',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'नौ बजे पसल बन्द हुन्छ', en: 'The shop closes at nine' },
					{ dev: 'म नौ बजे सुत्छु', en: 'I sleep at nine' },
				],
			},
			{
				id: 'das-ten',
				dev: 'दस',
				emoji: '🔟',
				en: 'Ten',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'दस बजे पसल खुल्छ', en: 'The shop opens at ten' },
					{ dev: 'मसँग दस रुपैयाँ छ', en: 'I have ten rupees' },
				],
			},
		],
	},
	{
		id: 'numbers-big',
		title: 'Bigger Numbers',
		kind: 'vocab',
		goal: 'Say tens, hundreds, and thousands',
		items: [
			{
				id: 'bis-twenty',
				dev: 'बीस',
				emoji: '🔢',
				en: 'Twenty',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'बीस मिनेट लाग्छ', en: 'It takes twenty minutes' },
					{ dev: 'बीस रुपैयाँ भयो', en: "It's twenty rupees" },
				],
			},
			{
				id: 'pachaas-fifty',
				dev: 'पचास',
				emoji: '🔢',
				en: 'Fifty',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'पचास प्रतिशत छुट', en: 'Fifty percent off' },
					{ dev: 'उहाँ पचास वर्षको हुनुहुन्छ', en: 'He is fifty years old' },
				],
			},
			{
				id: 'saya-hundred',
				dev: 'सय',
				emoji: '💯',
				en: 'Hundred',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'मलाई सय रुपैयाँ चाहियो', en: 'I need a hundred rupees' },
					{ dev: 'सय वर्ष पुरानो', en: 'A hundred years old' },
				],
			},
			{
				id: 'hajaar-thousand',
				dev: 'हजार',
				emoji: '🔢',
				en: 'Thousand',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'एक हजार रुपैयाँ', en: 'One thousand rupees' },
					{ dev: 'दुई हजार भयो', en: "It's two thousand" },
				],
			},
			{
				id: 'aadha-half',
				dev: 'आधा',
				emoji: '🌗',
				en: 'Half',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'आधा किलो दिनुस्', en: 'Half a kilo, please' },
					{ dev: 'आधा घण्टा लाग्छ', en: 'It takes half an hour' },
				],
			},
		],
	},
	{
		id: 'pronouns',
		title: 'Pronouns & Possessives',
		kind: 'phrases',
		goal: 'Say who you mean — I, you, we, theirs',
		items: [
			{
				id: 'ma-i-me',
				dev: 'म',
				en: 'I / Me',
				enEither: true,
				usage: 'The first-person pronoun. You have already seen it in phrases like "Ma Nepali bolchhu" (I speak Nepali). Nepali usually drops pronouns when the meaning is clear from the verb, so "ma" is often optional in casual speech.',
			},
			{
				id: 'hami-we-us',
				dev: 'हामी',
				en: 'We / Us',
				enEither: true,
				usage: 'The plural "we." You may also hear "hamiharu," which adds the plural marker "-haru" for emphasis. Used the same way in formal and informal speech.',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'हामी सँगै जान्छौं', en: 'We go together' },
					{ dev: 'हामी घरमा छौं', en: 'We are at home' },
				],
			},
			{
				id: 'tapai-you-formal',
				dev: 'तपाईं',
				en: 'You (formal)',
				usage: 'The respectful "you," safe to use with anyone — elders, strangers, in-laws, shopkeepers. When in doubt, use "tapai" rather than "timi." It pairs with the polite verb endings you have seen, like "-nu huncha."',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'तपाईं कहिले आउनुहुन्छ?', en: 'When will you come?' },
					{ dev: 'तपाईं राम्रो हुनुहुन्छ', en: 'You are kind' },
				],
			},
			{
				id: 'u-he-she-informal',
				dev: 'ऊ',
				en: 'He / She (informal)',
				enEither: true,
				usage: 'Informal third person, used for friends, children, or people younger than you. Like "timi," avoid it for elders or people you respect — use "uhaa" instead.',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'ऊ मेरो भाइ हो', en: 'He is my little brother' },
					{ dev: 'ऊ कहाँ छ?', en: 'Where is he?' },
				],
			},
			{
				id: 'uhaa-he-she-polite',
				dev: 'उहाँ',
				en: 'He / She (polite)',
				enEither: true,
				usage: 'The respectful way to say "he" or "she." Use it for elders, in-laws, teachers, or anyone you would address as "tapai." Using the polite third person shows the same respect as polite "you."',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'उहाँ मेरो बुवा हुनुहुन्छ', en: 'He is my father' },
					{ dev: 'उहाँ डाक्टर हुनुहुन्छ', en: 'She is a doctor' },
				],
			},
			{
				id: 'uniharu-they',
				dev: 'उनीहरू',
				en: 'They',
				usage: 'Third-person plural. The "-haru" ending is the general plural marker in Nepali — you can attach it to most nouns too: "saathiharu" (friends), "bachchaharu" (children).',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'उनीहरू साथी हुन्', en: 'They are friends' },
					{ dev: 'उनीहरू घरमा छन्', en: 'They are at home' },
				],
			},
			{
				id: 'yo-this',
				dev: 'यो',
				en: 'This',
				usage: 'Points to something near you. You have seen it in "Yo ke ho?" (What is this?). Also doubles as "it" for nearby things: "yo ramro cha" (this is good).',
			},
			{
				id: 'tyo-that',
				dev: 'त्यो',
				en: 'That',
				usage: 'Points to something away from you, the counterpart of "yo." "Tyo ke ho?" (What is that?), "tyo mero ho" (that is mine).',
			},
			{
				id: 'mero-my-mine',
				dev: 'मेरो',
				en: 'My / Mine',
				enEither: true,
				usage: 'First-person possessive, as in "Mero naam" (my name). Possessives agree with what is owned, but "mero" works in almost all everyday cases.',
			},
			{
				id: 'hamro-our-ours',
				dev: 'हाम्रो',
				en: 'Our / Ours',
				enEither: true,
				usage: 'The possessive of "hami." "Hamro ghar" means "our house." Very common in family settings since the household is usually talked about as shared.',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'हाम्रो घर सानो छ', en: 'Our house is small' },
					{ dev: 'हाम्रो देश नेपाल हो', en: 'Our country is Nepal' },
				],
			},
			{
				id: 'timro-your-informal',
				dev: 'तिम्रो',
				en: 'Your (informal)',
				usage: 'The possessive of "timi," for friends, children, and people younger than you. "Timro naam ke ho?" is the informal way to ask a name.',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'तिम्रो घर कहाँ छ?', en: 'Where is your house?' },
					{ dev: 'तिम्रो किताब यहाँ छ', en: 'Your book is here' },
				],
			},
			{
				id: 'tapaiko-your-formal',
				dev: 'तपाईंको',
				en: 'Your (formal)',
				usage: 'The possessive of "tapai." "Tapaiko" is built from "tapai" + "ko," the same "ko" that marks possession across Nepali: "Ross ko kitab" (Ross\'s book).',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'यो तपाईंको झोला हो?', en: 'Is this your bag?' },
					{ dev: 'तपाईंको घर राम्रो छ', en: 'Your house is nice' },
				],
			},
			{
				id: 'usko-his-her-informal',
				dev: 'उसको',
				en: 'His / Her (informal)',
				enEither: true,
				usage: 'The possessive of "u." For the polite version use "uhaako" (his/her, respectful), following the same pattern of adding "ko."',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'उसको नाम के हो?', en: 'What is his name?' },
					{ dev: 'उसको घर टाढा छ', en: 'His house is far' },
				],
			},
		],
	},
	{
		id: 'family-people',
		title: 'Family',
		kind: 'vocab',
		goal: 'Name your close family',
		items: [
			{
				id: 'aamaa-mother',
				dev: 'आमा',
				emoji: '👩',
				en: 'Mother',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'मेरी आमा घरमा हुनुहुन्छ', en: 'My mother is at home' },
					{ dev: 'म आमालाई सम्झन्छु', en: 'I miss my mother' },
				],
			},
			{
				id: 'buwa-father',
				dev: 'बुवा',
				emoji: '👨',
				en: 'Father',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'मेरो बुवा अफिस जानुहुन्छ', en: 'My father goes to the office' },
					{ dev: 'बुवा काम गर्नुहुन्छ', en: 'Father works' },
				],
			},
			{
				id: 'didi-older-sister',
				dev: 'दिदी',
				emoji: '👧',
				en: 'Older sister',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'मेरी दिदी बजार जानुहुन्छ', en: 'My older sister goes to the market' },
					{ dev: 'दिदी घरमा हुनुहुन्छ', en: 'Older sister is at home' },
				],
			},
			{
				id: 'bahini-younger-sister',
				dev: 'बहिनी',
				emoji: '👧',
				en: 'Younger sister',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'मेरी बहिनी छ', en: 'I have a younger sister' },
					{ dev: 'बहिनी पढ्दै छ', en: 'Little sister is studying' },
				],
			},
			{
				id: 'dai-older-brother',
				dev: 'दाइ',
				emoji: '👦',
				en: 'Older brother',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'दाइ, नमस्ते', en: 'Hello, brother' },
					{ dev: 'मेरो दाइ काममा हुनुहुन्छ', en: 'My older brother is at work' },
				],
			},
			{
				id: 'bhai-younger-brother',
				dev: 'भाइ',
				emoji: '👦',
				en: 'Younger brother',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'भाइ सानो छ', en: 'Little brother is small' },
					{ dev: 'भाइ खेल्दै छ', en: 'Little brother is playing' },
				],
			},
			{
				id: 'chhora-son',
				dev: 'छोरा',
				emoji: '👦',
				en: 'Son',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'मेरो छोरा सानो छ', en: 'My son is small' },
					{ dev: 'छोरा स्कुल जान्छ', en: 'The son goes to school' },
				],
			},
			{
				id: 'chhori-daughter',
				dev: 'छोरी',
				emoji: '👧',
				en: 'Daughter',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'मेरी छोरी छ', en: 'I have a daughter' },
					{ dev: 'छोरी स्कुलमा छ', en: 'My daughter is at school' },
				],
			},
		],
	},
	{
		id: 'people-others',
		title: 'People Around You',
		kind: 'vocab',
		goal: 'Name relatives, friends, and neighbors',
		items: [
			{
				id: 'hajurbuwa-grandfather',
				dev: 'हजुरबुवा',
				emoji: '👴',
				en: 'Grandfather',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'हजुरबुवा गाउँमा बस्नुहुन्छ', en: 'Grandfather lives in the village' },
					{ dev: 'मेरो हजुरबुवा हुनुहुन्छ', en: 'I have a grandfather' },
				],
			},
			{
				id: 'hajuraamaa-grandmother',
				dev: 'हजुरआमा',
				emoji: '👵',
				en: 'Grandmother',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'हजुरआमाको खाना मिठो छ', en: "Grandmother's cooking is delicious" },
					{ dev: 'हजुरआमा घरमा हुनुहुन्छ', en: 'Grandmother is at home' },
				],
			},
			{
				id: 'budha-husband',
				dev: 'बुढा',
				emoji: '🤵',
				en: 'Husband',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'मेरो बुढा खाना पकाउनुहुन्छ', en: 'My husband cooks' },
					{ dev: 'मेरो बुढा बजार जानुभयो', en: 'My husband went to the market' },
				],
			},
			{
				id: 'budhi-wife',
				dev: 'बुढी',
				emoji: '👰',
				en: 'Wife',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'मेरी बुढी डाक्टर हुनुहुन्छ', en: 'My wife is a doctor' },
					{ dev: 'मेरी बुढी नेपाली सिक्दै हुनुहुन्छ', en: 'My wife is learning Nepali' },
				],
			},
			{
				id: 'saathi-friend',
				dev: 'साथी',
				emoji: '🧑‍🤝‍🧑',
				en: 'Friend',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'मेरो साथी नेपाली हो', en: 'My friend is Nepali' },
					{ dev: 'म साथीसँग बजार जान्छु', en: 'I go to the market with my friend' },
				],
			},
			{
				id: 'chhimeki-neighbor',
				dev: 'छिमेकी',
				emoji: '🏠',
				en: 'Neighbor',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'मेरो छिमेकी राम्रो मान्छे हो', en: 'My neighbor is a good person' },
					{ dev: 'छिमेकीलाई नमस्ते भन्नुस्', en: 'Say namaste to the neighbor' },
				],
			},
			{
				id: 'manchhe-person',
				dev: 'मान्छे',
				emoji: '🧑',
				en: 'Person',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'त्यो मान्छे को हो?', en: 'Who is that person?' },
					{ dev: 'धेरै मान्छे छन्', en: 'There are many people' },
				],
			},
			{
				id: 'bachcha-child-baby',
				dev: 'बच्चा',
				emoji: '👶',
				en: 'Child',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'बच्चा सुत्दै छ', en: 'The baby is sleeping' },
					{ dev: 'बच्चालाई दूध दिनुस्', en: 'Give the baby milk' },
				],
			},
		],
	},
	{
		id: 'introductions',
		title: 'Introductions',
		kind: 'phrases',
		goal: 'Introduce yourself and ask names',
		items: [
			{
				id: 'tapai-ko-naam-ke-ho-what-is-your-name-formal',
				dev: 'तपाईंको नाम के हो?',
				en: 'What is your name? (formal)',
				usage: 'This is the polite way to ask someone\'s name. "Tapai" is the formal "you," "ko" makes it possessive, and "naam" means name. It\'s appropriate when meeting someone for the first time in a formal or respectful context.',
			},
			{
				id: 'hajur-ko-naam-ke-ho-what-is-your-name-very-polite',
				dev: 'हजुरको नाम के हो?',
				en: 'What is your name? (very polite)',
				usage: 'An extremely respectful way to ask someone\'s name, more formal than "tapaiko naam ke ho." Used when addressing someone of very high status, significantly older, or in very formal situations.',
			},
			{
				id: 'timro-naam-ke-ho-what-is-your-name-informal',
				dev: 'तिम्रो नाम के हो?',
				en: 'What is your name? (informal)',
				usage: 'The informal way to ask someone\'s name using "timro" (your - informal). Use this with friends, children, or people younger than you. It establishes a casual, friendly tone.',
			},
			{
				id: 'mero-naam-ho-my-name-is',
				dev: 'मेरो नाम ___ हो',
				en: 'My name is ___',
				usage: 'Use this phrase to introduce yourself. "Mero" means "my," "naam" is "name," and "ho" is "is." Simply insert your name in the blank space. This works in both formal and informal situations.',
			},
			{
				id: 'tapai-lai-bhetda-khushi-lagyo-nice-to-meet-you',
				dev: 'तपाईंलाई भेट्दा खुसी लाग्यो',
				en: 'Nice to meet you',
				usage: 'A polite phrase when meeting someone for the first time. "Bhetda" means "meeting," and "khushi lagyo" means "felt happy." It\'s a formal way to express pleasure at making someone\'s acquaintance.',
			},
			{
				id: 'tapai-lai-kasto-cha-how-are-you-formal',
				dev: 'तपाईंलाई कस्तो छ?',
				en: 'How are you? (formal)',
				usage: 'This is the polite way to ask someone how they\'re doing. Use "tapai" when addressing someone older, someone you don\'t know well, or in professional settings. The response is typically "Sanchai cha" (I\'m fine).',
			},
			{
				id: 'sanchai-chu-i-m-fine',
				dev: 'सञ्चै छु',
				en: "I'm fine",
				usage: 'This is the standard response when someone asks how you are. "Sanchai" means fine or well, and "cha" is the verb "to be." You might also hear "ramro cha" (it\'s good) as a similar response.',
			},
			{
				id: 'kaha-jane-where-are-you-going',
				dev: 'कहाँ जाने?',
				en: 'Where are you going?',
				usage: 'This is a common casual greeting in Nepal, similar to "How are you?" in English. It\'s not always a literal question about your destination. A common response is to state where you\'re going or simply say "tira tira" (around/here and there).',
			},
			{
				id: 'tapai-nepali-bolnu-hunchha-do-you-speak-nepali',
				dev: 'तपाईं नेपाली बोल्नुहुन्छ?',
				en: 'Do you speak Nepali?',
				usage: 'A polite way to ask if someone speaks Nepali. "Tapai" is the respectful "you," "bolnu hunchha" means "do you speak." You can replace "Nepali" with "English" (Angreji) or other languages.',
			},
		],
	},
	{
		id: 'introductions-origins',
		title: 'Getting to Know You',
		kind: 'phrases',
		goal: 'Talk about language, home, and goodbyes',
		items: [
			{
				id: 'ma-nepali-bolchhu-i-speak-nepali',
				dev: 'म नेपाली बोल्छु',
				en: 'I speak Nepali',
				usage: 'Use this to indicate you can speak Nepali. You might add "ali ali" before "Nepali bolchhu" to say you speak only a little. This phrase helps set expectations for communication.',
			},
			{
				id: 'ma-nepali-boldina-i-don-t-speak-nepali',
				dev: 'म नेपाली बोल्दिनँ',
				en: "I don't speak Nepali",
				usage: "The negative form, indicating you don't speak Nepali. This can be useful for tourists or new arrivals. Often people will still be helpful and try to communicate through gestures or simple words.",
			},
			{
				id: 'tapai-kaha-bata-ho-where-are-you-from',
				dev: 'तपाईं कहाँबाट हो?',
				en: 'Where are you from?',
				usage: 'A common question when meeting someone new. "Bata" means "from," so this literally asks "from where are you?" You can answer with your country or city name followed by "bata ho."',
			},
			{
				id: 'ma-bata-ho-i-am-from',
				dev: 'म ___ बाट हो',
				en: 'I am from ___',
				usage: 'Insert your country or city in the blank to answer where you\'re from. For example, "Ma America bata ho" (I am from America) or "Ma Kathmandu bata ho" (I am from Kathmandu).',
			},
			{
				id: 'kaha-basnu-huncha-where-do-you-live',
				dev: 'कहाँ बस्नुहुन्छ?',
				en: 'Where do you live?',
				usage: 'Polite way to ask where someone lives. "Basnu huncha" is the respectful form of "to live/stay." You can respond with your neighborhood, city, or country.',
			},
			{
				id: 'ma-ma-baschu-i-live-in',
				dev: 'म ___ मा बस्छु',
				en: 'I live in ___',
				usage: 'Response to where you live. Insert your location in the blank. "Ma" means I, and "baschu" means "I live." For example, "Ma Kathmandu ma baschu."',
			},
			{
				id: 'pheri-bhetaula-see-you-again-let-s-meet-again',
				dev: 'फेरि भेटौंला',
				en: 'See you again',
				usage: 'A friendly way to say goodbye with the expectation of meeting again. "Pheri" means again, and "bhetaula" means "let\'s meet." It\'s warmer than a simple goodbye and shows you value the relationship.',
			},
			{
				id: 'ramrari-jaanu-go-safely-take-care',
				dev: 'राम्ररी जानु',
				en: 'Go safely',
				usage: 'A caring farewell wishing someone a safe journey or safe travels. "Sanchai" means safe/well, and "jaanu" means to go. Shows concern for someone\'s wellbeing.',
			},
		],
	},
	{
		id: 'comprehension',
		title: 'Comprehension',
		kind: 'phrases',
		goal: 'Say whether you understand',
		items: [
			{
				id: 'maile-bujhe-i-understand-i-understood',
				dev: 'मैले बुझेँ',
				en: 'I understand',
				usage: 'This phrase indicates that you\'ve understood what someone said. "Bujhe" is the past tense form meaning "understood." You can also say "ma bujhchu" (I understand - present tense) for ongoing understanding. This is very useful when someone is explaining something to you and you want to confirm your comprehension.',
			},
			{
				id: 'maile-bujhina-i-don-t-understand',
				dev: 'मैले बुझिनँ',
				en: "I don't understand",
				usage: 'This essential phrase lets others know you haven\'t understood. "Ma" means "I" and "bujhina" is "don\'t understand." It\'s helpful when learning the language or when someone is speaking too quickly.',
			},
			{
				id: 'thaha-chhaina-i-don-t-know',
				dev: 'थाहा छैन',
				en: "I don't know",
				usage: 'Indicates you don\'t know something. "Thaha" means knowledge, and "chhaina" means "is not" or "don\'t have." A straightforward way to admit you lack information about something.',
			},
			{
				id: 'malai-thaaha-cha-i-know',
				dev: 'मलाई थाहा छ',
				en: 'I know',
				usage: 'The opposite of "thaha chhaina," indicating you do know something. "Malai" means "to me" or "I," and "thaha cha" means "have knowledge." Shows understanding or awareness.',
			},
			{
				id: 'sochna-parcha-need-to-think-let-me-think',
				dev: 'सोच्नु पर्छ',
				en: 'Let me think',
				usage: 'Indicates you need time to consider something. "Sochna" means "to think," and "parcha" means "is needed." A polite way to delay giving an immediate answer.',
			},
			{
				id: 'ali-bistari-bolnuhos-please-speak-slowly',
				dev: 'अलि बिस्तारै बोल्नुहोस्',
				en: 'Please speak slowly',
				usage: 'Use this when you need someone to slow down their speech. "Bistari" means slowly, and "bolnuhos" is the polite command form of "to speak." This is particularly useful for language learners.',
			},
			{
				id: 'yo-ke-ho-what-is-this',
				dev: 'यो के हो?',
				en: 'What is this?',
				usage: 'A simple and useful question for identifying objects or learning new vocabulary. "Yo" means "this," "ke" means "what," and "ho" is "is." You can also say "tyo ke ho?" for "What is that?"',
			},
			{
				id: 'kasari-how',
				dev: 'कसरी?',
				en: 'How?',
				usage: "A question word asking about the manner or method of doing something. It's useful when you need instructions or want to understand the process of something. People use it frequently when learning new tasks.",
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'बसपार्क कसरी जाने?', en: 'How do I get to the bus stop?' },
					{ dev: 'यो कसरी खाने?', en: 'How do you eat this?' },
				],
			},
		],
	},
	{
		id: 'comprehension-clarify',
		title: 'Asking Questions',
		kind: 'phrases',
		goal: 'Ask what, why, and how to say things',
		items: [
			{
				id: 'kina-why',
				dev: 'किन?',
				en: 'Why?',
				usage: 'Asks for the reason behind something. While useful, be mindful that repeatedly asking "kina" might be seen as challenging authority in more traditional or formal settings, especially with elders.',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'किन यति महँगो?', en: 'Why so expensive?' },
					{ dev: 'तपाईं किन दुखी हुनुहुन्छ?', en: 'Why are you sad?' },
				],
			},
			{
				id: 'kun-which',
				dev: 'कुन?',
				en: 'Which?',
				usage: 'Asks you to choose between options: "kun ek?" (which one?), "kun bato?" (which way?). It rounds out the question words alongside "ke" (what), "kina" (why), and "kasari" (how).',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'कुन राम्रो छ?', en: 'Which one is good?' },
					{ dev: 'कुन बाटो जाने?', en: 'Which way do we go?' },
				],
			},
			{
				id: 'nepali-ma-lai-ke-bhanchha-how-do-you-say-in-nepali',
				dev: 'नेपालीमा ___ लाई के भन्छ?',
				en: 'How do you say ___ in Nepali?',
				usage: 'Essential phrase for language learners. Insert the English word in the blank space. "Nepali ma" means "in Nepali," "lai" is a particle, "ke bhanchha" means "what do you say/call." For example, "Nepali ma \'apple\' lai ke bhanchha?" (How do you say \'apple\' in Nepali?). Very useful for building vocabulary.',
			},
			{
				id: 'lekhera-dinuhola-can-you-write-that-down',
				dev: 'लेखेर दिनुहोला?',
				en: 'Can you write that down?',
				usage: 'A polite request asking someone to write something for you. "Lekhera" means "writing" or "having written," "dinuhola" means "can you give/do." This is particularly helpful when learning new words, getting addresses, or when you need a written reference of something spoken.',
			},
			{
				id: 'pheri-bhannus-can-you-repeat-that',
				dev: 'फेरि भन्नुस्',
				en: 'Can you repeat that?',
				usage: 'Polite way to ask someone to repeat what they just said. "Pheri" means "again," and "bhannus" or "bhandinuhos" means "please say." Use this when you didn\'t hear clearly or need to hear something again to understand better. The second version is slightly more formal.',
			},
			{
				id: 'bujhnubhayo-bujyau-do-you-understand',
				dev: 'बुझ्नुभयो? / बुझ्यौ?',
				en: 'Do you understand?',
				usage: 'Used to check if someone has understood what you said. "Bujhnubhayo" is the polite/formal version, while "bujhyo" is more casual. Teachers, hosts, and anyone explaining something will use this phrase to ensure comprehension. You can respond with "bujhe" (I understood) or "bujhina" (I didn\'t understand).',
			},
			{
				id: 'tapai-bolnu-hunchha-do-you-speak-english',
				dev: 'तपाईं अङ्ग्रेजी बोल्नुहुन्छ?',
				en: 'Do you speak English?',
				usage: 'A lifeline when you\'re stuck. "Angreji" is English; "bolnu hunchha?" is the polite "do you speak?" Swap in another language: "Hindi bolnu hunchha?"',
			},
			{
				id: 'ma-bolchhu-i-speak-english',
				dev: 'म अङ्ग्रेजी बोल्छु',
				en: 'I speak English',
				usage: 'Your reply about yourself — "ma ___ bolchhu" (I speak ___). "Ma Angreji bolchhu" (I speak English), or "ma ali-ali Nepali bolchhu" (I speak a little Nepali).',
			},
		],
	},
	{
		id: 'connectors',
		title: 'Connectors & Fillers',
		kind: 'phrases',
		goal: 'Connect ideas with and, but, also',
		items: [
			{
				id: 'ra-and',
				dev: 'र',
				en: 'And',
				usage: 'The basic "and," joining words and clauses: "dal ra bhaat" (lentils and rice). One of the most frequent words in the language.',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'दाल र भात', en: 'Dal and rice' },
					{ dev: 'आमा र बुवा घरमा हुनुहुन्छ', en: 'Mother and father are at home' },
				],
			},
			{
				id: 'tara-but',
				dev: 'तर',
				en: 'But',
				usage: 'Introduces a contrast: "mitho cha, tara piro cha" (it\'s tasty, but it\'s spicy). Works just like English "but."',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'म जान्छु तर ऊ आउँदैन', en: "I'm going but he isn't coming" },
					{ dev: 'सस्तो छ तर राम्रो छैन', en: "It's cheap but not good" },
				],
			},
			{
				id: 'ani-and-then-also',
				dev: 'अनि',
				en: 'And then',
				usage: 'A conversational connector for continuing a story or list: "ani ke bhayo?" (and then what happened?). You will hear it constantly in spoken Nepali.',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'अनि तपाईं?', en: 'And you?' },
					{ dev: 'खाना खानुस् अनि सुत्नुस्', en: 'Eat and then sleep' },
				],
			},
			{
				id: 'ki-or-in-questions',
				dev: 'कि',
				en: 'Or (in questions)',
				usage: 'Offers alternatives in questions: "chiya ki coffee?" (tea or coffee?). It can also turn a statement into a tag question when added at the end.',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'चिया कि कफी?', en: 'Tea or coffee?' },
					{ dev: 'आज कि भोलि?', en: 'Today or tomorrow?' },
				],
			},
			{
				id: 'athawa-or-in-statements',
				dev: 'अथवा',
				en: 'Or (in statements)',
				usage: 'The "or" used in statements rather than questions: "aaja athawa bholi" (today or tomorrow). In casual speech, "ki" often covers both.',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'आज अथवा भोलि आउनुस्', en: 'Come today or tomorrow' },
					{ dev: 'बस अथवा ट्याक्सी लिनुस्', en: 'Take a bus or a taxi' },
				],
			},
			{
				id: 'pani-also-too',
				dev: 'पनि',
				en: 'Also / Too',
				enEither: true,
				usage: 'Means "also" or "even": "ma pani" (me too). Note the short vowel — "paani" with a long aa means water. Context and length tell them apart.',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'म पनि जान्छु', en: "I'm coming too" },
					{ dev: 'यो पनि दिनुस्', en: 'Give me this one too' },
				],
			},
			{
				id: 'matra-only-just',
				dev: 'मात्र',
				en: 'Only',
				usage: 'Restricts to "only": "ek matra" (only one), "herna matra" (just looking — useful in shops!). It follows the word it limits.',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'एक कप मात्र', en: 'Just one cup' },
					{ dev: 'पानी मात्र दिनुस्', en: 'Just water, please' },
				],
			},
			{
				id: 'ahile-now-right-now',
				dev: 'अहिले',
				en: 'Now',
				usage: 'Refers to the present moment: "ahile aaunus" (come now). "Ahile nai" makes it stronger — "right this moment."',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'अहिले समय छैन', en: "There's no time right now" },
					{ dev: 'म अहिले व्यस्त छु', en: "I'm busy right now" },
				],
			},
			{
				id: 'pachhi-later-after',
				dev: 'पछि',
				en: 'Later / After',
				enEither: true,
				usage: 'Means "later" on its own ("pachhi bhetaula" — see you later) or "after" following another word: "khana pachhi" (after the meal).',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'पछि भेटौंला', en: 'See you later' },
					{ dev: 'म पछि खान्छु', en: "I'll eat later" },
				],
			},
			{
				id: 'aba-now-next',
				dev: 'अब',
				en: 'Now / Next',
				enEither: true,
				usage: 'Marks a transition to what happens next: "aba ke garne?" (now what shall we do?). Slightly different from "ahile" — "aba" is about sequence, "ahile" about the present moment.',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'अब के गर्ने?', en: 'What to do now?' },
					{ dev: 'अब म घर जान्छु', en: "Now I'm going home" },
				],
			},
			{
				id: 'pheri-again',
				dev: 'फेरि',
				en: 'Again',
				usage: 'Means "again," already familiar from "pheri bhetaula" (let\'s meet again) and "pheri bhannus" (please say it again). On its own: "pheri garnus" (do it again).',
				// T29 batch 11 (pronouns & connectors; AI-drafted, approved):
				frames: [
					{ dev: 'फेरि आउनुस्', en: 'Come again' },
					{ dev: 'म फेरि आउँछु', en: "I'll come again" },
				],
			},
		],
	},
	{
		id: 'meals',
		title: 'Meals',
		kind: 'phrases',
		goal: 'Offer food and ask about meals',
		items: [
			{
				id: 'paani-dinus-please-give-me-water',
				dev: 'पानी दिनुस्',
				en: 'Please give me water',
				usage: 'A practical phrase for restaurants or homes. "Paani" is water and "dinus" is the polite form of "give me." You can replace "paani" with other items you need.',
			},
			{
				id: 'khana-khanu-bhayo-have-you-eaten',
				dev: 'खाना खानुभयो?',
				en: 'Have you eaten?',
				usage: 'This is a common way of showing care and concern in Nepali culture, similar to "How are you?" in English. It\'s not always a literal inquiry about whether someone has eaten, but rather a friendly greeting showing you care about their wellbeing.',
			},
			{
				id: 'chiya-khaane-will-you-have-tea',
				dev: 'चिया खाने?',
				en: 'Will you have tea?',
				usage: 'Common offer of hospitality. Note that Nepalis say "eat tea" (chiya khaane) rather than "drink tea." Tea is an integral part of Nepali culture and this offer is made frequently.',
			},
			{
				id: 'khana-khaeko-chu-i-ve-eaten-i-have-eaten',
				dev: 'खाना खाएको छु',
				en: 'I have eaten',
				usage: 'Past tense statement indicating you\'ve already eaten. "Khaeko" is the past participle of "to eat," and "chu" means "I am/have." Use this when someone asks if you\'ve eaten or offers you food when you\'ve already had a meal. It\'s a polite way to decline additional food.',
			},
			{
				id: 'khana-khaeko-chhaina-i-haven-t-eaten',
				dev: 'खाना खाएको छैन',
				en: "I haven't eaten",
				usage: 'Indicates you have not yet eaten. "Chhaina" is the negative form meaning "have not" or "is not." Use this when someone asks if you\'ve eaten and you\'re still hungry. In Nepali culture, asking "khana khanu bhayo?" (have you eaten?) is a common way of showing care, and this is the honest response if you haven\'t.',
			},
			{
				id: 'kasto-cha-how-is-it',
				dev: 'कस्तो छ?',
				en: 'How is it?',
				usage: 'Asks about the quality, condition, or state of something. Can refer to food, weather, experiences, or situations. A versatile question used frequently in conversation.',
			},
			{
				id: 'mitho-cha-it-s-delicious',
				dev: 'मिठो छ',
				en: "It's delicious",
				usage: 'Use this to compliment food. "Mitho" means delicious or sweet (in taste), and expressing appreciation for food is an important part of Nepali hospitality. Your host will be pleased to hear this.',
			},
			{
				id: 'pugyo-enough-that-s-sufficient',
				dev: 'पुग्यो',
				en: 'Enough',
				usage: "This word indicates you've had enough, whether it's food being served, items being poured, or any other quantity. It's polite but firm. In restaurants, servers might keep offering more food, and this word stops them.",
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'खाना पुग्यो', en: "That's enough food" },
					{ dev: 'मलाई पुग्यो', en: "I've had enough" },
				],
			},
			{
				id: 'pardaina-no-need-not-necessary',
				dev: 'पर्दैन',
				en: 'Not necessary',
				usage: 'Politely declines something or indicates something isn\'t necessary. Less direct than saying "no." For example, when someone offers something you don\'t need, you can say "pardaina, dhanyabad."',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'आउनु पर्दैन', en: "You don't need to come" },
					{ dev: 'चिन्ता गर्नु पर्दैन', en: 'No need to worry' },
				],
			},
			{
				id: 'thik-cha-it-s-okay-it-s-fine',
				dev: 'ठीक छ',
				en: "It's okay",
				usage: 'A versatile phrase expressing that something is acceptable, correct, or fine. "Thik" means right, correct, or okay. You can use it to agree with someone, to say something is acceptable, or to indicate you\'re doing fine.',
			},
			{
				id: 'khana-mitho-cha-the-food-is-delicious',
				dev: 'खाना मिठो छ',
				en: 'The food is delicious',
				usage: 'Past tense compliment for food. "Thiyo" is the past tense of "was." Use this after finishing a meal to thank your host or compliment the chef.',
			},
			{
				id: 'ma-machha-maasu-khadina-i-don-t-eat-meat',
				dev: 'म माछामासु खाँदिनँ',
				en: "I don't eat meat",
				usage: 'Important phrase for vegetarians. "Maasu" means meat, and "khadina" means "don\'t eat." This clearly communicates dietary restrictions. You can also say "ma shakahari ho" (I am vegetarian) for a more general statement. Nepal has a large vegetarian population, so this dietary preference is well understood and respected.',
			},
			{
				id: 'ma-dudh-dahi-ra-ghue-khadina-i-don-t-eat-milk-and-milk-prod',
				dev: 'म दूध, दही र घ्यू खाँदिनँ',
				en: "I don't eat milk and milk products",
				usage: '"Dudh" means milk, "ra" means and, "banaeko kura" means "made things/products." You might also learn "paneer" (cheese), "dahi" (yogurt), and "ghee" (clarified butter) to identify specific items to avoid.',
			},
			{
				id: 'khana-tayar-bhayo-the-food-is-ready',
				dev: 'खाना तयार भयो',
				en: 'The food is ready',
				usage: 'Announces that a meal is prepared and ready to eat. "Tayar" means ready or prepared, and "bhayo" means "became" or "is." Family members often use this to call others to the table. You might also hear "khana khana aaunu" (come to eat) as an invitation to the meal.',
			},
		],
	},
	{
		id: 'kitchen-items',
		title: 'Kitchen Items',
		kind: 'vocab',
		goal: 'Name things in the kitchen',
		items: [
			{
				id: 'bhanda-dishes-utensils',
				dev: 'भाँडा',
				emoji: '🍽️',
				en: 'Dishes',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'भाँडा फोहोर छ', en: 'The dishes are dirty' },
					{ dev: 'भाँडा माझ्नुस्', en: 'Wash the dishes' },
				],
			},
			{
				id: 'thaal-plate',
				dev: 'थाल',
				emoji: '🍽️',
				en: 'Plate',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'थालमा भात छ', en: "There's rice on the plate" },
					{ dev: 'थाल टेबलमा राख्नुस्', en: 'Put the plate on the table' },
				],
			},
			{
				id: 'kachaura-bowl',
				dev: 'कचौरा',
				emoji: '🥣',
				en: 'Bowl',
			},
			{
				id: 'chamcha-spoon',
				dev: 'चम्चा',
				emoji: '🥄',
				en: 'Spoon',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'मलाई चम्चा दिनुस्', en: 'Please give me a spoon' },
					{ dev: 'चम्चा सफा छैन', en: "The spoon isn't clean" },
				],
			},
			{
				id: 'kaanta-fork',
				dev: 'काँटा',
				emoji: '🍴',
				en: 'Fork',
			},
			{
				id: 'churaa-knife',
				dev: 'छुरा',
				emoji: '🔪',
				en: 'Knife',
			},
			{
				id: 'glass-glass-cup',
				dev: 'गिलास',
				emoji: '🥤',
				en: 'Glass',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'गिलासमा पानी छ', en: "There's water in the glass" },
					{ dev: 'अर्को गिलास दिनुस्', en: 'Another glass, please' },
				],
			},
			{
				id: 'kap-cup',
				dev: 'कप',
				emoji: '☕',
				en: 'Cup',
			},
			{
				id: 'dekchi-pot-cooking-pot',
				dev: 'डेक्ची',
				emoji: '🍲',
				en: 'Pot',
			},
			{
				id: 'taawaa-pan-frying-pan',
				dev: 'तावा',
				emoji: '🍳',
				en: 'Pan',
			},
			{
				id: 'chulho-stove',
				dev: 'चुल्हो',
				emoji: '🔥',
				en: 'Stove',
			},
			{
				id: 'fridge-refrigerator',
				dev: 'फ्रिज',
				emoji: '🧊',
				en: 'Refrigerator',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'दूध फ्रिजमा छ', en: 'The milk is in the fridge' },
					{ dev: 'फ्रिज खाली छ', en: 'The fridge is empty' },
				],
			},
			{
				id: 'kitli-kettle',
				dev: 'केतली',
				emoji: '🫖',
				en: 'Kettle',
			},
			{
				id: 'masala-daani-spice-container',
				dev: 'मसलादानी',
				emoji: '🧂',
				en: 'Spice container',
			},
		],
	},
	{
		id: 'food-grocery-items',
		title: 'Staple Foods',
		kind: 'vocab',
		goal: 'Name everyday staples',
		items: [
			{
				id: 'khana-food',
				dev: 'खाना',
				emoji: '🍽️',
				en: 'Food',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'म खाना खान्छु', en: 'I eat food' },
					{ dev: 'खाना मीठो छ', en: 'The food is delicious' },
				],
			},
			{
				id: 'bhaat-rice-cooked',
				dev: 'भात',
				emoji: '🍚',
				en: 'Rice (cooked)',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'भात तातो छ', en: 'The rice is hot' },
					{ dev: 'मलाई भात मन पर्छ', en: 'I like rice' },
				],
			},
			{
				id: 'daal-lentils',
				dev: 'दाल',
				emoji: '🫘',
				en: 'Lentils',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'म दाल खान्छु', en: 'I eat lentils' },
					{ dev: 'दाल मीठो छ', en: 'The lentils are tasty' },
				],
			},
			{
				id: 'tarkari-vegetables-curry',
				dev: 'तरकारी',
				emoji: '🥗',
				en: 'Vegetables / Curry',
				enEither: true,
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'म बजारमा तरकारी किन्छु', en: 'I buy vegetables at the market' },
					{ dev: 'तरकारी तयार भयो', en: 'The curry is ready' },
				],
			},
			{
				id: 'roti-bread-flatbread',
				dev: 'रोटी',
				emoji: '🫓',
				en: 'Flatbread',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'म रोटी बनाउँछु', en: 'I make flatbread' },
					{ dev: 'रोटी नरम छ', en: 'The flatbread is soft' },
				],
			},
			{
				id: 'maasu-meat',
				dev: 'मासु',
				emoji: '🍖',
				en: 'Meat',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'मासु महँगो छ', en: 'Meat is expensive' },
					{ dev: 'बाख्राको मासु मिठो छ', en: 'Goat meat is delicious' },
				],
			},
			{
				id: 'machha-fish',
				dev: 'माछा',
				emoji: '🐟',
				en: 'Fish',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'मलाई माछा मन पर्छ', en: 'I like fish' },
					{ dev: 'माछा पानीमा छ', en: 'The fish is in the water' },
				],
			},
			{
				id: 'kukhura-ko-phul-egg',
				dev: 'कुखुराको फुल',
				emoji: '🥚',
				en: 'Egg',
			},
			{
				id: 'dudh-milk',
				dev: 'दूध',
				emoji: '🥛',
				en: 'Milk',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'म दूध पिउँछु', en: 'I drink milk' },
					{ dev: 'दूध चिसो छ', en: 'The milk is cold' },
				],
			},
			{
				id: 'dahi-yogurt',
				dev: 'दही',
				emoji: '🥛',
				en: 'Yogurt',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'दही चिसो छ', en: 'The yogurt is cold' },
					{ dev: 'ताजा दही', en: 'Fresh yogurt' },
				],
			},
		],
	},
	{
		id: 'food-grocery-pantry',
		title: 'Pantry & Extras',
		kind: 'vocab',
		goal: 'Name drinks, seasonings, and pantry items',
		items: [
			{
				id: 'chiya-tea',
				dev: 'चिया',
				emoji: '🍵',
				en: 'Tea',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'चिया खानुहुन्छ?', en: 'Will you have tea?' },
					{ dev: 'चिया तातो छ', en: 'The tea is hot' },
				],
			},
			{
				id: 'paani-water',
				dev: 'पानी',
				emoji: '💧',
				en: 'Water',
			},
			{
				id: 'nun-salt',
				dev: 'नुन',
				emoji: '🧂',
				en: 'Salt',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'खानामा नुन छैन', en: "There's no salt in the food" },
					{ dev: 'नुन धेरै भयो', en: "There's too much salt" },
				],
			},
			{
				id: 'chini-sugar',
				dev: 'चिनी',
				emoji: '🍬',
				en: 'Sugar',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'चियामा चिनी हाल्नुस्', en: 'Put sugar in the tea' },
					{ dev: 'चिनी सकियो', en: 'The sugar is finished' },
				],
			},
			{
				id: 'tel-oil',
				dev: 'तेल',
				emoji: '🛢️',
				en: 'Oil',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'तेल तातो छ', en: 'The oil is hot' },
					{ dev: 'तेल धेरै नहाल्नुस्', en: "Don't use too much oil" },
				],
			},
			{
				id: 'masala-spices',
				dev: 'मसला',
				emoji: '🌶️',
				en: 'Spices',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'तरकारीमा मसला हाल्नुस्', en: 'Add spices to the curry' },
					{ dev: 'मसला धेरै भयो', en: 'Too much spice' },
				],
			},
			{
				id: 'phal-fruit',
				dev: 'फल',
				emoji: '🍎',
				en: 'Fruit',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'बिहान फल खानुस्', en: 'Eat fruit in the morning' },
					{ dev: 'यो फल पाकेको छ', en: 'This fruit is ripe' },
				],
			},
			{
				id: 'achar-pickle-chutney',
				dev: 'अचार',
				emoji: '🫙',
				en: 'Chutney',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'अचार पिरो हुन्छ', en: 'Pickle is spicy' },
					{ dev: 'भातसँग अचार खानुस्', en: 'Have pickle with rice' },
				],
			},
			{
				id: 'bhatta-soybean',
				dev: 'भटमास',
				emoji: '🫘',
				en: 'Soybean',
			},
			{
				id: 'maha-honey',
				dev: 'मह',
				emoji: '🍯',
				en: 'Honey',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'मह गुलियो हुन्छ', en: 'Honey is sweet' },
					{ dev: 'मह महँगो छ', en: 'Honey is expensive' },
				],
			},
		],
	},
	{
		id: 'patterns-requests',
		title: 'Patterns: Needs & Requests',
		kind: 'phrases',
		goal: 'Ask for what you need',
		items: [
			{
				id: 'malai-chaahiyo-i-need',
				dev: 'मलाई ___ चाहियो',
				en: 'I need ___',
				usage: 'The general "I need" pattern — you have already seen "Pani chaahiyo" and "Doctor chaahiyo." "Malai" means "to me," so this is literally "to me, ___ is needed." Insert any noun: "Malai chamcha chaahiyo" (I need a spoon).',
			},
			{
				id: 'malai-chiya-chaahiyo-i-need-tea',
				dev: 'मलाई चिया चाहियो',
				en: 'I need tea',
				usage: 'A worked example of the pattern — and a sentence you may use daily. Swap "chiya" for anything: paani, thaal, sabun.',
			},
			{
				id: 'dinus-please-give-me',
				dev: '___ दिनुस्',
				en: 'Please give me ___',
				usage: 'The polite request pattern behind "Paani dinus." "Dinus" is the respectful command form of "dinu" (to give). "Thaal dinus" (please give me a plate), "kap dinus" (please give me a cup).',
			},
			{
				id: 'nun-dinus-please-pass-the-salt',
				dev: 'नुन दिनुस्',
				en: 'Please pass the salt',
				usage: 'The pattern at the dinner table. Any item within reach works the same way: "achar dinus" (please pass the pickle).',
			},
			{
				id: 'kaha-cha-where-is',
				dev: '___ कहाँ छ?',
				en: 'Where is ___?',
				usage: 'The general "where is" pattern from "Charpi kaha cha?" Works for any object: "sabun kaha cha?" (where is the soap?). For people, use the polite verb instead: "Aamaa kaha hunuhunchha?" (where is mother?).',
			},
			{
				id: 'mero-kap-kaha-cha-where-is-my-cup',
				dev: 'मेरो कप कहाँ छ?',
				en: 'Where is my cup?',
				usage: 'Combines "mero" (my) with the where-is pattern. The everyday mystery of the missing cup, in Nepali.',
			},
			{
				id: 'ma-khojdai-chu-i-am-looking-for',
				dev: 'म ___ खोज्दै छु',
				en: 'I am looking for ___',
				usage: '"Khojnu" means to search. The "-dai chu" ending means "am currently doing" — you will see it again in other patterns. "Ma mero kap khojdai chu" (I am looking for my cup).',
			},
			{
				id: 'malai-man-parcha-i-like',
				dev: 'मलाई ___ मन पर्छ',
				en: 'I like ___',
				usage: 'Literally "to me, ___ falls on the mind." The standard way to express liking things: "Malai chiya man parcha" (I like tea). Great for complimenting food and making conversation.',
			},
			{
				id: 'malai-dahi-man-parcha-i-like-yogurt',
				dev: 'मलाई दही मन पर्छ',
				en: 'I like yogurt',
				usage: 'Worked example of "man parcha." Nepali dahi is often homemade and a point of pride — this sentence will land well.',
			},
			{
				id: 'malai-man-pardaina-i-don-t-like',
				dev: 'मलाई ___ मन पर्दैन',
				en: "I don't like ___",
				usage: 'The negative of "man parcha." Note it is gentler than refusing outright — pair it with "tara" (but) to stay polite: "Malai khursaani man pardaina, tara khana mitho cha" (I don\'t like chili, but the food is delicious).',
			},
			{
				id: 'tapai-sanga-cha-do-you-have',
				dev: 'तपाईंसँग ___ छ?',
				en: 'Do you have ___?',
				usage: '"Sanga" means "with," so this asks "is ___ with you?" — the standard "do you have" for things someone carries: "Tapai sanga paisa cha?" (do you have money?). "Paisa" is the everyday word for money.',
			},
			{
				id: 'arko-dinus-please-give-me-another',
				dev: 'अर्को ___ दिनुस्',
				en: 'Please give me another ___',
				usage: '"Arko" means "another" or "the next one." "Arko roti dinus" (one more roti, please) — guaranteed to make a Nepali host happy.',
			},
		],
	},
	{
		id: 'household-living',
		title: 'Household Living',
		kind: 'phrases',
		goal: 'Handle everyday life at home',
		items: [
			{
				id: 'charpi-kaha-cha-where-is-the-bathroom',
				dev: 'चर्पी कहाँ छ?',
				en: 'Where is the bathroom?',
				usage: 'Essential question for any household. "Charpi" is the common Nepali word for bathroom/toilet (you may also hear "toilet"). "Kaha cha" means "where is." This is often one of the first questions you need to ask in a new place. The response will typically include directions like "tyaha" (there) with a gesture.',
			},
			{
				id: 'ma-nuhaauna-jane-i-m-going-to-bathe',
				dev: 'म नुहाउन जाने',
				en: "I'm going to bathe",
				usage: 'Announces your intention to shower or bathe. "Nuhaauna" means "to bathe/shower," and "jane" means "going." This is polite to say in a household so others know the bathroom will be occupied.',
			},
			{
				id: 'ma-kehi-maddat-garna-sakchu-can-i-help-with-anything',
				dev: 'म केही मद्दत गर्न सक्छु?',
				en: 'Can I help with anything?',
				usage: 'Polite offer to assist with tasks. "Kehi" means "anything/something," "maddat" means "help," and "garna sakchu" means "can I do." This shows respect and willingness to contribute to household work. It\'s especially important in a host family situation to offer help with cooking, cleaning, or other chores.',
			},
			{
				id: 'ma-bhada-maajhchu-i-ll-wash-the-dishes',
				dev: 'म भाँडा माझ्छु',
				en: "I'll wash the dishes",
				usage: 'Offers to do the dishes. "Bhada" (or "bhanda") means dishes/utensils, and "maajhchu" means "I will wash/clean." This is a helpful phrase for contributing to household chores. In many Nepali homes, guests are not expected to wash dishes, but offering shows good manners and appreciation. Your host may decline the offer, but it\'s polite to make it.',
			},
			{
				id: 'dhoka-banda-garnus-please-close-the-door',
				dev: 'ढोका बन्द गर्नुस्',
				en: 'Please close the door',
				usage: '"Dhoka" is door and "banda" means closed or shut. "Banda garnus" (please close) works for windows too: "jhyaal banda garnus."',
			},
			{
				id: 'dhoka-banda-garnu-bhayo-did-you-close-the-door',
				dev: 'ढोका बन्द गर्नुभयो?',
				en: 'Did you close the door?',
				usage: 'The past-tense polite question form "-nu bhayo?" asks whether someone did something. The nightly household check.',
			},
			{
				id: 'batti-balnus-please-turn-on-the-light',
				dev: 'बत्ती बाल्नुस्',
				en: 'Please turn on the light',
				usage: '"Batti" is light or lamp, "balnu" is to light or switch on. Also used for the stove flame: "ggyaas balnus" (light the gas).',
			},
			{
				id: 'batti-nibhaunus-please-turn-off-the-light',
				dev: 'बत्ती निभाउनुस्',
				en: 'Please turn off the light',
				usage: '"Nibhaunu" means to extinguish or switch off — the counterpart of "balnu." Last one out of the room hears this.',
			},
		],
	},
	{
		id: 'household-chores',
		title: 'Helping Out',
		kind: 'phrases',
		goal: 'Offer help and pitch in with chores',
		items: [
			{
				id: 'ma-lugaa-dhunchu-i-will-wash-the-clothes',
				dev: 'म लुगा धुन्छु',
				en: 'I will wash the clothes',
				usage: '"Lugaa" is clothes and "dhunu" is to wash. The same verb covers washing dishes, hands, and laundry.',
			},
			{
				id: 'ma-ghar-safaa-garchu-i-will-clean-the-house',
				dev: 'म घर सफा गर्छु',
				en: 'I will clean the house',
				usage: '"Safaa" means clean. "Safaa garnu" (to make clean) is the general cleaning verb: "tebul safaa garnus" (please wipe the table).',
			},
			{
				id: 'ma-fohor-falchhu-i-will-take-out-the-trash',
				dev: 'म फोहोर फाल्छु',
				en: 'I will take out the trash',
				usage: '"Fohor" is trash (short for "fohormailaa") and "falnu" means to throw. A reliable way to be a popular member of the household.',
			},
			{
				id: 'paahuna-aaudai-chan-guests-are-coming',
				dev: 'पाहुना आउँदै छन्',
				en: 'Guests are coming',
				usage: '"Paahuna" means guest — a word with weight in Nepal, where guests are treated as honored. Expect rapid cleaning and cooking to follow this announcement.',
			},
			{
				id: 'bhitra-aaunus-please-come-in',
				dev: 'भित्र आउनुस्',
				en: 'Please come in',
				usage: '"Bhitra" means inside. The standard welcome at the door, usually followed by an offer of chiya.',
			},
			{
				id: 'ma-maddat-garchu-i-will-help',
				dev: 'म मद्दत गर्छु',
				en: 'I will help',
				usage: 'Combines "maddat" (help) with the "garchu" pattern. The active offer, next to the question "Ma kehi maddat garna sakchu?" you already know.',
			},
			{
				id: 'maddat-chaahiyo-do-you-need-help',
				dev: 'मद्दत चाहियो?',
				en: 'Do you need help?',
				usage: 'The "chaahiyo" pattern as a question. Said while walking into the kitchen with sleeves rolled up.',
			},
			{
				id: 'sutne-bela-bhayo-it-s-bedtime',
				dev: 'सुत्ने बेला भयो',
				en: "It's bedtime",
				usage: '"Bela" means time or moment, so this is literally "sleeping time happened." Used for children and tired adults alike.',
			},
		],
	},
	{
		id: 'cleaning-items',
		title: 'Cleaning Items',
		kind: 'vocab',
		goal: 'Name cleaning supplies',
		items: [
			{
				id: 'kucho-broom',
				dev: 'कुचो',
				emoji: '🧹',
				en: 'Broom',
			},
			{
				id: 'pochaa-mop',
				dev: 'पोछा',
				emoji: '🧽',
				en: 'Mop',
			},
			{
				id: 'fohormailaa-garbage-trash',
				dev: 'फोहोरमैला',
				emoji: '🗑️',
				en: 'Garbage',
			},
			{
				id: 'fohormailaa-ko-thungaa-trash-can',
				dev: 'डस्टबिन',
				emoji: '🗑️',
				en: 'Trash can',
			},
			{
				id: 'jhaaru-duster-cleaning-cloth',
				dev: 'डस्टर',
				emoji: '🧽',
				en: 'Duster',
			},
			{
				id: 'spanj-sponge',
				dev: 'स्पन्ज',
				emoji: '🧽',
				en: 'Sponge',
			},
			{
				id: 'baalti-bucket',
				dev: 'बाल्टी',
				emoji: '🪣',
				en: 'Bucket',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'बाल्टीमा पानी छ', en: "There's water in the bucket" },
					{ dev: 'बाल्टी ल्याउनुस्', en: 'Bring the bucket' },
				],
			},
			{
				id: 'panja-gloves',
				dev: 'पन्जा',
				emoji: '🧤',
				en: 'Gloves',
			},
		],
	},
	{
		id: 'patterns-doing',
		title: 'Patterns: Doing & Going',
		kind: 'phrases',
		goal: "Say what you're doing and where you're going",
		items: [
			{
				id: 'ma-jaanchu-i-am-going-to',
				dev: 'म ___ जान्छु',
				en: 'I am going to ___',
				usage: 'The "I go / I am going" pattern from "jaanu" (to go). "Ma bajaar jaanchu" (I am going to the market — "bajaar" is the market/shops). Announce where you are headed before leaving the house.',
			},
			{
				id: 'ma-ghar-jaanchu-i-am-going-home',
				dev: 'म घर जान्छु',
				en: 'I am going home',
				usage: '"Ghar" means house or home — one of the most useful words in Nepali, and the natural end to many conversations.',
			},
			{
				id: 'ma-garchu-i-will-do',
				dev: 'म ___ गर्छु',
				en: 'I will do ___',
				usage: 'From "garnu" (to do), the most versatile verb in Nepali. "Ma kaam garchu" (I will do the work). Offer to take on a task: "ma garchu!" (I\'ll do it!).',
			},
			{
				id: 'ma-kaam-garchu-i-am-working-i-will-do-the-work',
				dev: 'म काम गर्छु',
				en: 'I work',
				usage: '"Kaam" means work or task — both a job and household chores. "Kaam cha" (there is work / I\'m busy) is a phrase you will hear constantly.',
			},
			{
				id: 'tapai-ke-gardai-hunuhunchha-what-are-you-doing-polite',
				dev: 'तपाईं के गर्दै हुनुहुन्छ?',
				en: 'What are you doing? (polite)',
				usage: 'An everyday question around the house. "-dai" marks an action in progress, like English "-ing." The casual version with "timi" is "timi ke gardai chhau?"',
			},
			{
				id: 'ma-khana-pakaudai-chu-i-am-cooking-food',
				dev: 'म खाना पकाउँदै छु',
				en: 'I am cooking food',
				usage: '"Pakaunu" means to cook. Use it to announce you are at the stove — or "ma pakauchu" (I will cook) to volunteer for dinner duty.',
			},
			{
				id: 'ma-kinchhu-i-will-buy',
				dev: 'म ___ किन्छु',
				en: 'I will buy ___',
				usage: 'From "kinnu" (to buy). "Ma tarkari kinchhu" (I\'ll buy vegetables) — useful before a market run, and it pairs with the bargaining phrases you already know.',
			},
			{
				id: 'tapai-khanu-huncha-will-you-have-polite-offer',
				dev: 'तपाईं ___ खानुहुन्छ?',
				en: 'Will you have ___? (polite offer)',
				usage: 'The polite version of "Chiya khaane?" for offering food or drink. "Tapai bhaat khanu huncha?" (will you have rice?). Expect to be asked this often — hospitality is serious business.',
			},
			{
				id: 'ma-khanchu-i-will-have',
				dev: 'म ___ खान्छु',
				en: 'I will have ___',
				usage: 'Accepting an offer: "Ma chiya khanchu" (I\'ll have tea). Remember Nepali "eats" tea rather than drinking it. To decline politely, "pardaina, dhanyabad."',
			},
			{
				id: 'ma-sutna-jaane-i-am-going-to-bed',
				dev: 'म सुत्न जाने',
				en: 'I am going to bed',
				usage: '"Sutnu" means to sleep. The same shape as "Ma nuhaauna jane" (I\'m going to bathe) — verb + "jaane" announces what you are off to do.',
			},
			{
				id: 'ma-ahile-aaunchu-i-will-be-right-back',
				dev: 'म अहिले आउँछु',
				en: 'I will be right back',
				usage: 'Literally "I come now," from "aaunu" (to come) plus "ahile" (now). The standard way to excuse yourself briefly.',
			},
			{
				id: 'ek-chhin-parkhanus-wait-a-moment-please',
				dev: 'एक छिन पर्खनुस्',
				en: 'Wait a moment, please',
				usage: '"Ek chhin" is "one moment" — your number "ek" at work. "Parkhanus" is the polite form of "parkhanu" (to wait). Buy yourself time mid-conversation while you find a word!',
			},
		],
	},
	{
		id: 'verbs-present',
		title: 'Verbs: Present tense',
		kind: 'phrases',
		goal: 'Say what you do — talk in the present tense',
		items: [
			{
				id: 'ma-sikchu-i-learn',
				dev: 'म सिक्छु',
				en: 'I learn',
				usage: 'The present tense is the verb plus an ending that says who is doing it. "-chu" always means "I," so "ma sikchu" (I learn, from "siknu"). Every "Ma ___chu" below is built the same way — "I" plus the verb.',
				// T29 depth (AI-drafted → Ross's review):
				frames: [
					{ dev: 'म नेपाली सिक्छु', en: 'I learn Nepali' },
					{ dev: 'म नयाँ कुरा सिक्छु', en: 'I learn new things' },
				],
			},
			{
				id: 'tapai-siknuhunchha-do-you-study-polite',
				dev: 'तपाईं सिक्नुहुन्छ?',
				en: 'Do you learn? (polite)',
				usage: 'The polite "you" ending is "-nuhunchha." Use it to ask about someone respectfully: "tapai nepali siknuhunchha?" (are you learning Nepali?). It is the same shape you met in "tapai ke gardai hunuhunchha?"',
			},
			{
				id: 'ma-herchu-i-watch',
				dev: 'म हेर्छु',
				en: 'I watch / I look',
				enEither: true,
				usage: 'From "hernu" (to see, watch, look). "Ma TV herchu" (I watch TV). Same "-chu" = "I" ending as every verb in this unit.',
				// T29 depth (AI-drafted → Ross's review):
				frames: [
					{ dev: 'म टिभी हेर्छु', en: 'I watch TV' },
					{ dev: 'म फिल्म हेर्छु', en: 'I watch a movie' },
				],
			},
			{
				id: 'ma-bhanchu-i-say',
				dev: 'म भन्छु',
				en: 'I say',
				usage: 'From "bhannu" (to say, tell). "Ma pheri bhanchu" (I\'ll say it again). Its polite command form "bhannus" (please say it) may already be familiar.',
				// T29 depth (AI-drafted → Ross's review):
				frames: [
					{ dev: 'बिस्तारै भन्नुस्', en: 'Please say it slowly' },
					{ dev: 'फेरि भन्नुस्', en: 'Please say it again' },
				],
			},
			{
				id: 'ma-sodhchu-i-ask',
				dev: 'म सोध्छु',
				en: 'I ask',
				usage: 'From "sodhnu" (to ask a question). "Ma sodhchu" (I\'ll ask). Pairs naturally with question words like "ke" (what) and "kahaa" (where).',
				// T29 depth (AI-drafted → Ross's review):
				frames: [
					{ dev: 'म बाटो सोध्छु', en: 'I ask for directions' },
					{ dev: 'म प्रश्न सोध्छु', en: 'I ask a question' },
				],
			},
			{
				id: 'ma-padhchu-i-read',
				dev: 'म पढ्छु',
				en: 'I read',
				usage: 'From "padhnu" (to read, study). "Ma kitaab padhchu" (I read a book — "kitaab" is book). Nepali uses one verb for both reading and studying.',
				// T29 depth (AI-drafted → Ross's review):
				frames: [
					{ dev: 'म किताब पढ्छु', en: 'I read a book' },
					{ dev: 'म समाचार पढ्छु', en: 'I read the news' },
				],
			},
			{
				id: 'ma-lekhchu-i-write',
				dev: 'म लेख्छु',
				en: 'I write',
				usage: 'From "lekhnu" (to write). "Ma lekhchu" (I write / I\'ll write it down). You saw the root in "lekhera dinuhola" (please write it down).',
				// T29 depth (AI-drafted → Ross's review):
				frames: [
					{ dev: 'म चिठी लेख्छु', en: 'I write a letter' },
					{ dev: 'म नाम लेख्छु', en: 'I write my name' },
				],
			},
			{
				id: 'ma-dinchu-i-give',
				dev: 'म दिन्छु',
				en: 'I give',
				usage: 'From "dinu" (to give). A vowel-ending root slips an "n" in before the ending: "din-chu." "Ma dinchu" (I\'ll give it). Its command form is "dinus" (please give).',
				// T29 depth (AI-drafted → Ross's review):
				frames: [
					{ dev: 'म उपहार दिन्छु', en: 'I give a gift' },
					{ dev: 'मलाई पानी दिनुस्', en: 'Please give me water' },
				],
			},
			{
				id: 'ma-linchu-i-take',
				dev: 'म लिन्छु',
				en: 'I take',
				usage: 'From "linu" (to take), the natural partner of "dinu" (give): "ma linchu" (I\'ll take it). Like "din-chu," the vowel root adds an "n": "lin-chu."',
				// T29 depth (AI-drafted → Ross's review):
				frames: [
					{ dev: 'म फोटो लिन्छु', en: 'I take a photo' },
					{ dev: 'यो लिनुस्', en: 'Please take this' },
				],
			},
			{
				id: 'ma-piunchu-i-drink',
				dev: 'म पिउँछु',
				en: 'I drink',
				usage: 'From "piunu" (to drink). "Ma paani piunchu" (I drink water). Note: Nepali "eats" tea and other warm drinks ("chiya khanchu"), but "piunu" covers drinking in general.',
				// T29 depth (AI-drafted → Ross's review):
				frames: [
					{ dev: 'म पानी पिउँछु', en: 'I drink water' },
					{ dev: 'म चिया पिउँछु', en: 'I drink tea' },
				],
			},
			{
				id: 'u-git-sunchha-he-she-listens',
				dev: 'ऊ गीत सुन्छ',
				en: 'He / she listens to music (informal)',
				enAlt: ['He listens to music', 'She listens to music'],
				usage: 'For "he/she" (informal "u") the ending is "-cha / -chha": "u git sunchha" (he listens to songs — "git" is a song), from "sunnu" (to hear, listen). So "-chu" = I, "-nuhunchha" = you (polite), "-cha" = he/she.',
				// T29 depth (AI-drafted → Ross's review):
				frames: [
					{ dev: 'ऊ रेडियो सुन्छ', en: 'He listens to the radio' },
					{ dev: 'ऊ समाचार सुन्छ', en: 'He listens to the news' },
				],
			},
		],
	},
	{
		id: 'daily-routine',
		title: 'Daily Routine',
		kind: 'phrases',
		goal: 'Talk through your daily routine',
		items: [
			{
				id: 'ma-uthchu-i-get-up-i-will-get-up',
				dev: 'म उठ्छु',
				en: 'I get up / I will get up',
				enEither: true,
				usage: '"Uthnu" means to rise or get up. "Ma bihaana sabere uthchu" — I get up early in the morning.',
			},
			{
				id: 'uthnus-please-get-up-wake-up',
				dev: 'उठ्नुस्!',
				en: 'Get up / Wake up',
				enEither: true,
				usage: 'The polite wake-up call. For family you might hear the informal "utha!" instead.',
			},
			{
				id: 'ma-mukh-dhunchhu-i-will-wash-my-face',
				dev: 'म मुख धुन्छु',
				en: 'I will wash my face',
				usage: '"Mukh" means face (and mouth). Washing your face is the standard Nepali morning freshen-up, mentioned as routinely as brushing teeth.',
			},
			{
				id: 'ma-daat-maajhchu-i-will-brush-my-teeth',
				dev: 'म दाँत माझ्छु',
				en: 'I will brush my teeth',
				usage: '"Daat" is teeth, and "maajhnu" (to scrub) is the same verb as in "bhada maajhchu" (washing dishes). Scrubbing is scrubbing!',
			},
			{
				id: 'ma-kaam-jaanchu-i-am-going-to-work',
				dev: 'म काम जान्छु',
				en: 'I am going to work',
				usage: 'The "jaanchu" pattern with "kaam" (work). The standard morning departure line.',
			},
			{
				id: 'ma-ghar-aaye-i-am-home-i-came-home',
				dev: 'म घर आएँ',
				en: 'I came home',
				usage: '"Aaye" is the past tense of "aaunu" (to come). Announce yourself at the door with this.',
			},
			{
				id: 'khana-khane-bela-bhayo-it-is-time-to-eat',
				dev: 'खाना खाने बेला भयो',
				en: 'It is time to eat',
				usage: '"Bela" (time) again — literally "food-eating time happened." The call that gathers the household.',
			},
			{
				id: 'ma-thakeko-chu-i-am-tired',
				dev: 'म थकेको छु',
				en: 'I am tired',
				usage: 'Uses "thakeko" (tired) with "chu" (I am). The honest end-of-day report, and a polite path toward excusing yourself to bed.',
			},
			{
				id: 'dhilo-bhayo-it-is-late-i-am-running-late',
				dev: 'ढिलो भयो',
				en: 'It is late',
				usage: '"Dhilo" means late. Works for the hour ("it has gotten late") and for yourself when rushing out the door.',
			},
			{
				id: 'ma-chaadai-aaunchu-i-will-come-back-soon',
				dev: 'म चाँडै आउँछु',
				en: 'I will come back soon',
				usage: '"Chaadai" means soon or early. A fuller version of "Ma ahile aaunchu" when you will be gone a bit longer.',
			},
			{
				id: 'ramro-sanga-sutnus-sleep-well',
				dev: 'राम्रोसँग सुत्नुस्',
				en: 'Sleep well',
				usage: 'Literally "sleep with goodness." A warmer companion to "subha ratri" within the household.',
			},
			{
				id: 'subha-din-have-a-good-day',
				dev: 'शुभ दिन',
				en: 'Have a good day',
				usage: '"Din" means day — the same pattern as "subha prabhat" and "subha ratri." A slightly formal but kind send-off.',
			},
		],
	},
	{
		id: 'verbs-past',
		title: 'Verbs: Past tense',
		kind: 'phrases',
		goal: 'Say what you did — talk in the past tense',
		items: [
			{
				id: 'maile-siken-i-learned',
				dev: 'मैले सिकेँ',
				en: 'I learned',
				usage: 'The past tense. For most verbs (the ones that take an object), "I" becomes "maile" — not "ma" — and the verb ends in "-en," a nasal "eh": "maile siken" (I learned, from "siknu"). That "maile" subject is the big shift from the present.',
				// T29 batch 2 (past-tense verbs; AI-drafted, approved):
				frames: [
					{ dev: 'मैले नेपाली सिकेँ', en: 'I learned Nepali' },
					{ dev: 'मैले धेरै सिकेँ', en: 'I learned a lot' },
				],
			},
			{
				id: 'ma-gaen-i-went',
				dev: 'म गएँ',
				en: 'I went',
				usage: 'From "jaanu" (to go) — but the past stem is irregular: "ga-." Verbs of motion keep plain "ma" (no "maile"): "ma gaen" (I went), "ma bajaar gaen" (I went to the market).',
				// T29 batch 2 (past-tense verbs; AI-drafted, approved):
				frames: [
					{ dev: 'म बजार गएँ', en: 'I went to the market' },
					{ dev: 'म स्कूल गएँ', en: 'I went to school' },
				],
			},
			{
				id: 'ma-aaen-i-came',
				dev: 'म आएँ',
				en: 'I came',
				usage: 'From "aaunu" (to come), the partner of "go." Like "gaen," it keeps "ma": "ma aaen" (I came / I got back). Its present was "ma ahile aaunchu" (I\'ll be right back).',
				// T29 batch 2 (past-tense verbs; AI-drafted, approved):
				frames: [
					{ dev: 'म हिजो आएँ', en: 'I came yesterday' },
					{ dev: 'म घर आएँ', en: 'I came home' },
				],
			},
			{
				id: 'maile-garen-i-did',
				dev: 'मैले गरेँ',
				en: 'I did',
				usage: 'From "garnu" (to do), the verb you reach for everywhere. "Maile kaam garen" (I did the work). Back to "maile" + "-en," because "do" takes an object.',
				// T29 batch 2 (past-tense verbs; AI-drafted, approved):
				frames: [
					{ dev: 'मैले काम गरेँ', en: 'I did the work' },
					{ dev: 'मैले गल्ती गरेँ', en: 'I made a mistake' },
				],
			},
			{
				id: 'maile-khaen-i-ate',
				dev: 'मैले खाएँ',
				en: 'I ate',
				usage: 'From "khanu" (to eat). "Maile khana khaen" (I ate a meal). Compare the present "ma khanchu" (I eat) — the past adds "maile" and the "-en" ending.',
				// T29 batch 2 (past-tense verbs; AI-drafted, approved):
				frames: [
					{ dev: 'मैले भात खाएँ', en: 'I ate rice' },
					{ dev: 'मैले धेरै खाएँ', en: 'I ate a lot' },
				],
			},
			{
				id: 'maile-pien-i-drank',
				dev: 'मैले पिएँ',
				en: 'I drank',
				usage: 'From "piunu" (to drink). "Maile paani pien" (I drank water). For warm drinks Nepali still "eats": "maile chiya khaen" (I had tea).',
				// T29 batch 2 (past-tense verbs; AI-drafted, approved). Frame 1 uses खानु for
				// tea — the colloquial verb, per the usage note (swapped from पिएँ, Ross-approved):
				frames: [
					{ dev: 'मैले चिया खाएँ', en: 'I had tea' },
					{ dev: 'मैले पानी पिएँ', en: 'I drank water' },
				],
			},
			{
				id: 'maile-heren-i-watched',
				dev: 'मैले हेरेँ',
				en: 'I looked',
				usage: 'From "hernu" (to watch, look). "Maile TV heren" (I watched TV). Present "ma herchu" becomes past "maile heren."',
				// T29 batch 2 (past-tense verbs; AI-drafted, approved):
				frames: [
					{ dev: 'मैले फिल्म हेरेँ', en: 'I watched a movie' },
					{ dev: 'मैले टिभी हेरेँ', en: 'I watched TV' },
				],
			},
			{
				id: 'maile-sunen-i-heard',
				dev: 'मैले सुनेँ',
				en: 'I heard',
				usage: 'From "sunnu" (to hear, listen). "Maile git sunen" (I listened to a song). The "-en" ending is nasal — the Devanagari marks it with the candrabindu (ँ).',
				// T29 batch 2 (past-tense verbs; AI-drafted, approved):
				frames: [
					{ dev: 'मैले गीत सुनेँ', en: 'I heard a song' },
					{ dev: 'मैले समाचार सुनेँ', en: 'I heard the news' },
				],
			},
			{
				id: 'maile-bhanen-i-said',
				dev: 'मैले भनेँ',
				en: 'I said',
				usage: 'From "bhannu" (to say, tell). "Maile bhanen" (I said it / I told them) — a handy way to report what you already mentioned.',
				// T29 batch 2 (past-tense verbs; AI-drafted, approved):
				frames: [
					{ dev: 'मैले धन्यवाद भनेँ', en: 'I said thank you' },
					{ dev: 'मैले फेरि भनेँ', en: 'I said it again' },
				],
			},
			{
				id: 'maile-padhen-i-read',
				dev: 'मैले पढेँ',
				en: 'I read (past)',
				usage: 'From "padhnu" (to read, study). "Maile kitaab padhen" (I read a book). The same verb also gives "I studied."',
				// T29 batch 2 (past-tense verbs; AI-drafted, approved):
				frames: [
					{ dev: 'मैले किताब पढेँ', en: 'I read a book' },
					{ dev: 'मैले समाचार पढेँ', en: 'I read the news' },
				],
			},
			{
				id: 'maile-lekhen-i-wrote',
				dev: 'मैले लेखेँ',
				en: 'I wrote',
				usage: 'From "lekhnu" (to write). "Maile lekhen" (I wrote it down). Present "ma lekhchu" becomes past "maile lekhen."',
				// T29 batch 2 (past-tense verbs; AI-drafted, approved):
				frames: [
					{ dev: 'मैले चिठी लेखेँ', en: 'I wrote a letter' },
					{ dev: 'मैले नाम लेखेँ', en: 'I wrote my name' },
				],
			},
			{
				id: 'maile-dien-i-gave',
				dev: 'मैले दिएँ',
				en: 'I gave',
				usage: 'From "dinu" (to give). "Maile dien" (I gave it). Its partner "linu" (take) works the same way: "maile linen" (I took).',
				// T29 batch 2 (past-tense verbs; AI-drafted, approved):
				frames: [
					{ dev: 'मैले पैसा दिएँ', en: 'I gave money' },
					{ dev: 'मैले उपहार दिएँ', en: 'I gave a gift' },
				],
			},
		],
	},
	// --- T11 batch 1: everyday verbs (frames), interleaved after the past tense. Titles/goals AI-drafted.
	{
		id: 'verbs-reactions',
		title: 'Reactions & Opinions',
		kind: 'phrases',
		goal: 'React and give your opinion — say you understand, agree, and what you want',
		items: [
			{
				id: 'kati-bhayo-how-much-was-it',
				dev: 'कति भयो?',
				en: 'How much was it?',
				usage: "भयो is the past of हुनु ('became / was'). The go-to way to ask a total when you're paying.",
			},
			{
				id: 'ma-bujhchu-i-understand',
				dev: 'म बुझ्छु',
				en: 'I understand',
				usage: 'बुझ्नु = to understand.',
			},
			{
				id: 'ma-maanchu-i-agree',
				dev: 'म मान्छु',
				en: 'I accept',
				usage: 'मान्नु = to accept / agree / believe.',
			},
			{
				id: 'ma-chaahanchu-i-want',
				dev: 'म चाहन्छु',
				en: 'I want (it)',
				usage: "चाहनु = to want / wish. Put a noun before it: म चिया चाहन्छु 'I want tea'.",
			},
			{
				id: 'malai-chaahincha-i-need',
				dev: 'मलाई ___ चाहिन्छ',
				en: 'I need ___',
				usage: "चाहिनु = to need / be required. Fill the slot: मलाई पानी चाहिन्छ 'I need water'.",
			},
			{
				id: 'ramailo-bhayo-it-was-fun',
				dev: 'रमाइलो भयो',
				en: 'It was fun',
				usage: 'A warm way to say you enjoyed something. भयो is the past of हुनु.',
			},
			{
				id: 'malai-man-paryo-i-liked-it',
				dev: 'मलाई मन पर्यो',
				en: 'I liked it',
				usage: "मन पर्नु = to like. मलाई मन पर्यो — 'I liked it', perfect after a meal or a visit.",
			},
			{
				id: 'muulya-badhyo-the-price-went-up',
				dev: 'मूल्य बढ्यो',
				en: 'The price went up',
				usage: 'बढ्नु = to increase / grow / rise (on its own).',
			},
			{
				id: 'muulya-ghatyo-the-price-went-down',
				dev: 'मूल्य घट्यो',
				en: 'The price went down',
				usage: 'घट्नु = to decrease / go down (the opposite of बढ्नु).',
			},
			{
				id: 'malai-bujhaaunus-please-explain-to-me',
				dev: 'मलाई बुझाउनुस्',
				en: 'Please explain to me',
				usage: "बुझाउनु = to explain / make (someone) understand (from बुझ्नु 'to understand').",
			},
		],
	},
	{
		id: 'verbs-requests',
		title: 'Asking for Help',
		kind: 'phrases',
		goal: 'Ask politely for help — please tell me, bring, show, or find something',
		items: [
			{
				id: 'malai-saghaaunus-please-help-me',
				dev: 'मलाई सघाउनुस्',
				en: 'Please help me',
				usage: 'सघाउनु = to help / assist. The polite -नुस् makes it a request.',
			},
			{
				id: 'bistaarai-bolnus-please-speak-slowly',
				dev: 'बिस्तारै बोल्नुस्',
				en: 'Please speak slowly',
				usage: 'बोल्नु = to speak. Handy when someone talks too fast.',
			},
			{
				id: 'malai-bataaunus-please-tell-me',
				dev: 'मलाई बताउनुस्',
				en: 'Please tell me',
				usage: "बताउनु = to tell / inform (give information), distinct from भन्नु 'to say'.",
			},
			{
				id: 'malai-dekhaaunus-please-show-me',
				dev: 'मलाई देखाउनुस्',
				en: 'Please show me',
				usage: 'देखाउनु = to show / point out.',
			},
			{
				id: 'pani-lyaaunus-please-bring-water',
				dev: 'पानी ल्याउनुस्',
				en: 'Please bring water',
				usage: "ल्याउनु = to bring. Swap पानी for anything you'd like brought.",
			},
			{
				id: 'yo-raakhnus-please-put-this-here',
				dev: 'यो राख्नुस्',
				en: 'Please put this here',
				usage: 'राख्नु = to keep / put / place.',
			},
			{
				id: 'yo-nikaalnus-please-take-this-out',
				dev: 'यो निकाल्नुस्',
				en: 'Please take this out',
				usage: 'निकाल्नु = to take out / remove / withdraw.',
			},
			{
				id: 'yo-hataaunus-please-remove-this',
				dev: 'यो हटाउनुस्',
				en: 'Please remove this',
				usage: 'हटाउनु = to remove / take away / clear off.',
			},
			{
				id: 'saathi-bolaaunus-please-call-your-friend',
				dev: 'साथी बोलाउनुस्',
				en: 'Please call your friend over',
				usage: "बोलाउनु = to call / summon someone (not a phone call — that's फोन गर्नु).",
			},
			{
				id: 'yo-khojnus-please-look-for-this',
				dev: 'यो खोज्नुस्',
				en: 'Please look for this',
				usage: 'खोज्नु = to search / look for.',
			},
		],
	},
	{
		id: 'verbs-getting-around',
		title: 'Getting Around',
		kind: 'phrases',
		goal: 'Get around town — stop here, come back, take a seat, tell the time',
		items: [
			{
				id: 'ma-ghar-pugen-i-reached-home',
				dev: 'म घर पुगेँ',
				en: 'I reached home',
				usage: 'पुग्नु = to arrive / reach / be enough.',
			},
			{
				id: 'yaha-roknus-please-stop-here',
				dev: 'यहाँ रोक्नुस्',
				en: 'Please stop here',
				usage: 'रोक्नु = to stop / halt. Useful to a driver.',
			},
			{
				id: 'ma-pharkinchu-ill-come-back',
				dev: 'म फर्किन्छु',
				en: "I'll come back",
				usage: 'फर्किनु = to return / go back.',
			},
			{
				id: 'yaha-basnus-please-have-a-seat',
				dev: 'यहाँ बस्नुस्',
				en: 'Please have a seat',
				usage: "बस्नु = to sit / stay / live. As an invitation it means 'have a seat'.",
			},
			{
				id: 'dhoka-khulcha-the-door-opens',
				dev: 'ढोका खुल्छ',
				en: 'The door opens',
				usage: "खुल्नु = to open (by itself). Also: पसल खुल्छ 'the shop opens'.",
			},
			{
				id: 'gaadi-chalaaunus-please-drive-the-car',
				dev: 'गाडी चलाउनुस्',
				en: 'Please drive the car',
				usage: 'चलाउनु = to operate / drive / run something.',
			},
			{
				id: 'yo-chaldaina-this-isnt-working',
				dev: 'यो चल्दैन',
				en: "This isn't working",
				usage: 'चल्नु = to move / run / function. The negative for something broken.',
			},
			{
				id: 'ek-bajyo-its-one-oclock',
				dev: 'एक बज्यो',
				en: "It's one o'clock",
				usage: "बज्नु = to ring / strike. एक बज्यो is literally 'one has struck'.",
			},
			{
				id: 'yo-laijaanus-please-take-this-along',
				dev: 'यो लैजानुस्',
				en: 'Please take this along',
				usage: "लैजानु = to take / carry away (the opposite of ल्याउनु 'to bring').",
			},
			{
				id: 'yo-uthaaunus-please-pick-this-up',
				dev: 'यो उठाउनुस्',
				en: 'Please pick this up',
				usage: 'उठाउनु = to lift / raise / pick up (also to wake someone).',
			},
		],
	},
	{
		id: 'verbs-making-doing',
		title: 'Making & Doing',
		kind: 'phrases',
		goal: 'Talk about making and doing — cook, begin, send, carry, and deliver',
		items: [
			{
				id: 'ma-khaana-banaaunchu-i-cook',
				dev: 'म खाना बनाउँछु',
				en: 'I make food',
				usage: 'बनाउनु = to make / build / prepare.',
			},
			{
				id: 'kaam-thaalnus-please-begin-the-work',
				dev: 'काम थाल्नुस्',
				en: 'Please begin the work',
				usage: 'थाल्नु = to begin / start an activity.',
			},
			{
				id: 'yo-jodnus-please-join-these',
				dev: 'यो जोड्नुस्',
				en: 'Please join these together',
				usage: 'जोड्नु = to join / connect / add.',
			},
			{
				id: 'yo-pathaaunus-please-send-this',
				dev: 'यो पठाउनुस्',
				en: 'Please send this',
				usage: 'पठाउनु = to send / dispatch / forward.',
			},
			{
				id: 'maile-paaen-i-got-it',
				dev: 'मैले पाएँ',
				en: 'I received it',
				usage: "पाउनु = to get / receive / obtain (also 'to be allowed').",
			},
			{
				id: 'ma-bokchu-ill-carry-it',
				dev: 'म बोक्छु',
				en: "I'll carry it",
				usage: 'बोक्नु = to carry / bear (on your body).',
			},
			{
				id: 'paisa-jogaaunus-save-money',
				dev: 'पैसा जोगाउनुस्',
				en: 'Save money',
				usage: 'जोगाउनु = to save / protect / keep safe.',
			},
			{
				id: 'shaanta-rahanus-please-stay-calm',
				dev: 'शान्त रहनुस्',
				en: 'Please stay calm',
				usage: 'रहनु = to remain / stay (in a state).',
			},
			{
				id: 'ma-bannu-i-will-become',
				dev: 'म ___ बन्छु',
				en: 'I will become ___',
				usage: "बन्नु = to become / be made. Fill the slot with a role: म डाक्टर बन्छु 'I will become a doctor'.",
			},
			{
				id: 'ghar-samma-puryaaunus-please-deliver',
				dev: 'घरसम्म पुर्याउनुस्',
				en: 'Please deliver it home',
				usage: 'पुर्याउनु = to deliver / take (something) all the way to a place.',
			},
		],
	},
	{
		id: 'verbs-everyday-actions',
		title: 'Everyday Actions',
		kind: 'phrases',
		goal: 'More everyday actions — meet, play, celebrate, and turn things up',
		items: [
			{
				id: 'tapailai-bhetchu-see-you',
				dev: 'तपाईंलाई भेट्छु',
				en: "I'll meet you",
				usage: 'भेट्नु = to meet.',
			},
			{
				id: 'baccha-khelcha-the-child-plays',
				dev: 'बच्चा खेल्छ',
				en: 'The child plays',
				usage: 'खेल्नु = to play.',
			},
			{
				id: 'haami-jitchaun-we-will-win',
				dev: 'हामी जित्छौं',
				en: 'We will win',
				usage: 'जित्नु = to win / defeat.',
			},
			{
				id: 'chaabi-haraayo-i-lost-the-key',
				dev: 'चाबी हरायो',
				en: 'The key is lost',
				usage: "हराउनु = to lose / go missing. चाबी हरायो — 'the key got lost'.",
			},
			{
				id: 'chaad-manaaunchaun-we-celebrate-the-festival',
				dev: 'चाड मनाउँछौं',
				en: 'We celebrate the festival',
				usage: 'मनाउनु = to celebrate / observe (a festival).',
			},
			{
				id: 'maile-dekhen-i-saw-it',
				dev: 'मैले देखेँ',
				en: 'I saw it',
				usage: "देख्नु = to see (noticing), distinct from हेर्नु 'to look / watch'.",
			},
			{
				id: 'tapai-thik-hunuhuncha-are-you-okay',
				dev: 'तपाईं ठीक हुनुहुन्छ?',
				en: 'Are you okay?',
				usage: 'A caring check-in (polite). हुनुहुन्छ is the polite present of हुनु.',
			},
			{
				id: 'sabai-milcha-it-all-works-out',
				dev: 'सबै मिल्छ',
				en: 'It all works out',
				usage: "मिल्नु = to match / fit / come together. A reassuring 'it'll all work out'.",
			},
			{
				id: 'jutta-lagaaunus-please-put-on-your-shoes',
				dev: 'जुत्ता लगाउनुस्',
				en: 'Please put on your shoes',
				usage: 'लगाउनु = to wear / put on / apply.',
			},
			{
				id: 'aavaaj-badhaaunus-please-turn-up-the-volume',
				dev: 'आवाज बढाउनुस्',
				en: 'Please turn up the volume',
				usage: 'बढाउनु = to increase / raise / turn up (the causative of बढ्नु).',
			},
		],
	},
	{
		id: 'modals-can-want-must',
		title: 'Can, Want & Must',
		kind: 'phrases',
		goal: 'Say what you can, want, and have to do',
		items: [
			{
				id: 'ma-garna-sakchu-i-can-do',
				dev: 'म गर्न सक्छु',
				en: 'I can do it',
				usage: 'The "can" pattern: take the verb, add "-na," then "sakchu" (I am able). "Garnu" (to do) becomes "garna sakchu" (I can do). This "verb + na sak-" frame works with every verb you know.',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'म पढ्न सक्छु', en: 'I can read' },
					{ dev: 'म खान सक्छु', en: 'I can eat' },
				],
			},
			{
				id: 'ma-jana-sakchu-i-can-go',
				dev: 'म जान सक्छु',
				en: 'I can go',
				usage: 'From "jaanu" (to go): "jana sakchu" (I can go). Same "-na sak-" pattern as "garna sakchu." Add a place: "ma bajaar jana sakchu" (I can go to the market).',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'म आउन सक्छु', en: 'I can come' },
					{ dev: 'म बस्न सक्छु', en: 'I can stay' },
				],
			},
			{
				id: 'tapai-garna-saknuhunchha-can-you-do-it',
				dev: 'तपाईं गर्न सक्नुहुन्छ?',
				en: 'Can you do it? (polite)',
				usage: 'The polite "can you" question reuses the "-nuhunchha" ending from "tapai siknuhunchha?" Here it attaches to "sak-": "garna saknuhunchha?" (are you able to do it?).',
			},
			{
				id: 'ma-garna-sakdina-i-cannot-do',
				dev: 'म गर्न सक्दिनँ',
				en: 'I cannot do it',
				usage: 'The negative of "sakchu" is "sakdina" (I am not able). "Ma jana sakdina" (I cannot go). The "-dina" ending is the present "I" negative.',
			},
			{
				id: 'malai-jana-man-laagcha-i-want-to-go',
				dev: 'मलाई जान मन लाग्छ',
				en: 'I want to go',
				usage: 'The "want to" pattern: "verb + na" then "man laagcha" (literally "the mind feels"). The subject is "malai" (to me), not "ma": "malai jana man laagcha" (I feel like going).',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'मलाई सुत्न मन लाग्छ', en: 'I want to sleep' },
					{ dev: 'मलाई घर जान मन लाग्छ', en: 'I want to go home' },
				],
			},
			{
				id: 'malai-khana-man-laagcha-i-want-to-eat',
				dev: 'मलाई खान मन लाग्छ',
				en: 'I want to eat',
				usage: 'From "khanu" (to eat): "khana man laagcha" (I want to eat). Swap the verb to want anything: "malai sutna man laagcha" (I want to sleep).',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'मलाई चिया पिउन मन लाग्छ', en: 'I want to drink tea' },
					{ dev: 'मलाई हेर्न मन लाग्छ', en: 'I want to watch' },
				],
			},
			{
				id: 'malai-man-laagdaina-i-dont-want-to',
				dev: 'मलाई जान मन लाग्दैन',
				en: "I don't want to go",
				usage: 'The negative of "man laagcha" is "man laagdaina" (the mind does not feel like it). "Malai jana man laagdaina" (I don\'t feel like going).',
			},
			{
				id: 'malai-jaanu-parcha-i-have-to-go',
				dev: 'मलाई जानु पर्छ',
				en: 'I have to go',
				usage: 'The "must / have to" pattern: the full "-nu" verb then "parcha" (it is required). You met it in "sochnu parcha" (I have to think). Like "want," it takes "malai": "malai jaanu parcha" (I have to go).',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'मलाई पढ्नु पर्छ', en: 'I have to study' },
					{ dev: 'मलाई अहिले जानु पर्छ', en: 'I have to go now' },
				],
			},
			{
				id: 'malai-kaam-garnu-parcha-i-have-to-work',
				dev: 'मलाई काम गर्नु पर्छ',
				en: 'I have to work',
				usage: 'From "garnu" (to do): "kaam garnu parcha" (I have to do the work). The obligation pattern uses the "-nu" form, the same one in "sochnu parcha."',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'मलाई किन्नु पर्छ', en: 'I have to buy it' },
					{ dev: 'मलाई सिक्नु पर्छ', en: 'I have to learn' },
				],
			},
			{
				id: 'malai-jaanu-pardaina-i-dont-have-to-go',
				dev: 'मलाई जानु पर्दैन',
				en: "I don't have to go",
				usage: 'The negative of "parcha" is "pardaina" (it is not required). Contrast "garnu parcha" (must do) with "garnu pardaina" (need not do).',
			},
			{
				id: 'ke-garnu-parcha-what-should-i-do',
				dev: 'के गर्नु पर्छ?',
				en: 'What should I do?',
				usage: 'Combine the question word "ke" (what) with the obligation pattern: "ke garnu parcha?" (what must one do? / what should I do?). A handy way to ask for instructions.',
			},
		],
	},
	{
		id: 'cooking',
		title: 'Cooking Together',
		kind: 'phrases',
		goal: 'Pitch in while cooking',
		items: [
			{
				id: 'ma-tarkari-kaatchu-i-will-cut-the-vegetables',
				dev: 'म तरकारी काट्छु',
				en: 'I will cut the vegetables',
				usage: '"Kaatnu" means to cut or chop. The classic way to be useful in a Nepali kitchen without being trusted with the spices yet.',
			},
			{
				id: 'paani-umaalnus-please-boil-the-water',
				dev: 'पानी उमाल्नुस्',
				en: 'Please boil the water',
				usage: '"Umaalnu" means to boil. Step one of chiya, daal, and most of dinner.',
			},
			{
				id: 'bhaat-pakaunus-please-cook-the-rice',
				dev: 'भात पकाउनुस्',
				en: 'Please cook the rice',
				usage: '"Pakaunu" (to cook) from the pattern you know, now as a polite request. "Bhaat" is cooked rice — the center of the Nepali meal.',
			},
			{
				id: 'yo-piro-cha-is-this-spicy',
				dev: 'यो पिरो छ?',
				en: 'Is this spicy?',
				usage: '"Piro" means spicy-hot. Possibly the most important question in this entire course. Ask before, not after.',
			},
			{
				id: 'dherai-piro-cha-it-is-very-spicy',
				dev: 'धेरै पिरो छ',
				en: 'It is very spicy',
				usage: 'When the answer to "yo piro cha?" arrives too late. "Dherai" (very) plus "piro." Keep water nearby — or better, dahi.',
			},
			{
				id: 'nun-pugyo-there-is-enough-salt',
				dev: 'नुन पुग्यो',
				en: 'There is enough salt',
				usage: '"Pugyo" (enough) applied to cooking. The verdict after tasting: "nun pugyo" — the salt has arrived where it needs to be.',
			},
			{
				id: 'aru-nun-chaahiyo-it-needs-more-salt',
				dev: 'अरु नुन चाहियो',
				en: 'It needs more salt',
				usage: '"Aru" (more) + "chaahiyo" (is needed). The other possible verdict. Salt opinions are strong in every kitchen on earth.',
			},
			{
				id: 'chakhnus-please-taste-it',
				dev: 'चाख्नुस्',
				en: 'Please taste it',
				usage: '"Chakhnu" means to taste. An invitation and an honor — the cook wants your opinion. Answer with "mitho cha!" if at all possible.',
			},
			{
				id: 'tato-cha-it-is-hot-to-the-touch',
				dev: 'तातो छ',
				en: 'It is hot (to the touch)',
				usage: '"Tato" is hot for objects and food — different from "garmi," which is hot weather. Said while handing someone a fresh cup of chiya.',
			},
			{
				id: 'hosh-garnus-be-careful',
				dev: 'होश गर्नुस्',
				en: 'Be careful',
				usage: '"Hosh" means awareness or care. Heard around hot pots, sharp knives, and slippery floors — anywhere in a busy kitchen.',
			},
			{
				id: 'ramro-basna-aayo-it-smells-good',
				dev: 'राम्रो बास्ना आयो',
				en: 'It smells good',
				usage: '"Basna" is smell or aroma — literally "a good smell came." The compliment that drifts into the kitchen before dinner.',
			},
			{
				id: 'khana-sakiyo-the-food-is-finished-all-gone',
				dev: 'खाना सकियो',
				en: 'The food is finished',
				usage: '"Sakinu" means to be finished or used up. Works for the pot being empty and for declaring the meal over.',
			},
		],
	},
	{
		id: 'purchasing',
		title: 'Purchasing',
		kind: 'phrases',
		goal: 'Ask prices and bargain',
		items: [
			{
				id: 'kati-ho-how-much-is-it',
				dev: 'कति हो?',
				en: 'How much is it?',
				usage: 'Essential for shopping and bargaining, this phrase asks about price or quantity. "Kati" means "how much" or "how many." Shopkeepers will understand this immediately and tell you the price.',
			},
			{
				id: 'mahango-cha-it-s-expensive',
				dev: 'महँगो छ',
				en: "It's expensive",
				usage: 'Use this when you think the price is too high. Bargaining is common in Nepal\'s markets, and saying something is expensive might lead to a lower price. "Mahango" means expensive and "cha" means "is."',
			},
			{
				id: 'sasto-cha-it-s-cheap',
				dev: 'सस्तो छ',
				en: "It's cheap",
				usage: 'The opposite of mahango, this phrase indicates something is inexpensive or affordable. "Sasto" means cheap or inexpensive. You can use this to compliment a good price or good value.',
			},
			{
				id: 'kati-ma-for-how-much',
				dev: 'कतिमा?',
				en: 'For how much?',
				usage: 'Asks about price or cost. "Ma" in this context means "for." Used when bargaining or asking about prices. Shopkeepers expect this question in markets.',
			},
			{
				id: 'kam-garnuhos-please-reduce-make-it-less',
				dev: 'कम गर्नुहोस्',
				en: 'Please reduce',
				usage: 'Used when bargaining to ask for a lower price. "Kam" means less or reduce. Bargaining is common in Nepali markets, and sellers often expect some negotiation.',
			},
			{
				id: 'antim-mol-kati-what-s-your-final-price',
				dev: 'अन्तिम मोल कति?',
				en: "What's your final price?",
				usage: 'Asks for the best/final price. "Antim" means final or last, and "mol" means price. Use this near the end of bargaining to get the seller\'s best offer.',
			},
			{
				id: 'aru-ke-chaahiyo-what-else-do-you-need',
				dev: 'अरु के चाहियो?',
				en: 'What else do you need?',
				usage: 'Shopkeepers often ask this after you\'ve purchased something. "Aru" means other or else, "ke" means what, and "chaahiyo" means needed. You can say "aru kehi chhaina" (nothing else).',
			},
			{
				id: 'aru-kehi-chhaina-nothing-else',
				dev: 'अरु केही छैन',
				en: 'Nothing else',
				usage: 'Indicates you don\'t need anything else. "Kehi" means something/anything. A polite way to decline further offers from shopkeepers or hosts.',
			},
		],
	},
	{
		id: 'time',
		title: 'Time of Day',
		kind: 'phrases',
		goal: 'Talk about times of day',
		items: [
			{
				id: 'kahile-when',
				dev: 'कहिले?',
				en: 'When?',
				usage: "A simple question word asking about time. You can use it to ask when something will happen, when someone will arrive, or when an event occurred. It's useful in planning and making arrangements.",
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'पसल कहिले खुल्छ?', en: 'When does the shop open?' },
					{ dev: 'खाना कहिले तयार हुन्छ?', en: 'When will the food be ready?' },
				],
			},
			{
				id: 'aaja-today',
				dev: 'आज',
				en: 'Today',
				usage: 'Refers to the current day. You\'ll hear this often in daily conversation when discussing plans or events. It can be combined with other words like "aaja nai" (today itself, meaning right away).',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'आज मेरो जन्मदिन हो', en: 'Today is my birthday' },
					{ dev: 'आज राम्रो दिन हो', en: 'Today is a good day' },
				],
			},
			{
				id: 'bholi-tomorrow',
				dev: 'भोलि',
				en: 'Tomorrow',
				usage: 'Refers to the next day. Interestingly, "bholi" is also used colloquially to mean "later" or "in the future," and Nepal has a reputation for "bholi culture" where things are often postponed until tomorrow.',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'भोलि बिदा छ', en: 'Tomorrow is a day off' },
					{ dev: 'भोलि हामी बजार जान्छौं', en: 'Tomorrow we go to the market' },
				],
			},
			{
				id: 'hijo-yesterday',
				dev: 'हिजो',
				en: 'Yesterday',
				usage: "Refers to the previous day. This word is commonly used when recounting recent events or experiences. It's a straightforward time marker in everyday conversation.",
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'हिजो म बजार गएँ', en: 'Yesterday I went to the market' },
					{ dev: 'हिजो राति जाडो थियो', en: 'Last night was cold' },
				],
			},
			{
				id: 'bihaana-morning',
				dev: 'बिहान',
				en: 'Morning',
				usage: 'The morning hours. "Bihaana sabere" means early in the morning. Often used with meals: morning daal bhaat is eaten mid-morning in most households.',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'बिहान हावा सफा हुन्छ', en: 'The morning air is clean' },
					{ dev: 'म बिहान चिया खान्छु', en: 'I have tea in the morning' },
				],
			},
			{
				id: 'diuso-afternoon-daytime',
				dev: 'दिउँसो',
				en: 'Afternoon / Daytime',
				enEither: true,
				usage: 'The middle of the day, roughly late morning through afternoon. "Diuso khana" can refer to a midday meal or snack.',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'दिउँसो गर्मी हुन्छ', en: 'It gets hot in the afternoon' },
					{ dev: 'म दिउँसो सुत्छु', en: 'I nap in the afternoon' },
				],
			},
			{
				id: 'beluka-evening',
				dev: 'बेलुका',
				en: 'Evening',
				usage: 'The evening hours, from late afternoon until night. "Beluka bhetaula" — see you this evening.',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'बेलुका के गर्नुहुन्छ?', en: 'What do you do in the evening?' },
					{ dev: 'हामी बेलुका टिभी हेर्छौं', en: 'We watch TV in the evening' },
				],
			},
			{
				id: 'raati-night',
				dev: 'राति',
				en: 'Night',
				usage: 'Nighttime. You have seen it inside "Subha ratri" (good night). "Raati abela" means late at night.',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'राति चिसो हुन्छ', en: 'It gets cold at night' },
					{ dev: 'राति ढोका बन्द गर्नुस्', en: 'Lock the door at night' },
				],
			},
		],
	},
	{
		id: 'time-week',
		title: 'Days & the Clock',
		kind: 'phrases',
		goal: 'Name the days and tell the time',
		items: [
			{
				id: 'aaitabaar-sunday',
				dev: 'आइतबार',
				en: 'Sunday',
				usage: 'The first day of the Nepali week — and a normal working day in Nepal, unlike in Western countries.',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'आज आइतबार हो', en: 'Today is Sunday' },
					{ dev: 'आइतबार म काम गर्छु', en: 'I work on Sundays' },
				],
			},
			{
				id: 'sombaar-monday',
				dev: 'सोमबार',
				en: 'Monday',
				usage: 'All weekday names end in "-baar" (day). "Som" relates to the moon, like "lunes" or "Monday" in other languages.',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'सोमबार भेटौंला', en: 'See you Monday' },
					{ dev: 'सोमबार पसल खुला छ', en: 'The shop is open on Monday' },
				],
			},
			{
				id: 'mangalbaar-tuesday',
				dev: 'मङ्गलबार',
				en: 'Tuesday',
				usage: 'Tuesday. The "-baar" ending marks the days of the week; "Mangal" is Mars. Sits between "Sombaar" (Monday) and "Budhabaar" (Wednesday).',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'मङ्गलबार म व्यस्त छु', en: "I'm busy on Tuesday" },
					{ dev: 'मङ्गलबार आउनुस्', en: 'Come on Tuesday' },
				],
			},
			{
				id: 'budhabaar-wednesday',
				dev: 'बुधबार',
				en: 'Wednesday',
				usage: 'Wednesday, from "Budha" (Mercury) plus "-baar" (day). Comes after "Mangalbaar" (Tuesday) and before "Bihibaar" (Thursday).',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'आज बुधबार हो?', en: 'Is today Wednesday?' },
					{ dev: 'बुधबार म बजार जान्छु', en: 'On Wednesday I go to the market' },
				],
			},
			{
				id: 'bihibaar-thursday',
				dev: 'बिहीबार',
				en: 'Thursday',
				usage: 'Thursday — "Bihi" plus "-baar" (day). Follows "Budhabaar" (Wednesday) and leads into "Sukrabaar" (Friday).',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'बिहीबार म घर सफा गर्छु', en: 'On Thursday I clean the house' },
					{ dev: 'बिहीबार म फिल्म हेर्छु', en: 'On Thursday I watch a movie' },
				],
			},
			{
				id: 'sukrabaar-friday',
				dev: 'शुक्रबार',
				en: 'Friday',
				usage: 'Friday, from "Sukra" (Venus) plus "-baar" (day). The lead-in to "Sanibaar" (Saturday), the usual day off in Nepal.',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'शुक्रबार बेलुका रमाइलो हुन्छ', en: 'Friday evening is fun' },
					{ dev: 'शुक्रबार अन्तिम दिन हो', en: 'Friday is the last day' },
				],
			},
			{
				id: 'sanibaar-saturday',
				dev: 'शनिबार',
				en: 'Saturday',
				usage: 'The weekly holiday in Nepal — offices and schools close on Saturday, not Sunday. Weekend plans revolve around Sanibaar.',
				// T29 batch 12 (numbers & weekdays; AI-drafted, approved):
				frames: [
					{ dev: 'शनिबार बिदा हो', en: 'Saturday is the day off' },
					{ dev: 'शनिबार म आराम गर्छु', en: 'On Saturday I rest' },
				],
			},
			{
				id: 'kati-bajyo-what-time-is-it',
				dev: 'कति बज्यो?',
				en: 'What time is it?',
				usage: 'The standard way to ask the time. "Bajyo" literally means "(the hour) struck." You will need your numbers for the answer!',
			},
			{
				id: 'bajyo-it-s-o-clock',
				dev: '___ बज्यो',
				en: "It's ___ o'clock",
				usage: 'The answer to "kati bajyo?" — insert a number: "paanch bajyo" (it\'s five o\'clock). For half past, add "saadhe": "saadhe paanch" (half past five).',
			},
		],
	},
	{
		id: 'navigation',
		title: 'Navigation',
		kind: 'phrases',
		goal: 'Ask where things are',
		items: [
			{
				id: 'kaha-where',
				dev: 'कहाँ?',
				en: 'Where?',
				usage: "A basic question word asking about location. It's essential for getting directions, finding places, or asking where things or people are. You can combine it with other words to form more specific questions.",
			},
			{
				id: 'yaha-here',
				dev: 'यहाँ',
				en: 'Here',
				usage: "Indicates this location or place. Use it when pointing out where something is or where you are. It's often accompanied by a gesture to indicate the specific spot being referenced.",
			},
			{
				id: 'tyaha-there',
				dev: 'त्यहाँ',
				en: 'There',
				usage: "Indicates that location or place, somewhere away from the speaker. It's used for pointing to locations that aren't immediately near. You can use it when giving or receiving directions.",
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'त्यहाँ के छ?', en: "What's over there?" },
					{ dev: 'त्यहाँ नजानुस्', en: "Don't go there" },
				],
			},
			{
				id: 'kata-janey-where-to-go',
				dev: 'कता जाने?',
				en: 'Where to go?',
				usage: 'Taxi drivers commonly ask this when you get in. It\'s a shortened, casual form asking where you want to go. You can respond with your destination or say "tira" (that way) while pointing.',
			},
			{
				id: 'roknus-please-stop',
				dev: 'रोक्नुस्',
				en: 'Please stop',
				usage: 'Useful in taxis or buses when you want the vehicle to stop. "Rok" means to stop, and "nus" makes it polite. You can also say "yaha roknus" (stop here) to indicate you want to get off at this location.',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'गाडी रोक्नुस्', en: 'Stop the car' },
					{ dev: 'एक छिन रोक्नुस्', en: 'Stop for a moment' },
				],
			},
			{
				id: 'bato-dekhaunus-please-show-me-the-way',
				dev: 'बाटो देखाउनुस्',
				en: 'Please show me the way',
				usage: 'A polite request for directions. "Bato" means road or way, and "dekhaunus" means "please show." People in Nepal are generally helpful with directions and will often go out of their way to help.',
			},
			{
				id: 'toilet-kaha-cha-where-is-the-toilet',
				dev: 'टोइलेट कहाँ छ?',
				en: 'Where is the toilet?',
				usage: 'A practical necessity. In Nepal, you might also hear "charpi" as another word for toilet. This question will be understood everywhere and people will happily point you in the right direction.',
			},
			{
				id: 'baayaa-left',
				dev: 'बायाँ',
				en: 'Left',
				usage: 'A direction for giving or following the way. "Baayaa janus" (go left), "baayaa tira" (to the left). It pairs with "daayaa" (right) and "sidhaa" (straight).',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'बायाँ हात', en: 'Left hand' },
					{ dev: 'बायाँतिर पसल छ', en: 'The shop is on the left' },
				],
			},
			{
				id: 'daayaa-right',
				dev: 'दायाँ',
				en: 'Right',
				usage: 'The opposite of "baayaa." "Daayaa janus" (go right). Handy when steering a taxi or finding a shop down a side street.',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'दायाँ आँखा', en: 'Right eye' },
					{ dev: 'दायाँतिर बैंक छ', en: 'The bank is on the right' },
				],
			},
			{
				id: 'sidhaa-janus-go-straight',
				dev: 'सीधा जानुस्',
				en: 'Go straight',
				usage: '"Sidhaa" is straight ahead; "janus" is the polite "please go." Drop "janus" and just say "sidhaa" to mean "straight on."',
			},
			{
				id: 'najik-near',
				dev: 'नजिक',
				en: 'Near / close',
				enEither: true,
				usage: 'Says something is close by: "najikai cha" (it\'s nearby). The opposite is "taadhaa" (far).',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'नजिक आउनुस्', en: 'Come closer' },
					{ dev: 'मेरो घर नजिक छ', en: 'My house is close' },
				],
			},
			{
				id: 'taadhaa-far',
				dev: 'टाढा',
				en: 'Far',
				usage: 'Says something is a long way off: "dherai taadhaa cha" (it\'s very far). Useful for judging whether to walk or flag a taxi.',
			},
			{
				id: 'tira-toward-that-way',
				dev: 'तिर',
				en: 'Toward / that way',
				enEither: true,
				usage: 'Points a direction without naming the exact spot: "ghar tira" (toward home), "tyaha tira" (that way). Nepalis often just point and say "tira."',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'पहाडतिर हेर्नुस्', en: 'Look toward the mountains' },
					{ dev: 'म घरतिर जान्छु', en: "I'm heading home" },
				],
			},
		],
	},
	{
		id: 'places-getting-around',
		title: 'Places & Getting Around',
		kind: 'vocab',
		goal: 'Name key places and ways to get around',
		items: [
			{
				id: 'hospital-hospital',
				dev: 'हस्पिटल',
				emoji: '🏥',
				en: 'Hospital',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'म हस्पिटल जान्छु', en: 'I go to the hospital' },
					{ dev: 'हस्पिटल नजिक छ', en: 'The hospital is near' },
				],
			},
			{
				id: 'skul-school',
				dev: 'स्कुल',
				emoji: '🏫',
				en: 'School',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'म स्कुल जान्छु', en: 'I go to school' },
					{ dev: 'स्कुल ठूलो छ', en: 'The school is big' },
				],
			},
			{
				id: 'baink-bank',
				dev: 'बैंक',
				emoji: '🏦',
				en: 'Bank',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'बैंक कहाँ छ?', en: 'Where is the bank?' },
					{ dev: 'बैंक दस बजे खुल्छ', en: 'The bank opens at ten' },
				],
			},
			{
				id: 'aphis-office',
				dev: 'अफिस',
				emoji: '🏢',
				en: 'Office',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'म अफिस जान्छु', en: 'I go to the office' },
					{ dev: 'अफिस पाँच बजे बन्द हुन्छ', en: 'The office closes at five' },
				],
			},
			{
				id: 'pasal-shop-store',
				dev: 'पसल',
				emoji: '🏪',
				en: 'Shop',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'म पसल जान्छु', en: 'I go to the shop' },
					{ dev: 'पसल बन्द छ', en: 'The shop is closed' },
				],
			},
			{
				id: 'bajaar-market',
				dev: 'बजार',
				emoji: '🛒',
				en: 'Market',
			},
			{
				id: 'hotel-restaurant-eatery',
				dev: 'होटल',
				emoji: '🍽️',
				en: 'Hotel',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'यो होटल राम्रो छ', en: 'This hotel is good' },
					{ dev: 'म होटलमा खाना खान्छु', en: 'I eat at the hotel' },
				],
			},
			{
				id: 'bas-bus',
				dev: 'बस',
				emoji: '🚌',
				en: 'Bus',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'बस आयो', en: 'The bus came' },
					{ dev: 'म बसमा जान्छु', en: 'I go by bus' },
				],
			},
			{
				id: 'tyaksi-taxi',
				dev: 'ट्याक्सी',
				emoji: '🚕',
				en: 'Taxi',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'ट्याक्सी बोलाउनुस्', en: 'Call a taxi' },
					{ dev: 'ट्याक्सी महँगो हुन्छ', en: 'Taxis are expensive' },
				],
			},
			{
				id: 'buspark-bus-stop',
				dev: 'बसपार्क',
				emoji: '🚏',
				en: 'Bus stop',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'बसपार्क नजिक छ', en: 'The bus stop is near' },
					{ dev: 'बसपार्कमा धेरै मान्छे छन्', en: 'The bus stop is crowded' },
				],
			},
			{
				id: 'eyarport-airport',
				dev: 'एयरपोर्ट',
				emoji: '✈️',
				en: 'Airport',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'एयरपोर्ट जानुस्', en: 'To the airport, please' },
					{ dev: 'एयरपोर्ट टाढा छ?', en: 'Is the airport far?' },
				],
			},
		],
	},
	{
		id: 'place-position',
		title: 'Place & Position',
		kind: 'phrases',
		goal: 'Say where things are',
		items: [
			{
				id: 'gharma-at-home-in',
				dev: 'घरमा',
				en: 'At home / In the house',
				usage: 'The postposition "-maa" means "in," "at," or "on," and attaches to the end of a word: "ghar" + "maa" = "gharma" (at home). "Bajaarma" (at the market), "Nepalma" (in Nepal). It is the most common location word in Nepali.',
			},
			{
				id: 'mathi-on-above',
				dev: 'माथि',
				en: 'On / Above / Up',
				usage: '"Mathi" means on top of or above. It follows the noun: "tebul mathi" (on the table), "ghar mathi" (above the house). Alone, "mathi" means "up" or "upstairs."',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'किताब टेबलमाथि छ', en: 'The book is on the table' },
					{ dev: 'माथि जानुस्', en: 'Go up' },
				],
			},
			{
				id: 'muni-under-below',
				dev: 'मुनि',
				en: 'Under / Below',
				usage: 'The opposite of "mathi." "Tebul muni" (under the table), "rukh muni" (under the tree). Alone, "muni" means "below" or "underneath."',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'बिरालो टेबलमुनि छ', en: 'The cat is under the table' },
					{ dev: 'कुर्सीमुनि हेर्नुस्', en: 'Look under the chair' },
				],
			},
			{
				id: 'agadi-in-front-ahead',
				dev: 'अगाडि',
				en: 'In front / Ahead',
				usage: '"Agaadi" means in front of or ahead. "Ghar agaadi" (in front of the house). It also means "forward" or "earlier": "agaadi janus" (go ahead).',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'घर अगाडि', en: 'In front of the house' },
					{ dev: 'अगाडि जानुस्', en: 'Go ahead' },
				],
			},
			{
				id: 'pachadi-behind-back',
				dev: 'पछाडि',
				en: 'Behind / At the back',
				usage: 'The opposite of "agaadi." "Ghar pachaadi" (behind the house). It also means "afterward" or "at the back": "pachaadi basnus" (sit at the back).',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'घर पछाडि', en: 'Behind the house' },
					{ dev: 'पछाडि हेर्नुस्', en: 'Look behind' },
				],
			},
			{
				id: 'bhitra-inside-within',
				dev: 'भित्र',
				en: 'Inside / Within',
				usage: '"Bhitra" means inside. "Ghar bhitra" (inside the house). You met it in "bhitra aaunus" (please come in).',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'भित्र आउनुस्', en: 'Come inside' },
					{ dev: 'कोठा भित्र', en: 'Inside the room' },
				],
			},
			{
				id: 'bahira-outside',
				dev: 'बाहिर',
				en: 'Outside',
				usage: 'The opposite of "bhitra." "Ghar bahira" (outside the house), "bahira janus" (go outside). You may hear it for "out": "u bahira cha" (he is out).',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'बाहिर जाऔं', en: 'Let us go outside' },
					{ dev: 'ऊ बाहिर छ', en: 'He is outside' },
				],
			},
			{
				id: 'bichma-between-middle',
				dev: 'बीचमा',
				en: 'Between / In the middle',
				usage: '"Bich" is the middle, and with "-maa" it becomes "bichma" (in the middle / between). "Dui jana bichma" (between two people), "baatoko bichma" (in the middle of the road).',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'बीचमा बस्नुस्', en: 'Sit in the middle' },
					{ dev: 'बाटोको बीचमा नबस्नुस्', en: "Don't sit in the middle of the road" },
				],
			},
			{
				id: 'ma-sanga-with-me',
				dev: 'म सँग',
				en: 'With me',
				usage: 'The postposition "sanga" means "with." "Ma sanga" (with me), "saathi sanga" (with a friend). It also shows what you carry: "ma sanga paisa cha" (I have money on me).',
			},
			{
				id: 'samma-until-up-to',
				dev: 'सम्म',
				en: 'Until / Up to',
				usage: '"Samma" means up to, until, or as far as — for distance and time alike. "Ghar samma" (up to the house), "ahile samma" (until now). "Kati samma?" (how far / until when?).',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'बसपार्कसम्म जानुस्', en: 'Go as far as the bus stop' },
					{ dev: 'पाँच बजेसम्म काम गर्छु', en: 'I work until five' },
				],
			},
		],
	},
	{
		id: 'descriptors',
		title: 'Descriptors',
		kind: 'phrases',
		goal: 'Describe what things are like',
		items: [
			{
				id: 'ramro-good-nice-beautiful',
				dev: 'राम्रो',
				en: 'Good / Nice / Beautiful',
				usage: 'A versatile adjective describing something positive. It can describe food, weather, people, experiences, or objects. "Ramro cha" means "it\'s good" and is a common way to express satisfaction.',
			},
			{
				id: 'naramro-bad-not-good',
				dev: 'नराम्रो',
				en: 'Bad / Not good',
				usage: 'The opposite of ramro, this word describes something negative or unpleasant. "Na-" is a common prefix in Nepali that negates words. It can describe poor quality, bad behavior, or unfortunate situations.',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'मौसम नराम्रो छ', en: 'The weather is bad' },
					{ dev: 'यो फिल्म नराम्रो छ', en: 'This movie is bad' },
				],
			},
			{
				id: 'thulo-big-large',
				dev: 'ठूलो',
				en: 'Big / Large',
				usage: 'Describes size or importance. It can refer to physical size or to someone\'s age or status. "Thulo manchhe" means "big person" and can refer to someone important or an adult.',
			},
			{
				id: 'sano-small-little',
				dev: 'सानो',
				en: 'Small / Little',
				usage: 'The opposite of thulo, indicating something small in size. "Sano bacha" means small child. It can also indicate something minor or insignificant in context.',
			},
			{
				id: 'dherai-many-much-very',
				dev: 'धेरै',
				en: 'Many / Much / Very',
				usage: 'Indicates a large quantity or high degree. "Dherai dhanyabad" means "thank you very much." It\'s also used to say there\'s a lot of something. "Dherai manchhe" means many people.',
			},
			{
				id: 'ali-ali-a-little-bit',
				dev: 'अलि अलि',
				en: 'A little bit',
				usage: 'Indicates a small amount or degree. When learning Nepali, you might say "ma ali ali Nepali bolchhu" (I speak a little Nepali). The repetition emphasizes the smallness of the quantity.',
			},
			{
				id: 'naya-new',
				dev: 'नयाँ',
				en: 'New',
				usage: 'Describes something new: "naya lugaa" (new clothes), "naya ghar" (new house). The opposite is "purano" (old).',
			},
			{
				id: 'purano-old',
				dev: 'पुरानो',
				en: 'Old (of things)',
				usage: 'For old objects, not people: "purano kitaab" (an old book). For an older person you\'d use age words instead. The opposite is "naya" (new).',
			},
			{
				id: 'tato-hot-to-touch',
				dev: 'तातो',
				en: 'Hot (to the touch)',
				usage: 'For hot food or objects: "tato chiya" (hot tea), "tato paani" (hot water). Hot weather is "garmi"; this is the hot you feel by touching.',
			},
		],
	},
	{
		id: 'colors',
		title: 'Colors',
		kind: 'vocab',
		goal: 'Name colors',
		items: [
			{
				id: 'rang-color',
				dev: 'रङ',
				emoji: '🎨',
				en: 'Color',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'तपाईंलाई कुन रङ मन पर्छ?', en: 'Which color do you like?' },
					{ dev: 'मलाई निलो रङ मन पर्छ', en: 'I like the color blue' },
				],
			},
			{
				id: 'rato-red',
				dev: 'रातो',
				emoji: '🔴',
				en: 'Red',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'स्याउ रातो छ', en: 'The apple is red' },
					{ dev: 'रातो टोपी', en: 'A red hat' },
				],
			},
			{
				id: 'nilo-blue',
				dev: 'निलो',
				emoji: '🔵',
				en: 'Blue',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'मेरो झोला निलो छ', en: 'My bag is blue' },
					{ dev: 'यो निलो हो कि कालो?', en: 'Is this blue or black?' },
				],
			},
			{
				id: 'hariyo-green',
				dev: 'हरियो',
				emoji: '🟢',
				en: 'Green',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'हरियो घाँस', en: 'Green grass' },
					{ dev: 'नेपालको पहाड हरियो छ', en: "Nepal's hills are green" },
				],
			},
			{
				id: 'pahelo-yellow',
				dev: 'पहेँलो',
				emoji: '🟡',
				en: 'Yellow',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'पहेँलो फूल', en: 'A yellow flower' },
					{ dev: 'मेरो कमिज पहेँलो छ', en: 'My shirt is yellow' },
				],
			},
			{
				id: 'seto-white',
				dev: 'सेतो',
				emoji: '⚪',
				en: 'White',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'सेतो हिमाल', en: 'White mountains' },
					{ dev: 'मेरो कपाल सेतो भयो', en: 'My hair has gone white' },
				],
			},
			{
				id: 'kalo-black',
				dev: 'कालो',
				emoji: '⚫',
				en: 'Black',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'कालो कफी दिनुस्', en: 'Black coffee, please' },
					{ dev: 'कालो कुकुर', en: 'A black dog' },
				],
			},
			{
				id: 'khairo-brown',
				dev: 'खैरो',
				emoji: '🟤',
				en: 'Brown',
				// T29 batch 13 (grandparents, time-of-day & colors; AI-drafted, approved):
				frames: [
					{ dev: 'खैरो गाई', en: 'A brown cow' },
					{ dev: 'यो जुत्ता खैरो छ', en: 'These shoes are brown' },
				],
			},
			{
				id: 'suntala-rang-orange-color',
				dev: 'सुन्तला रङ',
				emoji: '🟠',
				en: 'Orange (color)',
			},
			{
				id: 'gulabi-pink',
				dev: 'गुलाबी',
				emoji: '🌸',
				en: 'Pink',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'तपाईंलाई गुलाबी मन पर्छ?', en: 'Do you like pink?' },
					{ dev: 'गुलाबी कमिज', en: 'A pink shirt' },
				],
			},
			{
				id: 'pyaji-purple',
				dev: 'प्याजी',
				emoji: '🟣',
				en: 'Purple',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'प्याजी रङ राम्रो छ', en: 'Purple is a nice color' },
					{ dev: 'प्याजी फूल', en: 'A purple flower' },
				],
			},
		],
	},
	{
		id: 'comparing-things',
		title: 'Comparing Things',
		kind: 'phrases',
		goal: 'Compare things — bigger, smaller, the best',
		items: [
			{
				id: 'bhanda-than',
				dev: 'भन्दा',
				en: 'Than (for comparing)',
				usage: 'The comparison word "bhanda" means "than." The thing you compare against comes first: "yo tyo bhanda" = "this, than that." Then add the quality. It is the key to every comparison in Nepali.',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'चिया कफी भन्दा सस्तो छ', en: 'Tea is cheaper than coffee' },
					{ dev: 'हिजो भन्दा आज गर्मी छ', en: "It's hotter today than yesterday" },
				],
			},
			{
				id: 'yo-tyo-bhanda-thulo-bigger',
				dev: 'यो त्यो भन्दा ठूलो छ',
				en: 'This is bigger than that',
				usage: 'The full comparison: "yo" (this) + "tyo bhanda" (than that) + "thulo cha" (is big). Nepali has no "-er" ending; "bhanda" plus the plain adjective does the work.',
			},
			{
				id: 'yo-tyo-bhanda-sano-smaller',
				dev: 'यो त्यो भन्दा सानो छ',
				en: 'This is smaller than that',
				usage: 'Swap the adjective: "sano" (small) gives "smaller than." The "X bhanda Y" frame stays the same — only the quality word changes.',
			},
			{
				id: 'yo-tyo-bhanda-ramro-better',
				dev: 'यो त्यो भन्दा राम्रो छ',
				en: 'This is better than that',
				usage: '"Ramro" (good) becomes "better than" in the comparison frame. No separate word for "better" is needed.',
			},
			{
				id: 'yo-tyo-bhanda-mahango-more-expensive',
				dev: 'यो त्यो भन्दा महँगो छ',
				en: 'This is more expensive than that',
				usage: 'Useful while shopping: "mahango" (expensive) gives "more expensive than." Pair it with "sasto" (cheap): "tyo bhanda sasto cha?" (is it cheaper than that?).',
			},
			{
				id: 'sabai-bhanda-thulo-the-biggest',
				dev: 'सबै भन्दा ठूलो',
				en: 'The biggest',
				usage: 'For the superlative ("the most"), compare against "sabai" (all): "sabai bhanda thulo" = "bigger than all" = "the biggest." "Sabai bhanda sano" (the smallest).',
			},
			{
				id: 'sabai-bhanda-ramro-the-best',
				dev: 'सबै भन्दा राम्रो',
				en: 'The best',
				usage: 'The same "sabai bhanda" (than all) frame with "ramro" (good) gives "the best." This is how you crown a favorite: "yo sabai bhanda ramro cha" (this is the best).',
			},
			{
				id: 'badhi-more',
				dev: 'बढी',
				en: 'More',
				usage: '"Badhi" means more (in amount). "Ali badhi" (a little more), "dherai badhi" (much more), "badhi paisa" (more money). Its opposite is "kam" (less).',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'अलि बढी दिनुस्', en: 'A little more, please' },
					{ dev: 'पानी बढी भयो', en: "There's too much water" },
				],
			},
			{
				id: 'kam-less-few',
				dev: 'कम',
				en: 'Less / Few',
				usage: 'The opposite of "badhi." "Kam paisa" (less money), "ali kam" (a little less) — handy when something is too much.',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'नुन कम हाल्नुस्', en: 'Go easy on the salt' },
					{ dev: 'अलि कम गर्नुस्', en: 'Bring the price down a bit' },
				],
			},
			{
				id: 'barabar-equal-same',
				dev: 'बराबर',
				en: 'Equal / The same',
				usage: '"Barabar" means equal or the same. "Yo ra tyo barabar cha" (this and that are equal). Also used for "level" and, in markets, a fair matching price.',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'दुवै बराबर छ', en: "They're both the same" },
					{ dev: 'यो र त्यो बराबर हो', en: 'This and that are equal' },
				],
			},
		],
	},
	// --- T11 batch 2: everyday adjectives (frames), interleaved into the descriptor cluster. Titles/goals AI-drafted.
	{
		id: 'adj-size-physical',
		title: 'Size & Feel',
		kind: 'phrases',
		goal: 'Describe things by size, weight, and feel — long, heavy, light, strong, cold, empty, open',
		items: [
			{
				id: 'adj-lamo-long',
				dev: 'दिन लामो छ',
				en: 'The day is long',
				usage: "लामो = long (or 'tall' for a person).",
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'मेरो कपाल लामो छ', en: 'My hair is long' },
					{ dev: 'लामो बाटो', en: 'A long road' },
				],
			},
			{
				id: 'adj-bhari-heavy',
				dev: 'झोला भारी छ',
				en: 'The bag is heavy',
				usage: 'भारी = heavy. झोला = bag.',
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'भारी किताब', en: 'A heavy book' },
					{ dev: 'यो धेरै भारी छ', en: 'This is very heavy' },
				],
			},
			{
				id: 'adj-halka-light',
				dev: 'यो हल्का छ',
				en: 'This is light',
				usage: "हल्का = light (in weight); also 'mild', 'slight'.",
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'हल्का झोला', en: 'A light bag' },
					{ dev: 'यो धेरै हल्का छ', en: 'This is very light' },
				],
			},
			{
				id: 'adj-baliyo-strong',
				dev: 'ऊ बलियो छ',
				en: 'He is strong',
				usage: 'बलियो = strong / sturdy.',
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'बलियो मान्छे', en: 'A strong person' },
					{ dev: 'चिया बलियो छ', en: 'The tea is strong' },
				],
			},
			{
				id: 'adj-kamjor-weak',
				dev: 'म कमजोर छु',
				en: 'I am weak',
				usage: 'कमजोर = weak / frail.',
			},
			{
				id: 'adj-chiso-cold',
				dev: 'पानी चिसो छ',
				en: 'The water is cold',
				usage: "चिसो = cold (to the touch) — the pair of तातो 'hot'.",
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'चिसो पानी', en: 'Cold water' },
					{ dev: 'आज चिसो छ', en: 'It is cold today' },
				],
			},
			{
				id: 'adj-khali-empty',
				dev: 'कोठा खाली छ',
				en: 'The room is empty',
				usage: "खाली = empty / vacant (also 'free', as in free time).",
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'बोतल खाली छ', en: 'The bottle is empty' },
					{ dev: 'खाली ठाउँ', en: 'An empty space' },
				],
			},
			{
				id: 'adj-pura-complete',
				dev: 'काम पूरा भयो',
				en: 'The work is done',
				usage: 'पूरा = complete / full / finished.',
			},
			{
				id: 'adj-khula-open',
				dev: 'ढोका खुला छ',
				en: 'The door is open',
				usage: "खुला = open; opposite of बन्द 'closed'.",
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'पसल खुला छ', en: 'The shop is open' },
					{ dev: 'बाटो खुला छ', en: 'The road is open' },
				],
			},
		],
	},
	{
		id: 'adj-good-bad-right',
		title: 'Good, Bad & Right',
		kind: 'phrases',
		goal: 'Give a verdict — easy or hard, right or wrong, good, bad, or beautiful',
		items: [
			{
				id: 'adj-sajilo-easy',
				dev: 'यो सजिलो छ',
				en: 'This is easy',
				usage: 'सजिलो = easy / simple / convenient.',
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'नेपाली सजिलो छ', en: 'Nepali is easy' },
					{ dev: 'सजिलो काम', en: 'An easy job' },
				],
			},
			{
				id: 'adj-gahro-hard',
				dev: 'यो गाह्रो छ',
				en: 'This is hard',
				usage: 'गाह्रो = difficult / hard (of a task or situation).',
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'यो प्रश्न गाह्रो छ', en: 'This question is hard' },
					{ dev: 'गाह्रो काम', en: 'Hard work' },
				],
			},
			{
				id: 'adj-sahi-correct',
				dev: 'तपाईं सही हुनुहुन्छ',
				en: 'You are right',
				usage: 'सही = correct / right / accurate.',
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'यो सही हो', en: 'This is correct' },
					{ dev: 'सही समय', en: 'The right time' },
				],
			},
			{
				id: 'adj-galat-wrong',
				dev: 'यो गलत हो',
				en: 'This is wrong',
				usage: 'गलत = wrong / incorrect / mistaken.',
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'गलत बाटो', en: 'The wrong road' },
					{ dev: 'यो गलत हो', en: 'This is wrong' },
				],
			},
			{
				id: 'adj-kharab-bad',
				dev: 'मौसम खराब छ',
				en: 'The weather is bad',
				usage: 'खराब = bad / spoiled (blunter than नराम्रो). मौसम = weather.',
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'खराब मान्छे', en: 'A bad person' },
					{ dev: 'आज मौसम खराब छ', en: 'The weather is bad today' },
				],
			},
			{
				id: 'adj-saphal-successful',
				dev: 'ऊ सफल भयो',
				en: 'He succeeded',
				usage: "सफल = successful. सफल भयो = 'became successful / succeeded'.",
			},
			{
				id: 'adj-sundar-beautiful',
				dev: 'कति सुन्दर छ!',
				en: 'How beautiful!',
				usage: 'सुन्दर = beautiful / lovely (a step up from राम्रो).',
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'सुन्दर ठाउँ', en: 'A beautiful place' },
					{ dev: 'नेपाल सुन्दर छ', en: 'Nepal is beautiful' },
				],
			},
			{
				id: 'adj-bahadur-brave',
				dev: 'ऊ बहादुर छ',
				en: 'He is brave',
				usage: 'बहादुर = brave / courageous (also a common Nepali name).',
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'बहादुर मान्छे', en: 'A brave person' },
					{ dev: 'ऊ धेरै बहादुर छ', en: 'He is very brave' },
				],
			},
			{
				id: 'adj-kada-strict',
				dev: 'नियम कडा छ',
				en: 'The rule is strict',
				usage: 'कडा = strict / tough / hard. नियम = rule.',
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'कडा नियम', en: 'A strict rule' },
					{ dev: 'मेरो बुबा कडा हुनुहुन्छ', en: 'My father is strict' },
				],
			},
		],
	},
	{
		id: 'adj-order-sequence',
		title: 'Order & Sequence',
		kind: 'phrases',
		goal: 'Put things in order — first through fifth, previous, recent, main, extra',
		items: [
			{
				id: 'adj-pahilo-first',
				dev: 'पहिलो पटक',
				en: 'The first time',
				usage: 'पहिलो = first. पटक = time / occurrence.',
			},
			{
				id: 'adj-dosro-second',
				dev: 'दोस्रो दिन',
				en: 'The second day',
				usage: 'दोस्रो = second.',
			},
			{
				id: 'adj-tesro-third',
				dev: 'तेस्रो पटक',
				en: 'The third time',
				usage: 'तेस्रो = third.',
			},
			{
				id: 'adj-chautho-fourth',
				dev: 'चौथो तला',
				en: 'The fourth floor',
				usage: 'चौथो = fourth. तला = floor / storey.',
			},
			{
				id: 'adj-panchaun-fifth',
				dev: 'पाँचौं दिन',
				en: 'The fifth day',
				usage: 'पाँचौं = fifth.',
			},
			{
				id: 'adj-aghillo-previous',
				dev: 'अघिल्लो हप्ता',
				en: 'Last week',
				usage: 'अघिल्लो = previous / former. हप्ता = week.',
			},
			{
				id: 'adj-pachhillo-recent',
				dev: 'पछिल्लो पटक',
				en: 'The last time',
				usage: 'पछिल्लो = last / latest / most recent.',
			},
			{
				id: 'adj-mukhya-main',
				dev: 'मुख्य बाटो',
				en: 'The main road',
				usage: 'मुख्य = main / chief / primary. बाटो = road / way.',
			},
			{
				id: 'adj-thap-additional',
				dev: 'थप जानकारी',
				en: 'More information',
				usage: 'थप = additional / more / extra. जानकारी = information.',
			},
		],
	},
	{
		id: 'adj-same-different',
		title: 'Same or Different',
		kind: 'phrases',
		goal: 'Compare and count — same, equal, different, both, many, remaining',
		items: [
			{
				id: 'adj-duwai-both',
				dev: 'दुवै जना',
				en: 'Both of them',
				usage: 'दुवै = both. जना counts people (दुई जना = two people).',
			},
			{
				id: 'adj-eutai-same',
				dev: 'एउटै कुरा',
				en: 'The same thing',
				usage: 'एउटै = the same / one and the same. कुरा = thing / matter.',
			},
			{
				id: 'adj-saman-equal',
				dev: 'हामी समान छौं',
				en: 'We are equal',
				usage: 'समान = equal / similar.',
			},
			{
				id: 'adj-farak-different',
				dev: 'यो फरक छ',
				en: 'This is different',
				usage: "फरक = different (also 'a difference').",
			},
			{
				id: 'adj-aru-other',
				dev: 'अरु केही?',
				en: 'Anything else?',
				usage: 'अरु = other / more / else. केही = something / anything.',
			},
			{
				id: 'adj-thuprai-many',
				dev: 'थुप्रै मान्छे',
				en: 'A lot of people',
				usage: 'थुप्रै = many / a lot / plenty. मान्छे = person / people.',
			},
			{
				id: 'adj-baki-remaining',
				dev: 'बाँकी पैसा',
				en: 'The remaining money',
				usage: 'बाँकी = remaining / the rest / leftover. पैसा = money.',
			},
			{
				id: 'adj-alag-separate',
				dev: 'यो अलग हो',
				en: 'This is separate',
				usage: 'अलग = separate / apart / different.',
			},
			{
				id: 'adj-samanya-ordinary',
				dev: 'यो सामान्य कुरा हो',
				en: 'This is a normal thing',
				usage: 'सामान्य = ordinary / normal / general.',
			},
		],
	},
	{
		id: 'adj-states-conditions',
		title: 'States & Conditions',
		kind: 'phrases',
		goal: 'Say how things stand — busy, necessary, possible, safe, on, sure, healed',
		items: [
			{
				id: 'adj-byasta-busy',
				dev: 'म व्यस्त छु',
				en: 'I am busy',
				usage: 'व्यस्त = busy / occupied.',
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'आज म धेरै व्यस्त छु', en: 'I am very busy today' },
					{ dev: 'व्यस्त बाटो', en: 'A busy road' },
				],
			},
			{
				id: 'adj-jaruri-necessary',
				dev: 'यो जरुरी छ',
				en: 'This is important',
				usage: 'जरुरी = necessary / important / urgent.',
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'जरुरी काम', en: 'Important work' },
					{ dev: 'यो धेरै जरुरी छ', en: 'This is very important' },
				],
			},
			{
				id: 'adj-sambhav-possible',
				dev: 'के यो सम्भव छ?',
				en: 'Is this possible?',
				usage: 'सम्भव = possible / feasible. के … छ? frames a yes/no question.',
			},
			{
				id: 'adj-pakka-sure',
				dev: 'मलाई पक्का छैन',
				en: "I'm not sure",
				usage: "पक्का = sure / certain (also 'solid', 'firm').",
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'के तपाईं पक्का हुनुहुन्छ?', en: 'Are you sure?' },
					{ dev: 'मलाई पक्का थाहा छैन', en: 'I am not sure' },
				],
			},
			{
				id: 'adj-niko-healed',
				dev: 'अब निको भयो',
				en: "Now it's better",
				usage: 'निको = healed / cured / recovered (of health, or a problem). अब = now.',
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'म अब निको छु', en: 'I am better now' },
					{ dev: 'छिटो निको हुनुहोस्', en: 'Get well soon' },
				],
			},
			{
				id: 'adj-chalu-on',
				dev: 'बत्ती चालु छ',
				en: 'The light is on',
				usage: 'चालु = on / running / working. बत्ती = light / lamp.',
			},
			{
				id: 'adj-nishchit-certain',
				dev: 'केही निश्चित छैन',
				en: 'Nothing is certain',
				usage: 'निश्चित = certain / definite / fixed.',
			},
			{
				id: 'adj-surakshit-safe',
				dev: 'यहाँ सुरक्षित छ',
				en: "It's safe here",
				usage: 'सुरक्षित = safe / secure. यहाँ = here.',
				// T29 batch 3 (descriptive adjectives; AI-drafted, approved):
				frames: [
					{ dev: 'सुरक्षित ठाउँ', en: 'A safe place' },
					{ dev: 'यो बाटो सुरक्षित छ', en: 'This road is safe' },
				],
			},
			{
				id: 'adj-dainik-daily',
				dev: 'दैनिक जीवन',
				en: 'Daily life',
				usage: 'दैनिक = daily / everyday. जीवन = life.',
			},
		],
	},
	{
		id: 'weather',
		title: 'Weather',
		kind: 'phrases',
		goal: 'Talk about the weather',
		items: [
			{
				id: 'kasto-chha-hawa-how-s-the-weather',
				dev: 'कस्तो छ हावा?',
				en: "How's the weather?",
				usage: 'Asks about weather conditions. "Hawa" means air or weather. You can respond with descriptions like "ramro cha" (it\'s good), "garmi cha" (it\'s hot), or "pani pariracha" (it\'s raining).',
			},
			{
				id: 'pani-pariracha-it-is-raining',
				dev: 'पानी परिरहेको छ',
				en: 'It is raining',
				usage: 'A useful phrase for discussing weather. "Pani" means water, and "pariracha" is the present continuous form of "to fall." Monsoon season makes this a frequently used phrase in Nepal.',
			},
			{
				id: 'garmi-cha-it-s-hot',
				dev: 'गर्मी छ',
				en: "It's hot",
				usage: 'Describes hot weather or temperature. "Garmi" means heat. In Nepal\'s hot season (before monsoon), this phrase is commonly used. You might also hear "dherai garmi cha" (it\'s very hot).',
			},
			{
				id: 'jado-cha-it-s-cold',
				dev: 'जाडो छ',
				en: "It's cold",
				usage: 'Used to describe cold weather, especially during winter months in Nepal. "Jado" means cold or winter. In mountainous areas, you\'ll hear this frequently during the colder seasons.',
			},
			{
				id: 'ghaam-laagyo-it-s-sunny',
				dev: 'घाम लाग्यो',
				en: "It's sunny / the sun is out",
				usage: '"Ghaam" is sunshine and "laagyo" means it has come out — a welcome thing to say after the rain. The opposite is "badal laagyo" (it\'s cloudy).',
			},
			{
				id: 'badal-laagyo-it-s-cloudy',
				dev: 'बादल लाग्यो',
				en: "It's cloudy",
				usage: '"Badal" means cloud. Often the warning before "pani parcha" (it will rain). Same "laagyo" pattern as "ghaam laagyo."',
			},
			{
				id: 'hawa-chalyo-it-s-windy',
				dev: 'हावा चल्यो',
				en: "It's windy",
				usage: '"Hawa" is wind or air — you met it in "kasto chha hawa?" — and "chalyo" means it blew up. Common in the hills before a storm.',
			},
			{
				id: 'hussu-laagyo-it-s-foggy',
				dev: 'हुस्सु लाग्यो',
				en: "It's foggy",
				usage: '"Hussu" is fog or mist — frequent on winter mornings in the hills. Same "laagyo" pattern as "ghaam" and "badal."',
			},
			{
				id: 'hiun-pariraheko-cha-it-is-snowing',
				dev: 'हिउँ परिरहेको छ',
				en: 'It is snowing',
				usage: '"Hiun" is snow — also the name of the snow leopard companion! Same "pariraheko cha" pattern as "pani pariraheko cha" (it is raining). Heard high in the mountains.',
			},
		],
	},
	{
		id: 'body-parts',
		title: 'Body Parts',
		kind: 'vocab',
		goal: 'Name parts of the body',
		items: [
			{
				id: 'tauko-head',
				dev: 'टाउको',
				emoji: '👤',
				en: 'Head',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'मेरो टाउको दुख्छ', en: 'My head hurts' },
					{ dev: 'उसको टाउको ठूलो छ', en: 'His head is big' },
				],
			},
			{
				id: 'kapaal-hair',
				dev: 'कपाल',
				emoji: '💇',
				en: 'Hair',
			},
			{
				id: 'aankha-eye',
				dev: 'आँखा',
				emoji: '👁️',
				en: 'Eye',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'मेरो आँखा रातो छ', en: 'My eye is red' },
					{ dev: 'आँखा बन्द गर्नुस्', en: 'Close your eyes' },
				],
			},
			{
				id: 'kaan-ear',
				dev: 'कान',
				emoji: '👂',
				en: 'Ear',
			},
			{
				id: 'naak-nose',
				dev: 'नाक',
				emoji: '👃',
				en: 'Nose',
			},
			{
				id: 'mukh-mouth-face',
				dev: 'मुख',
				emoji: '👄',
				en: 'Mouth / Face',
			},
			{
				id: 'oth-lips',
				dev: 'ओठ',
				emoji: '💋',
				en: 'Lips',
			},
			{
				id: 'daat-teeth',
				dev: 'दाँत',
				emoji: '🦷',
				en: 'Teeth',
			},
			{
				id: 'ghaati-throat-neck',
				dev: 'घाँटी',
				emoji: '🗣️',
				en: 'Throat / Neck',
			},
			{
				id: 'haat-hand-arm',
				dev: 'हात',
				emoji: '✋',
				en: 'Hand / Arm',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'हात धुनुस्', en: 'Wash your hands' },
					{ dev: 'मेरो हात चिसो छ', en: 'My hand is cold' },
				],
			},
			{
				id: 'aaulaa-finger',
				dev: 'औंला',
				emoji: '☝️',
				en: 'Finger',
			},
			{
				id: 'pet-stomach-belly',
				dev: 'पेट',
				emoji: '🫄',
				en: 'Stomach / Belly',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'मलाई पेट दुख्यो', en: 'My stomach hurts' },
					{ dev: 'पेट खाली छ', en: 'The stomach is empty' },
				],
			},
			{
				id: 'dhaad-back',
				dev: 'ढाड',
				emoji: '🦴',
				en: 'Back',
			},
			{
				id: 'khutta-leg-foot',
				dev: 'खुट्टा',
				emoji: '🦵',
				en: 'Leg / Foot',
			},
		],
	},
	{
		id: 'medical',
		title: 'Medical',
		kind: 'phrases',
		goal: 'Ask for help in an emergency',
		items: [
			{
				id: 'kripaya-malaai-maddat-garnuhos-please-help-me',
				dev: 'कृपया मलाई मद्दत गर्नुहोस्',
				en: 'Please help me',
				usage: 'A polite request for assistance. "Malaai" means "to me," and "maddat garnuhos" means "please help." This is useful in emergencies or when you need someone\'s assistance with something.',
			},
			{
				id: 'sahayog-garnuhos-please-cooperate-please-help',
				dev: 'सहयोग गर्नुहोस्',
				en: 'Please cooperate / Please help',
				usage: 'Formal request for cooperation or assistance. "Sahayog" means cooperation or help. Often used in official or semi-formal contexts when requesting someone\'s support.',
			},
			{
				id: 'ma-haraye-i-am-lost',
				dev: 'म हराएँ',
				en: 'I am lost',
				usage: 'Essential phrase if you get lost. "Haraye" means lost. You can follow this with "bato dekhaunus" (please show me the way) to ask for directions back to your destination.',
			},
			{
				id: 'hospital-kaha-cha-where-is-the-hospital',
				dev: 'हस्पिटल कहाँ छ?',
				en: 'Where is the hospital?',
				usage: 'Important phrase for emergencies. You can replace "hospital" with other locations like "police" (police station), "pharmacy" (ausadhi pasal), or any other place you need to find.',
			},
			{
				id: 'malai-biraami-bhayo-i-am-sick',
				dev: 'मलाई बिरामी भयो',
				en: 'I am sick',
				usage: 'Use this when you\'re not feeling well. "Malai" means "to me," and "biraami" means sick. You can specify symptoms by adding words like "tauko dukhyo" (headache) or "pet dukhyo" (stomach ache).',
			},
			{
				id: 'doctor-chaahiyo-i-need-a-doctor',
				dev: 'डाक्टर चाहियो',
				en: 'I need a doctor',
				usage: 'An urgent phrase when medical attention is needed. "Chaahiyo" means "is needed." This will help communicate that you require medical assistance quickly.',
			},
			{
				id: 'tauko-dukhyo-my-head-hurts-headache',
				dev: 'टाउको दुख्यो',
				en: 'My head hurts / Headache',
				usage: 'Describes a headache. "Tauko" means head and "dukhyo" means hurts or pained. You can use this pattern with other body parts: "pet dukhyo" (stomach ache), "khutta dukhyo" (leg hurts).',
			},
			{
				id: 'pani-chaahiyo-i-need-water',
				dev: 'पानी चाहियो',
				en: 'I need water',
				usage: 'States you need water. More direct than "paani dinus" (please give water). "Chaahiyo" indicates necessity. You can replace "paani" with other items you need urgently.',
			},
		],
	},
	{
		id: 'commands',
		title: 'Commands',
		kind: 'phrases',
		goal: 'Tell someone to slow down or come',
		items: [
			{
				id: 'bistari-slowly-carefully',
				dev: 'बिस्तारी',
				en: 'Slowly / Carefully',
				usage: 'Can mean either slowly or carefully depending on context. You\'ll hear this when someone wants you to take your time or be cautious. "Bistari jaanus" means "go slowly/carefully."',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'बिस्तारी खानुस्', en: 'Eat slowly' },
					{ dev: 'गाडी बिस्तारी चलाउनुस्', en: 'Drive slowly' },
				],
			},
			{
				id: 'chitto-quickly-fast',
				dev: 'छिटो',
				en: 'Quickly / Fast',
				usage: 'The opposite of bistari, meaning quickly or fast. "Chitto garnuhos" means "please do it quickly." Useful when you\'re in a hurry or need something done promptly.',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'छिटो आउनुस्', en: 'Come quickly' },
					{ dev: 'छिटो गर्नुस्', en: 'Hurry up' },
				],
			},
			{
				id: 'aaija-come-here',
				dev: 'आइज',
				en: 'Come here',
				usage: 'An informal command asking someone to come. Used with friends, family, or subordinates. For a more polite version, use "aaunu hos" (please come).',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'कुकुर, आइज', en: 'Come here, dog' },
					{ dev: 'छिटो आइज', en: 'Come quick' },
				],
			},
			{
				id: 'jaau-go-let-s-go',
				dev: 'जाऊ',
				en: "Go / Let's go",
				usage: 'Can be a command "go" or a suggestion "let\'s go" depending on context and tone. Among friends, it\'s a common way to suggest leaving. More polite form is "jaanuhola" (shall we go?).',
				// T29 batch 14 (everyday communication & getting around; AI-drafted, approved):
				frames: [
					{ dev: 'घर जाऊ', en: 'Go home' },
					{ dev: 'स्कुल जाऊ', en: 'Go to school' },
				],
			},
			{
				id: 'basnus-please-sit',
				dev: 'बस्नुस्',
				en: 'Please sit',
				usage: 'Polite invitation to sit down. "Bas" means sit, and "nus" makes it respectful. Hosts commonly use this to invite guests to be seated. You can also say "yaha basnus" (sit here).',
			},
			{
				id: 'khadai-garnus-please-eat',
				dev: 'खाँदै गर्नुस्',
				en: 'Please eat',
				usage: "Polite invitation to eat, commonly used by hosts. It's considered good hospitality to encourage guests to eat. You'll hear this often at meals in Nepali homes.",
			},
			{
				id: 'piudai-garnus-please-drink',
				dev: 'पिउँदै गर्नुस्',
				en: 'Please drink',
				usage: "Polite invitation to drink, whether it's water, tea, or other beverages. Hosts use this to ensure guests are comfortable and well-served.",
			},
			{
				id: 'aaunus-please-come',
				dev: 'आउनुस्',
				en: 'Please come',
				usage: 'The polite "come," from "aaunu" (to come) — the courteous counterpart to the casual "aaija." A warm "aaunus" welcomes a guest in.',
			},
			{
				id: 'hernus-please-look',
				dev: 'हेर्नुस्',
				en: 'Please look / look here',
				usage: 'The polite "look," from "hernu" (to see, watch). "Yaha hernus" (look here) — handy for drawing attention to something.',
			},
		],
	},
	{
		id: 'bedroom-items',
		title: 'Bedroom Items',
		kind: 'vocab',
		goal: 'Name things in the bedroom',
		items: [
			{
				id: 'ochyan-bed',
				dev: 'ओछ्यान',
				emoji: '🛏️',
				en: 'Bed',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'म ओछ्यानमा सुत्छु', en: 'I sleep in my bed' },
					{ dev: 'ओछ्यान नरम छ', en: 'The bed is soft' },
				],
			},
			{
				id: 'sirani-pillow',
				dev: 'सिरानी',
				emoji: '🛋️',
				en: 'Pillow',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'सिरानी नरम छ', en: 'The pillow is soft' },
					{ dev: 'दुईवटा सिरानी दिनुस्', en: 'Two pillows, please' },
				],
			},
			{
				id: 'kambal-blanket',
				dev: 'कम्बल',
				emoji: '🧺',
				en: 'Blanket',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'जाडोमा कम्बल चाहिन्छ', en: 'You need a blanket in winter' },
					{ dev: 'कम्बल न्यानो छ', en: 'The blanket is warm' },
				],
			},
			{
				id: 'gadda-mattress',
				dev: 'गद्दा',
				emoji: '🛏️',
				en: 'Mattress',
			},
			{
				id: 'almari-wardrobe-closet',
				dev: 'अलमारी',
				emoji: '🚪',
				en: 'Wardrobe / Closet',
			},
			{
				id: 'aaina-mirror',
				dev: 'ऐना',
				emoji: '🪞',
				en: 'Mirror',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'ऐनामा हेर्नुस्', en: 'Look in the mirror' },
					{ dev: 'ऐना सफा गर्नुस्', en: 'Clean the mirror' },
				],
			},
			{
				id: 'batti-light-lamp',
				dev: 'बत्ती',
				emoji: '💡',
				en: 'Light / Lamp',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'बत्ती गयो', en: 'The power went out' },
					{ dev: 'बत्ती आयो', en: 'The power is back' },
				],
			},
			{
				id: 'ghaadi-clock-watch',
				dev: 'घडी',
				emoji: '⏰',
				en: 'Clock / Watch',
			},
			{
				id: 'pankha-fan',
				dev: 'पंखा',
				emoji: '🌀',
				en: 'Fan',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'पंखा चलाउनुस्', en: 'Turn on the fan' },
					{ dev: 'पंखा बन्द गर्नुस्', en: 'Turn off the fan' },
				],
			},
		],
	},
	{
		id: 'bathroom-items',
		title: 'Bathroom Items',
		kind: 'vocab',
		goal: 'Name things in the bathroom',
		items: [
			{
				id: 'charpi-toilet',
				dev: 'चर्पी',
				emoji: '🚽',
				en: 'Toilet',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'चर्पी बाहिर छ', en: 'The toilet is outside' },
					{ dev: 'चर्पी सफा छ', en: 'The toilet is clean' },
				],
			},
			{
				id: 'nuhaaune-thaau-shower-bathing-area',
				dev: 'नुहाउने ठाउँ',
				emoji: '🚿',
				en: 'Shower / Bathing area',
			},
			{
				id: 'sabun-soap',
				dev: 'साबुन',
				emoji: '🧼',
				en: 'Soap',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'साबुनले हात धुनुस्', en: 'Wash your hands with soap' },
					{ dev: 'साबुन कहाँ छ?', en: 'Where is the soap?' },
				],
			},
			{
				id: 'taauliya-towel',
				dev: 'तौलिया',
				emoji: '🧽',
				en: 'Towel',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'तौलिया सफा छ', en: 'The towel is clean' },
					{ dev: 'मलाई तौलिया दिनुस्', en: 'Please give me a towel' },
				],
			},
			{
				id: 'brush-toothbrush',
				dev: 'ब्रश',
				emoji: '🪥',
				en: 'Toothbrush',
			},
			{
				id: 'toothpaste-toothpaste',
				dev: 'टुथपेस्ट',
				emoji: '🦷',
				en: 'Toothpaste',
			},
			{
				id: 'shampoo-shampoo',
				dev: 'स्याम्पु',
				emoji: '🧴',
				en: 'Shampoo',
			},
			{
				id: 'tissue-tissue-paper',
				dev: 'टिस्यु',
				emoji: '🧻',
				en: 'Tissue paper',
			},
			{
				id: 'kaanchi-scissors',
				dev: 'कैंची',
				emoji: '✂️',
				en: 'Scissors',
			},
		],
	},
	{
		id: 'living-room-items',
		title: 'Living Room Items',
		kind: 'vocab',
		goal: 'Name things in the living room',
		items: [
			{
				id: 'kursi-chair',
				dev: 'कुर्सी',
				emoji: '🪑',
				en: 'Chair',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'कुर्सीमा बस्नुस्', en: 'Sit on the chair' },
					{ dev: 'यो कुर्सी खाली छ?', en: 'Is this seat free?' },
				],
			},
			{
				id: 'tebul-table',
				dev: 'टेबल',
				emoji: '🪑',
				en: 'Table',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'टेबल सफा गर्नुस्', en: 'Clean the table' },
					{ dev: 'टेबलमुनि झोला छ', en: 'The bag is under the table' },
				],
			},
			{
				id: 'sofa-sofa-couch',
				dev: 'सोफा',
				emoji: '🛋️',
				en: 'Sofa / Couch',
			},
			{
				id: 'tv-television',
				dev: 'टिभी',
				emoji: '📺',
				en: 'Television',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'टिभी बन्द गर्नुस्', en: 'Turn off the TV' },
					{ dev: 'टिभीमा के छ?', en: "What's on TV?" },
				],
			},
			{
				id: 'remote-remote-control',
				dev: 'रिमोट',
				emoji: '📱',
				en: 'Remote control',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'रिमोट मलाई दिनुस्', en: 'Give me the remote' },
					{ dev: 'रिमोट चल्दैन', en: "The remote isn't working" },
				],
			},
			{
				id: 'kitab-book',
				dev: 'किताब',
				emoji: '📚',
				en: 'Book',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'यो किताब राम्रो छ', en: 'This book is good' },
					{ dev: 'किताब झोलामा छ', en: 'The book is in the bag' },
				],
			},
			{
				id: 'kaagaj-paper',
				dev: 'कागज',
				emoji: '📄',
				en: 'Paper',
			},
			{
				id: 'kalam-pen',
				dev: 'कलम',
				emoji: '🖊️',
				en: 'Pen',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'मलाई कलम दिनुस्', en: 'Please give me a pen' },
					{ dev: 'निलो कलम', en: 'A blue pen' },
				],
			},
			{
				id: 'galaicha-carpet-rug',
				dev: 'गलैंचा',
				emoji: '🧶',
				en: 'Carpet / Rug',
			},
			{
				id: 'jhyaal-window',
				dev: 'झ्याल',
				emoji: '🪟',
				en: 'Window',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'झ्यालबाट हावा आउँछ', en: 'Air comes in through the window' },
					{ dev: 'झ्याल फोहोर छ', en: 'The window is dirty' },
				],
			},
			{
				id: 'dhoka-door',
				dev: 'ढोका',
				emoji: '🚪',
				en: 'Door',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'ढोका खोल्नुस्', en: 'Open the door' },
					{ dev: 'ढोकामा को छ?', en: 'Who is at the door?' },
				],
			},
			{
				id: 'taala-lock',
				dev: 'ताला',
				emoji: '🔒',
				en: 'Lock',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'ढोकामा ताला लगाउनुस्', en: 'Put a lock on the door' },
					{ dev: 'ताला खुल्दैन', en: "The lock won't open" },
				],
			},
			{
				id: 'saacho-key',
				dev: 'साँचो',
				emoji: '🔑',
				en: 'Key',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'साँचो ढोकामा छ', en: 'The key is in the door' },
					{ dev: 'मैले साँचो बिर्सें', en: 'I forgot the key' },
				],
			},
		],
	},
	{
		id: 'laundry-clothing-items',
		title: 'Laundry & Clothing Items',
		kind: 'vocab',
		goal: 'Name clothes and laundry items',
		items: [
			{
				id: 'lugaa-clothes',
				dev: 'लुगा',
				emoji: '👕',
				en: 'Clothes',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'लुगा सफा छ', en: 'The clothes are clean' },
					{ dev: 'दशैंमा नयाँ लुगा किन्छौं', en: 'We buy new clothes at Dashain' },
				],
			},
			{
				id: 'dhune-sabun-laundry-detergent',
				dev: 'धुने साबुन',
				emoji: '🧴',
				en: 'Laundry detergent',
			},
			{
				id: 'lugaa-dhune-machine-washing-machine',
				dev: 'लुगा धुने मेसिन',
				emoji: '🧺',
				en: 'Washing machine',
			},
			{
				id: 'dori-rope-clothesline',
				dev: 'डोरी',
				emoji: '🪢',
				en: 'Rope / Clothesline',
			},
			{
				id: 'chimtaa-clothespin-peg',
				dev: 'चिम्टा',
				emoji: '📎',
				en: 'Clothespin / Peg',
			},
			{
				id: 'istry-iron-for-clothes',
				dev: 'इस्त्री',
				emoji: '🔌',
				en: 'Iron (for clothes)',
			},
			{
				id: 'topi-hat-cap',
				dev: 'टोपी',
				emoji: '🧢',
				en: 'Hat / cap',
			},
			{
				id: 'kamij-shirt',
				dev: 'कमिज',
				emoji: '👕',
				en: 'Shirt',
			},
			{
				id: 'suruwal-trousers-pants',
				dev: 'सुरुवाल',
				emoji: '👖',
				en: 'Trousers / pants',
			},
		],
	},
	{
		id: 'technology-electronics',
		title: 'Technology & Electronics',
		kind: 'vocab',
		goal: 'Name devices and electronics',
		items: [
			{
				id: 'mobile-cell-phone',
				dev: 'मोबाइल',
				emoji: '📱',
				en: 'Cell phone',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'मेरो मोबाइल कहाँ छ?', en: 'Where is my phone?' },
					{ dev: 'मोबाइल चार्ज गर्नुस्', en: 'Charge the phone' },
				],
			},
			{
				id: 'computer-computer',
				dev: 'कम्प्युटर',
				emoji: '💻',
				en: 'Computer',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'म कम्प्युटरमा काम गर्छु', en: 'I work on the computer' },
					{ dev: 'कम्प्युटर बन्द गर्नुस्', en: 'Shut down the computer' },
				],
			},
			{
				id: 'laptop-laptop',
				dev: 'ल्यापटप',
				emoji: '💻',
				en: 'Laptop',
			},
			{
				id: 'charger-charger',
				dev: 'चार्जर',
				emoji: '🔌',
				en: 'Charger',
			},
			{
				id: 'wifi-wifi',
				dev: 'वाइफाइ',
				emoji: '🛜',
				en: 'WiFi',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'वाइफाइ छैन', en: "There's no wifi" },
					{ dev: 'वाइफाइ पासवर्ड के हो?', en: "What's the wifi password?" },
				],
			},
			{
				id: 'plug-electrical-plug',
				dev: 'प्लग',
				emoji: '🔌',
				en: 'Electrical plug',
			},
			{
				id: 'switch-light-switch',
				dev: 'स्विच',
				emoji: '🎚️',
				en: 'Light switch',
			},
			{
				id: 'headphone-headphones',
				dev: 'हेडफोन',
				emoji: '🎧',
				en: 'Headphones',
			},
			{
				id: 'baytri-battery',
				dev: 'ब्याट्री',
				emoji: '🔋',
				en: 'Battery',
			},
		],
	},
	{
		id: 'personal-items',
		title: 'Personal Items',
		kind: 'vocab',
		goal: 'Name your personal belongings',
		items: [
			{
				id: 'jhola-bag',
				dev: 'झोला',
				emoji: '👜',
				en: 'Bag',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'झोलामा के छ?', en: "What's in the bag?" },
					{ dev: 'झोला यहाँ राख्नुस्', en: 'Put the bag here' },
				],
			},
			{
				id: 'chashma-glasses-spectacles',
				dev: 'चश्मा',
				emoji: '👓',
				en: 'Glasses / Spectacles',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'मेरो चश्मा कहाँ छ?', en: 'Where are my glasses?' },
					{ dev: 'हजुरबुवा चश्मा लगाउनुहुन्छ', en: 'Grandfather wears glasses' },
				],
			},
			{
				id: 'kapi-notebook',
				dev: 'कापी',
				emoji: '📓',
				en: 'Notebook',
			},
			{
				id: 'wallet-wallet',
				dev: 'वालेट',
				emoji: '💳',
				en: 'Wallet',
			},
			{
				id: 'chappal-slippers-sandals',
				dev: 'चप्पल',
				emoji: '🩴',
				en: 'Slippers / Sandals',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'घरभित्र चप्पल लगाउनुस्', en: 'Wear slippers inside the house' },
					{ dev: 'चप्पल सस्तो छ', en: 'Slippers are cheap' },
				],
			},
			{
				id: 'jutta-shoes',
				dev: 'जुत्ता',
				emoji: '👞',
				en: 'Shoes',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'जुत्ता बाहिर राख्नुस्', en: 'Leave your shoes outside' },
					{ dev: 'मेरो जुत्ता नयाँ छ', en: 'My shoes are new' },
				],
			},
			{
				id: 'chhata-umbrella',
				dev: 'छाता',
				emoji: '☂️',
				en: 'Umbrella',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'छाता लिनुस्', en: 'Take an umbrella' },
					{ dev: 'मेरो छाता हरायो', en: 'I lost my umbrella' },
				],
			},
			{
				id: 'ghadi-watch-clock',
				dev: 'घडी',
				emoji: '⌚',
				en: 'Watch / clock',
			},
		],
	},
	{
		id: 'miscellaneous',
		title: 'Miscellaneous',
		kind: 'vocab',
		goal: 'Name common objects',
		items: [
			{
				id: 'bottle-bottle',
				dev: 'बोतल',
				emoji: '🍾',
				en: 'Bottle',
			},
			{
				id: 'box-box',
				dev: 'बाकस',
				emoji: '📦',
				en: 'Box',
			},
			{
				id: 'thailo-bag-cloth-shopping',
				dev: 'थैलो',
				emoji: '🛍️',
				en: 'Bag (cloth/shopping)',
			},
			{
				id: 'mombatti-candle',
				dev: 'मोमबत्ती',
				emoji: '🕯️',
				en: 'Candle',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'बत्ती गयो, मोमबत्ती बाल्नुस्', en: "The power's out — light a candle" },
					{ dev: 'जन्मदिनमा मोमबत्ती बाल्छौं', en: 'We light candles on birthdays' },
				],
			},
			{
				id: 'salai-match-matchstick',
				dev: 'सलाई',
				emoji: '🔥',
				en: 'Match / Matchstick',
			},
			{
				id: 'aushadhi-medicine',
				dev: 'औषधि',
				emoji: '💊',
				en: 'Medicine',
				// T29 batch 8 (household objects; AI-drafted, approved):
				frames: [
					{ dev: 'औषधि खानुस्', en: 'Take your medicine' },
					{ dev: 'मलाई औषधि चाहियो', en: 'I need medicine' },
				],
			},
			{
				id: 'chitthi-letter',
				dev: 'चिठ्ठी',
				emoji: '✉️',
				en: 'Letter',
			},
			{
				id: 'ramailo-fun-entertainment',
				dev: 'रमाइलो',
				emoji: '🎉',
				en: 'Fun / Entertainment',
			},
			{
				id: 'baajaa-musical-instrument',
				dev: 'बाजा',
				emoji: '🎸',
				en: 'Musical instrument',
			},
		],
	},
	{
		id: 'emotions-feelings',
		title: 'Emotions & Feelings',
		kind: 'vocab',
		goal: 'Say how you feel',
		items: [
			{
				id: 'khushi-happy',
				dev: 'खुसी',
				emoji: '😊',
				en: 'Happy',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'आज म खुसी छु', en: "I'm happy today" },
					{ dev: 'तपाईं खुसी हुनुहुन्छ?', en: 'Are you happy?' },
				],
			},
			{
				id: 'dukhi-sad',
				dev: 'दुखी',
				emoji: '😢',
				en: 'Sad',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'ऊ आज दुखी छ', en: "He's sad today" },
					{ dev: 'दुखी गीत', en: 'A sad song' },
				],
			},
			{
				id: 'risaeko-angry',
				dev: 'रिसाएको',
				emoji: '😠',
				en: 'Angry',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'ऊ रिसाएको छ', en: 'He is angry' },
					{ dev: 'म रिसाएको छैन', en: "I'm not angry" },
				],
			},
			{
				id: 'dar-fear-scared',
				dev: 'डर',
				emoji: '😨',
				en: 'Fear / Scared',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'मलाई डर लाग्यो', en: 'I got scared' },
					{ dev: 'बाघदेखि डर लाग्छ', en: "I'm scared of tigers" },
				],
			},
			{
				id: 'chinta-worry-anxiety',
				dev: 'चिन्ता',
				emoji: '😟',
				en: 'Worry / Anxiety',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'चिन्ता नगर्नुस्', en: "Don't worry" },
					{ dev: 'आमालाई चिन्ता छ', en: 'Mother is worried' },
				],
			},
			{
				id: 'maya-love',
				dev: 'माया',
				emoji: '❤️',
				en: 'Love',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'म तिमीलाई माया गर्छु', en: 'I love you' },
					{ dev: 'आमाको माया', en: "A mother's love" },
				],
			},
			{
				id: 'ghamanda-pride-arrogance',
				dev: 'घमण्ड',
				emoji: '😤',
				en: 'Pride / Arrogance',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'घमण्ड नगर्नुस्', en: "Don't be arrogant" },
					{ dev: 'घमण्ड राम्रो होइन', en: "Pride isn't good" },
				],
			},
			{
				id: 'lajja-shame-embarrassment',
				dev: 'लज्जा',
				emoji: '😳',
				en: 'Shame / Embarrassment',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'उसलाई लज्जा लाग्यो', en: 'He was embarrassed' },
					{ dev: 'मलाई लज्जा लाग्दैन', en: "I'm not embarrassed" },
				],
			},
			{
				id: 'thakeko-tired',
				dev: 'थकेको',
				emoji: '😴',
				en: 'Tired',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'तपाईं थकेको हुनुहुन्छ?', en: 'Are you tired?' },
					{ dev: 'थकेको बच्चा', en: 'A tired child' },
				],
			},
			{
				id: 'biraami-sick',
				dev: 'बिरामी',
				emoji: '🤒',
				en: 'Sick',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'मेरो साथी बिरामी छ', en: 'My friend is sick' },
					{ dev: 'म बिरामी छैन', en: "I'm not sick" },
				],
			},
		],
	},
	{
		id: 'emotions-more',
		title: 'More Feelings',
		kind: 'vocab',
		goal: 'Describe subtler moods and emotions',
		items: [
			{
				id: 'aascharyya-surprise-wonder',
				dev: 'आश्चर्य',
				emoji: '😲',
				en: 'Surprise / Wonder',
			},
			{
				id: 'bore-bored',
				dev: 'बोर',
				emoji: '😑',
				en: 'Bored',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'बोर भयो', en: 'This is boring' },
					{ dev: 'फिल्म बोर छ', en: 'The movie is boring' },
				],
			},
			{
				id: 'aaramadaayak-comfortable-relaxed',
				dev: 'आरामदायक',
				emoji: '😌',
				en: 'Comfortable / Relaxed',
			},
			{
				id: 'uddwignata-stress-tension',
				dev: 'उद्विग्नता',
				emoji: '😰',
				en: 'Stress / Tension',
			},
			{
				id: 'khijna-frustration-irritation',
				dev: 'खिज्ने',
				emoji: '😖',
				en: 'Frustration / Irritation',
			},
			{
				id: 'santushta-content-satisfied',
				dev: 'सन्तुष्ट',
				emoji: '😇',
				en: 'Content / Satisfied',
			},
			{
				id: 'upekshya-indifferent-neglected',
				dev: 'उपेक्षा',
				emoji: '😐',
				en: 'Indifferent / Neglected',
			},
			{
				id: 'aakrosh-rage-fury',
				dev: 'आक्रोश',
				emoji: '😡',
				en: 'Rage / Fury',
			},
			{
				id: 'nirasha-disappointment',
				dev: 'निराशा',
				emoji: '😞',
				en: 'Disappointment',
			},
			{
				id: 'asha-hope',
				dev: 'आशा',
				emoji: '🤞',
				en: 'Hope',
				// T29 batch 10 (emotions & people; AI-drafted, approved):
				frames: [
					{ dev: 'मलाई आशा छ', en: 'I have hope' },
					{ dev: 'आशा राम्रो कुरा हो', en: 'Hope is a good thing' },
				],
			},
		],
	},
	{
		id: 'animals',
		title: 'Farm & Home Animals',
		kind: 'vocab',
		goal: 'Name common domestic animals',
		items: [
			{
				id: 'kukur-dog',
				dev: 'कुकुर',
				emoji: '🐕',
				en: 'Dog',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'कुकुर घरमा छ', en: 'The dog is at home' },
					{ dev: 'मेरो कुकुर सानो छ', en: 'My dog is small' },
				],
			},
			{
				id: 'biralo-cat',
				dev: 'बिरालो',
				emoji: '🐈',
				en: 'Cat',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'बिरालो दूध पिउँछ', en: 'The cat drinks milk' },
					{ dev: 'बिरालो सानो छ', en: 'The cat is small' },
				],
			},
			{
				id: 'gai-cow',
				dev: 'गाई',
				emoji: '🐄',
				en: 'Cow',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'गाई दूध दिन्छ', en: 'The cow gives milk' },
					{ dev: 'गाई ठूलो छ', en: 'The cow is big' },
				],
			},
			{
				id: 'raangaa-buffalo',
				dev: 'राँगा',
				emoji: '🐃',
				en: 'Buffalo',
			},
			{
				id: 'bakhra-goat',
				dev: 'बाख्रा',
				emoji: '🐐',
				en: 'Goat',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'बाख्रा घाँस खान्छ', en: 'The goat eats grass' },
					{ dev: 'बाख्रा बाहिर छ', en: 'The goat is outside' },
				],
			},
			{
				id: 'bhedaa-sheep',
				dev: 'भेडा',
				emoji: '🐑',
				en: 'Sheep',
			},
			{
				id: 'ghoda-horse',
				dev: 'घोडा',
				emoji: '🐴',
				en: 'Horse',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'घोडा छिटो दौडन्छ', en: 'The horse runs fast' },
					{ dev: 'घोडा सेतो छ', en: 'The horse is white' },
				],
			},
			{
				id: 'gadha-donkey',
				dev: 'गधा',
				emoji: '🫏',
				en: 'Donkey',
			},
			{
				id: 'sungur-pig',
				dev: 'सुँगुर',
				emoji: '🐖',
				en: 'Pig',
			},
			{
				id: 'kukhura-chicken',
				dev: 'कुखुरा',
				emoji: '🐔',
				en: 'Chicken',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'कुखुरा अन्डा दिन्छ', en: 'The chicken gives eggs' },
					{ dev: 'मलाई कुखुराको मासु मन पर्छ', en: 'I like chicken meat' },
				],
			},
			{
				id: 'haas-duck',
				dev: 'हाँस',
				emoji: '🦆',
				en: 'Duck',
			},
			{
				id: 'chaara-bird',
				dev: 'चरा',
				emoji: '🐦',
				en: 'Bird',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'चरा रुखमा छ', en: 'The bird is in the tree' },
					{ dev: 'चरा उड्छ', en: 'The bird flies' },
				],
			},
		],
	},
	{
		id: 'animals-wild',
		title: 'Wild Animals & Bugs',
		kind: 'vocab',
		goal: 'Name wild animals and insects',
		items: [
			{
				id: 'kaag-crow',
				dev: 'काग',
				emoji: '🐦‍⬛',
				en: 'Crow',
			},
			{
				id: 'mauri-bee',
				dev: 'मौरी',
				emoji: '🐝',
				en: 'Bee',
			},
			{
				id: 'putali-butterfly',
				dev: 'पुतली',
				emoji: '🦋',
				en: 'Butterfly',
			},
			{
				id: 'phatyangro-cricket-grasshopper',
				dev: 'फट्याङ्ग्रो',
				emoji: '🦗',
				en: 'Cricket / Grasshopper',
			},
			{
				id: 'saanp-snake',
				dev: 'साँप',
				emoji: '🐍',
				en: 'Snake',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'साँप लामो छ', en: 'The snake is long' },
					{ dev: 'साँपदेखि डर लाग्छ', en: "I'm scared of snakes" },
				],
			},
			{
				id: 'kachuwa-turtle-tortoise',
				dev: 'कछुवा',
				emoji: '🐢',
				en: 'Turtle / Tortoise',
			},
			{
				id: 'bagh-tiger',
				dev: 'बाघ',
				emoji: '🐅',
				en: 'Tiger',
			},
			{
				id: 'singha-lion',
				dev: 'सिंह',
				emoji: '🦁',
				en: 'Lion',
			},
			{
				id: 'hatti-elephant',
				dev: 'हात्ती',
				emoji: '🐘',
				en: 'Elephant',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'हात्ती धेरै ठूलो छ', en: 'The elephant is very big' },
					{ dev: 'चितवनमा हात्ती छ', en: 'There are elephants in Chitwan' },
				],
			},
			{
				id: 'bandar-monkey',
				dev: 'बाँदर',
				emoji: '🐒',
				en: 'Monkey',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'मन्दिरमा बाँदर छ', en: 'There are monkeys at the temple' },
					{ dev: 'बाँदर केरा खान्छ', en: 'The monkey eats bananas' },
				],
			},
			{
				id: 'bhalu-bear',
				dev: 'भालु',
				emoji: '🐻',
				en: 'Bear',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'वनमा भालु छ', en: "There's a bear in the forest" },
					{ dev: 'भालु ठूलो हुन्छ', en: 'Bears are big' },
				],
			},
		],
	},
	{
		id: 'food-drink',
		title: 'Fruit',
		kind: 'vocab',
		goal: 'Name fruits',
		items: [
			{
				id: 'syau-apple',
				dev: 'स्याउ',
				emoji: '🍎',
				en: 'Apple',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'स्याउ ताजा छ', en: 'The apples are fresh' },
					{ dev: 'एक किलो स्याउ दिनुस्', en: 'A kilo of apples, please' },
				],
			},
			{
				id: 'kera-banana',
				dev: 'केरा',
				emoji: '🍌',
				en: 'Banana',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'केरा पहेँलो छ', en: 'The banana is yellow' },
					{ dev: 'केरा सस्तो छ', en: 'Bananas are cheap' },
				],
			},
			{
				id: 'suntala-orange',
				dev: 'सुन्तला',
				emoji: '🍊',
				en: 'Orange',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'सुन्तला मिठो छ', en: 'The orange is tasty' },
					{ dev: 'जाडोमा सुन्तला खान्छौं', en: 'We eat oranges in winter' },
				],
			},
			{
				id: 'aaru-peach',
				dev: 'आरु',
				emoji: '🍑',
				en: 'Peach',
			},
			{
				id: 'amba-mango',
				dev: 'आँप',
				emoji: '🥭',
				en: 'Mango',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'मलाई आँप मन पर्छ', en: 'I like mangoes' },
					{ dev: 'गर्मीमा आँप आउँछ', en: 'Mangoes come in summer' },
				],
			},
			{
				id: 'angur-grapes',
				dev: 'अंगुर',
				emoji: '🍇',
				en: 'Grapes',
			},
			{
				id: 'tarbujo-watermelon',
				dev: 'तरबुजा',
				emoji: '🍉',
				en: 'Watermelon',
			},
			{
				id: 'kagati-lemon-lime',
				dev: 'कागती',
				emoji: '🍋',
				en: 'Lemon / Lime',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'कागती अमिलो हुन्छ', en: 'Lemons are sour' },
					{ dev: 'कागती पानीमा हाल्नुस्', en: 'Put lemon in the water' },
				],
			},
			{
				id: 'rukh-kaatahar-pineapple',
				dev: 'भुइँकटहर',
				emoji: '🍍',
				en: 'Pineapple',
			},
		],
	},
	{
		id: 'food-veg',
		title: 'Vegetables',
		kind: 'vocab',
		goal: 'Name vegetables',
		items: [
			{
				id: 'golbheda-tomato',
				dev: 'गोलभेंडा',
				emoji: '🍅',
				en: 'Tomato',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'गोलभेंडा रातो छ', en: 'The tomato is red' },
					{ dev: 'गोलभेंडाको अचार मिठो छ', en: 'Tomato chutney is delicious' },
				],
			},
			{
				id: 'kaankro-cucumber',
				dev: 'काँक्रो',
				emoji: '🥒',
				en: 'Cucumber',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'काँक्रो काट्नुस्', en: 'Cut the cucumber' },
					{ dev: 'काँक्रो लामो छ', en: 'The cucumber is long' },
				],
			},
			{
				id: 'bandaa-cabbage',
				dev: 'बन्दाकोबी',
				emoji: '🥬',
				en: 'Cabbage',
			},
			{
				id: 'aaalu-potato',
				dev: 'आलु',
				emoji: '🥔',
				en: 'Potato',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'तरकारीमा आलु हाल्नुस्', en: 'Put potatoes in the curry' },
					{ dev: 'आलु ठूलो छ', en: 'The potato is big' },
				],
			},
			{
				id: 'pyaaj-onion',
				dev: 'प्याज',
				emoji: '🧅',
				en: 'Onion',
			},
			{
				id: 'lasun-garlic',
				dev: 'लसुन',
				emoji: '🧄',
				en: 'Garlic',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'लसुन नहाल्नुस्', en: 'No garlic, please' },
					{ dev: 'लसुनको बास्ना आयो', en: 'I smell garlic' },
				],
			},
			{
				id: 'khursaani-chili-pepper',
				dev: 'खुर्सानी',
				emoji: '🌶️',
				en: 'Chili pepper',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'यो खुर्सानी पिरो छ', en: 'This chili is spicy' },
					{ dev: 'खुर्सानी नहाल्नुस्', en: "Don't add chili" },
				],
			},
			{
				id: 'karela-bitter-gourd',
				dev: 'करेला',
				emoji: '🥒',
				en: 'Bitter gourd',
			},
			{
				id: 'pharsi-pumpkin',
				dev: 'फर्सी',
				emoji: '🎃',
				en: 'Pumpkin',
			},
			{
				id: 'mula-radish-daikon',
				dev: 'मुला',
				emoji: '🥕',
				en: 'Radish / Daikon',
			},
			{
				id: 'simi-green-beans',
				dev: 'सिमी',
				emoji: '🫛',
				en: 'Green beans',
			},
			{
				id: 'broccoli-broccoli',
				dev: 'ब्रोकाउली',
				emoji: '🥦',
				en: 'Broccoli',
			},
			{
				id: 'dhania-coriander-cilantro',
				dev: 'धनिया',
				emoji: '🌿',
				en: 'Coriander / Cilantro',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'धनिया ताजा छ', en: 'The coriander is fresh' },
					{ dev: 'मलाई धनिया मन पर्दैन', en: "I don't like coriander" },
				],
			},
		],
	},
	{
		id: 'food-dishes',
		title: 'Dishes & Drinks',
		kind: 'vocab',
		goal: 'Name Nepali dishes and drinks',
		items: [
			{
				id: 'chiura-beaten-rice-flattened-rice',
				dev: 'चिउरा',
				emoji: '🌾',
				en: 'Beaten rice / Flattened rice',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'दहीसँग चिउरा खान्छु', en: 'I eat beaten rice with yogurt' },
					{ dev: 'खाजामा चिउरा खान्छौं', en: 'We have chiura for snacks' },
				],
			},
			{
				id: 'momo-dumplings-momos',
				dev: 'मोमो',
				emoji: '🥟',
				en: 'Dumplings / Momos',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'मलाई मोमो मन पर्छ', en: 'I like momos' },
					{ dev: 'दसवटा मोमो दिनुस्', en: 'Ten momos, please' },
				],
			},
			{
				id: 'sel-roti-sel-roti-ring-shaped-bread',
				dev: 'सेलरोटी',
				emoji: '🍩',
				en: 'Sel roti (ring-shaped bread)',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'तिहारमा सेलरोटी बनाउँछौं', en: 'We make sel roti at Tihar' },
					{ dev: 'सेलरोटी मिठो हुन्छ', en: 'Sel roti is delicious' },
				],
			},
			{
				id: 'chatamari-nepali-pizza',
				dev: 'चटामरी',
				emoji: '🍕',
				en: 'Nepali pizza',
			},
			{
				id: 'samosa-samosa',
				dev: 'समोसा',
				emoji: '🥟',
				en: 'Samosa',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'समोसा तातो छ', en: 'The samosas are hot' },
					{ dev: 'समोसा र चिया', en: 'Samosas and tea' },
				],
			},
			{
				id: 'pakoda-fritters-pakora',
				dev: 'पकौडा',
				emoji: '🍤',
				en: 'Fritters / Pakora',
			},
			{
				id: 'mithai-sweets-dessert',
				dev: 'मिठाई',
				emoji: '🍬',
				en: 'Sweets / Dessert',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'मिठाई धेरै नखानुस्', en: "Don't eat too many sweets" },
					{ dev: 'बच्चालाई मिठाई मन पर्छ', en: 'Kids love sweets' },
				],
			},
			{
				id: 'khaja-snacks',
				dev: 'खाजा',
				emoji: '🍪',
				en: 'Snacks',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'खाजा खाने बेला भयो', en: "It's snack time" },
					{ dev: 'म खाजा खान्छु', en: 'I eat a snack' },
				],
			},
			{
				id: 'raksi-traditional-alcohol',
				dev: 'रक्सी',
				emoji: '🍶',
				en: 'Traditional alcohol',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'म रक्सी खाँदिनँ', en: "I don't drink" },
					{ dev: 'रक्सी नखानुस्', en: "Don't drink" },
				],
			},
			{
				id: 'lassi-yogurt-drink',
				dev: 'लस्सी',
				emoji: '🥛',
				en: 'Yogurt drink',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'एक गिलास लस्सी दिनुस्', en: 'A glass of lassi, please' },
					{ dev: 'लस्सी चिसो हुन्छ', en: 'Lassi is served cold' },
				],
			},
			{
				id: 'juice-juice',
				dev: 'जुस',
				emoji: '🧃',
				en: 'Juice',
				// T29 batch 16 (food & kitchen leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'आँपको जुस दिनुस्', en: 'Mango juice, please' },
					{ dev: 'जुस ताजा छ', en: 'The juice is fresh' },
				],
			},
			{
				id: 'coffee-coffee',
				dev: 'कफी',
				emoji: '☕',
				en: 'Coffee',
				// T29 batch 7 (food & kitchen; AI-drafted, approved):
				frames: [
					{ dev: 'म बिहान कफी पिउँछु', en: 'I drink coffee in the morning' },
					{ dev: 'कफी बलियो छ', en: 'The coffee is strong' },
				],
			},
		],
	},
	{
		id: 'weather-nature',
		title: 'Sky & Storms',
		kind: 'vocab',
		goal: 'Name the sky and weather',
		items: [
			{
				id: 'ghaam-sun-sunshine',
				dev: 'घाम',
				emoji: '☀️',
				en: 'Sun / Sunshine',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'बिहान घाम लाग्छ', en: 'The sun shines in the morning' },
					{ dev: 'आज घाम छैन', en: "There's no sun today" },
				],
			},
			{
				id: 'chandra-moon',
				dev: 'चन्द्रमा',
				emoji: '🌙',
				en: 'Moon',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'राति चन्द्रमा हेर्नुस्', en: 'Look at the moon tonight' },
					{ dev: 'चन्द्रमा टाढा छ', en: 'The moon is far away' },
				],
			},
			{
				id: 'tara-star',
				dev: 'तारा',
				emoji: '⭐',
				en: 'Star',
			},
			{
				id: 'badal-cloud',
				dev: 'बादल',
				emoji: '☁️',
				en: 'Cloud',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'आकाशमा बादल छ', en: 'There are clouds in the sky' },
					{ dev: 'कालो बादल आयो', en: 'Dark clouds came' },
				],
			},
			{
				id: 'pani-rain',
				dev: 'पानी',
				emoji: '🌧️',
				en: 'Rain',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'मनसुनमा पानी धेरै पर्छ', en: 'It rains a lot in monsoon' },
					{ dev: 'पानी पर्छ जस्तो छ', en: 'It looks like rain' },
				],
			},
			{
				id: 'haawaahuri-wind-storm',
				dev: 'हावाहुरी',
				emoji: '💨',
				en: 'Wind / Storm',
			},
			{
				id: 'aandhi-storm-windstorm',
				dev: 'आँधी',
				emoji: '🌪️',
				en: 'Storm / Windstorm',
			},
			{
				id: 'hiune-paani-snow',
				dev: 'हिउँ',
				emoji: '❄️',
				en: 'Snow',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'पहाडमा हिउँ छ', en: "There's snow on the mountains" },
					{ dev: 'हिउँ सेतो छ', en: 'Snow is white' },
				],
			},
			{
				id: 'tuhaaro-fog-mist',
				dev: 'तुवाँलो',
				emoji: '🌫️',
				en: 'Fog / Mist',
			},
			{
				id: 'asina-hail',
				dev: 'असिना',
				emoji: '🌨️',
				en: 'Hail',
			},
			{
				id: 'bijuli-lightning-electricity',
				dev: 'बिजुली',
				emoji: '⚡',
				en: 'Lightning / Electricity',
			},
			{
				id: 'megh-garjana-thunder',
				dev: 'मेघ गर्जन',
				emoji: '⛈️',
				en: 'Thunder',
			},
		],
	},
	{
		id: 'weather-land',
		title: 'Weather & Land',
		kind: 'vocab',
		goal: 'Name climate and landforms',
		items: [
			{
				id: 'indreini-rainbow',
				dev: 'इन्द्रेणी',
				emoji: '🌈',
				en: 'Rainbow',
			},
			{
				id: 'garmi-heat-hot-weather',
				dev: 'गर्मी',
				emoji: '🔥',
				en: 'Heat / Hot weather',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'आज धेरै गर्मी छ', en: "It's very hot today" },
					{ dev: 'गर्मीमा धेरै पानी खानुस्', en: 'Drink lots of water in the heat' },
				],
			},
			{
				id: 'jado-cold-winter',
				dev: 'जाडो',
				emoji: '🧊',
				en: 'Cold / Winter',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'मलाई जाडो लाग्यो', en: 'I feel cold' },
					{ dev: 'जाडोमा हिउँ पर्छ', en: 'It snows in winter' },
				],
			},
			{
				id: 'monsoon-monsoon-season',
				dev: 'मनसुन',
				emoji: '🌧️',
				en: 'Monsoon season',
			},
			{
				id: 'haawa-air-wind',
				dev: 'हावा',
				emoji: '💨',
				en: 'Air / Wind',
			},
			{
				id: 'aakaash-sky',
				dev: 'आकाश',
				emoji: '🌌',
				en: 'Sky',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'आकाश निलो छ', en: 'The sky is blue' },
					{ dev: 'राति आकाशमा तारा छ', en: 'There are stars in the sky at night' },
				],
			},
			{
				id: 'dharti-earth-ground',
				dev: 'धरती',
				emoji: '🌍',
				en: 'Earth / Ground',
			},
			{
				id: 'sansar-world',
				dev: 'संसार',
				emoji: '🌍',
				en: 'World',
			},
			{
				id: 'pahaad-mountain-hill',
				dev: 'पहाड',
				emoji: '⛰️',
				en: 'Mountain / Hill',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'नेपालमा पहाड धेरै छ', en: 'Nepal has many mountains' },
					{ dev: 'म पहाड जान्छु', en: 'I go to the mountains' },
				],
			},
		],
	},
	{
		id: 'weather-water',
		title: 'Water & Nature',
		kind: 'vocab',
		goal: 'Name rivers, forests, and nature',
		items: [
			{
				id: 'nadhi-river',
				dev: 'नदी',
				emoji: '🏞️',
				en: 'River',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'नदी सफा छ', en: 'The river is clean' },
					{ dev: 'नदी पहाडबाट आउँछ', en: 'The river comes from the mountains' },
				],
			},
			{
				id: 'taal-lake',
				dev: 'ताल',
				emoji: '🏞️',
				en: 'Lake',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'तालमा माछा छ', en: 'There are fish in the lake' },
					{ dev: 'पोखरामा ठूलो ताल छ', en: 'Pokhara has a big lake' },
				],
			},
			{
				id: 'ban-forest',
				dev: 'वन',
				emoji: '🌲',
				en: 'Forest',
			},
			{
				id: 'rukh-tree',
				dev: 'रुख',
				emoji: '🌳',
				en: 'Tree',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'रुख ठूलो छ', en: 'The tree is big' },
					{ dev: 'रुखमुनि बस्नुस्', en: 'Sit under the tree' },
				],
			},
			{
				id: 'phul-flower',
				dev: 'फूल',
				emoji: '🌸',
				en: 'Flower',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'फूल राम्रो छ', en: 'The flower is pretty' },
					{ dev: 'फूलमा मौरी छ', en: "There's a bee on the flower" },
				],
			},
			{
				id: 'ghaas-grass',
				dev: 'घाँस',
				emoji: '🌱',
				en: 'Grass',
			},
			{
				id: 'khola-stream-creek',
				dev: 'खोला',
				emoji: '🏞️',
				en: 'Stream / Creek',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'खोलामा पानी चिसो छ', en: 'The stream water is cold' },
					{ dev: 'खोला सानो छ', en: 'The stream is small' },
				],
			},
			{
				id: 'jharna-waterfall',
				dev: 'झरना',
				emoji: '💧',
				en: 'Waterfall',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'झरना हेर्न जान्छौं', en: 'We go to see the waterfall' },
					{ dev: 'झरना ठूलो छ', en: 'The waterfall is big' },
				],
			},
			{
				id: 'bhukampa-earthquake',
				dev: 'भूकम्प',
				emoji: '🌋',
				en: 'Earthquake',
				// T29 batch 17 (household & nature leftovers; AI-drafted, approved):
				frames: [
					{ dev: 'भूकम्प आयो!', en: 'Earthquake!' },
					{ dev: 'भूकम्पमा बाहिर जानुस्', en: 'In an earthquake, go outside' },
				],
			},
			{
				id: 'aago-fire',
				dev: 'आगो',
				emoji: '🔥',
				en: 'Fire',
				// T29 batch 9 (weather, nature & animals; AI-drafted, approved):
				frames: [
					{ dev: 'आगो तातो छ', en: 'Fire is hot' },
					{ dev: 'आगो नछुनुस्', en: "Don't touch the fire" },
				],
			},
		],
	},
	{
		id: 'jobs-work',
		title: 'Jobs & Work',
		kind: 'phrases',
		goal: 'Talk about jobs and what you do',
		items: [
			{
				id: 'tapai-ke-kaam-garnuhunchha-what-do-you-do',
				dev: 'तपाईं के काम गर्नुहुन्छ?',
				en: 'What do you do? (your job)',
				usage: 'The polite way to ask someone\'s occupation — literally "what work do you do?" It uses "kaam" (work) and the polite "garnuhunchha." The natural reply is "Ma ___ hu" (I am a ___).',
			},
			{
				id: 'ma-shikshak-hu-i-am-a-teacher',
				dev: 'म शिक्षक हुँ',
				en: 'I am a teacher',
				usage: 'The "I am a ___" pattern for jobs: "ma" (I) + the job + "hu" (am). "Shikshak" is a teacher. Just swap the job word to say what you do.',
			},
			{
				id: 'ma-vidyarthi-hu-i-am-a-student',
				dev: 'म विद्यार्थी हुँ',
				en: 'I am a student',
				usage: '"Vidyarthi" means student. "Ma vidyarthi hu" (I am a student) — the same "Ma ___ hu" frame as every job below.',
			},
			{
				id: 'ma-daktar-hu-i-am-a-doctor',
				dev: 'म डाक्टर हुँ',
				en: 'I am a doctor',
				usage: 'Nepali borrows the English word: "daktar" (doctor). "Ma daktar hu" (I am a doctor). You saw it in "doctor chaahiyo" (I need a doctor).',
			},
			{
				id: 'ma-kisaan-hu-i-am-a-farmer',
				dev: 'म किसान हुँ',
				en: 'I am a farmer',
				usage: '"Kisaan" is a farmer — a very common occupation in Nepal. "Ma kisaan hu" (I am a farmer).',
			},
			{
				id: 'ma-pasale-hu-i-am-a-shopkeeper',
				dev: 'म पसले हुँ',
				en: 'I am a shopkeeper',
				usage: '"Pasal" is a shop, and a "pasale" is the shopkeeper who runs it. "Ma pasale hu" (I am a shopkeeper).',
			},
			{
				id: 'ma-draibhar-hu-i-am-a-driver',
				dev: 'म ड्राइभर हुँ',
				en: 'I am a driver',
				usage: 'Another borrowed word: "draibhar" (driver). "Ma draibhar hu" (I am a driver). Handy when talking to a "taxi draibhar."',
			},
			{
				id: 'ma-injinier-hu-i-am-an-engineer',
				dev: 'म इन्जिनियर हुँ',
				en: 'I am an engineer',
				usage: '"Injinier" (engineer) is borrowed from English. "Ma injinier hu" (I am an engineer). Common in cities and among Nepalis working abroad.',
			},
			{
				id: 'ma-nars-hu-i-am-a-nurse',
				dev: 'म नर्स हुँ',
				en: 'I am a nurse',
				usage: '"Nars" (nurse) is another English loanword. "Ma nars hu" (I am a nurse). Pairs with "daktar" (doctor) from the same medical world.',
			},
			{
				id: 'ma-prahari-hu-i-am-a-police-officer',
				dev: 'म प्रहरी हुँ',
				en: 'I am a police officer',
				usage: '"Prahari" means police or a police officer. "Ma prahari hu" (I am a police officer). You\'ll see the word on uniforms and stations ("Nepal Prahari").',
			},
			{
				id: 'tapai-kaha-kaam-garnuhunchha-where-do-you-work',
				dev: 'तपाईं कहाँ काम गर्नुहुन्छ?',
				en: 'Where do you work?',
				usage: 'A natural follow-up: "kaha" (where) + "kaam garnuhunchha" (do you work). Answer with a place + "ma" (in): "ma haspitalma kaam garchu" (I work at the hospital).',
			},
		],
	},
	{
		id: 'duration-frequency',
		title: 'Duration & Frequency',
		kind: 'phrases',
		goal: 'Talk about how long and how often',
		items: [
			{
				id: 'minet-minute',
				dev: 'मिनेट',
				en: 'Minute',
				usage: 'The borrowed word "minet" (minute). "Ek minet" (one minute), "paach minet" (five minutes). "Ek minet parkhanus" (please wait a minute) is the everyday use.',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'पाँच मिनेट', en: 'Five minutes' },
					{ dev: 'एक मिनेट पर्खनुस्', en: 'Wait a minute' },
				],
			},
			{
				id: 'ek-ghanta-one-hour',
				dev: 'एक घण्टा',
				en: 'One hour',
				usage: '"Ghanta" is an hour (it also means a bell). "Ek ghanta" (one hour), "dui ghanta" (two hours). To ask how long: "kati ghanta laagcha?" (how many hours does it take?).',
			},
			{
				id: 'haptaa-week',
				dev: 'हप्ता',
				en: 'Week',
				usage: '"Haptaa" means a week. "Yo haptaa" (this week), "aaune haptaa" (next week), "gaeko haptaa" (last week).',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'अर्को हप्ता', en: 'Next week' },
					{ dev: 'यो हप्ता', en: 'This week' },
				],
			},
			{
				id: 'mahina-month',
				dev: 'महिना',
				en: 'Month',
				usage: '"Mahina" means a month. "Yo mahina" (this month), "aaune mahina" (next month), "ek mahina" (one month).',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'अर्को महिना', en: 'Next month' },
					{ dev: 'यो महिना', en: 'This month' },
				],
			},
			{
				id: 'barsa-year',
				dev: 'वर्ष',
				en: 'Year',
				usage: '"Barsa" means a year (you may also hear "saal"). "Yo barsa" (this year), "aghillo barsa" (last year). It also appears in "Naya barsa" (New Year).',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'यो वर्ष', en: 'This year' },
					{ dev: 'अर्को वर्ष', en: 'Next year' },
				],
			},
			{
				id: 'pachi-later-after',
				dev: 'पछि',
				en: 'Later / After',
				usage: '"Pachi" means later or after. "Pachi bhetaula" (see you later), "ek ghanta pachi" (after one hour). The opposite is "aghi" (before / ago).',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'म पछि आउँछु', en: 'I will come later' },
					{ dev: 'खाना पछि', en: 'After the meal' },
				],
			},
			{
				id: 'aghi-before-ago',
				dev: 'अघि',
				en: 'Before / Ago',
				usage: '"Aghi" means before or ago. "Ek ghanta aghi" (an hour ago), "aghi nai" (already / beforehand). Pairs with "pachi" (after).',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'एक घण्टा अघि', en: 'An hour ago' },
					{ dev: 'धेरै अघि', en: 'Long ago' },
				],
			},
			{
				id: 'sadhai-always',
				dev: 'सधैं',
				en: 'Always',
				usage: '"Sadhai" means always. For frequency it sits before the verb: "u sadhai aaunchha" (he always comes), "ma sadhai bihaana hidchu" (I always walk in the morning).',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'म सधैं बिहान उठ्छु', en: 'I always get up in the morning' },
					{ dev: 'ऊ सधैं ढिलो आउँछ', en: 'He always comes late' },
				],
			},
			{
				id: 'kahilekaahi-sometimes',
				dev: 'कहिलेकाहीं',
				en: 'Sometimes',
				usage: '"Kahilekaahi" means sometimes or occasionally. "Ma kahilekaahi maasu khanchu" (I sometimes eat meat). It is built from "kahile" (when) doubled.',
				// T29 depth frames (AI-drafted, approved):
				frames: [
					{ dev: 'म कहिलेकाहीं बजार जान्छु', en: 'I sometimes go to the market' },
					{ dev: 'कहिलेकाहीं गाह्रो हुन्छ', en: 'Sometimes it is hard' },
				],
			},
			{
				id: 'kahile-pani-never',
				dev: 'कहिले पनि',
				en: 'Never (with a negative)',
				usage: '"Kahile pani" with a negative verb means "never." "Ma kahile pani jaadina" (I never go). The "pani" (even) plus a negative is what makes it "not ever."',
			},
		],
	},
	// --- T11 batch 3: everyday nouns (frames), appended as an advanced tier at the end of the path. Titles/goals AI-drafted.
	{
		id: 'noun-time-events',
		title: 'Time & Events',
		kind: 'phrases',
		goal: 'Talk about time and occasions — the date, a plan, an event, a meeting',
		items: [
			{
				id: 'noun-samay-time',
				dev: 'मसँग समय छैन',
				en: "I don't have time",
				usage: "समय = time. मसँग = 'with me / I have'.",
			},
			{
				id: 'noun-saal-year',
				dev: 'अर्को साल',
				en: 'Next year',
				usage: 'साल = year. अर्को = next / another.',
			},
			{
				id: 'noun-gate-date',
				dev: 'आज कति गते?',
				en: "What's the date today?",
				usage: 'गते = the date (day of the month). Nepali dates count गते 1–32.',
			},
			{
				id: 'noun-abasar-opportunity',
				dev: 'राम्रो अवसर',
				en: 'A good opportunity',
				usage: 'अवसर = opportunity / chance.',
			},
			{
				id: 'noun-ghatana-event',
				dev: 'ठूलो घटना',
				en: 'A big event',
				usage: 'घटना = event / incident / happening.',
			},
			{
				id: 'noun-karyakram-program',
				dev: 'आजको कार्यक्रम',
				en: "Today's program",
				usage: "कार्यक्रम = program / event / schedule. आजको = today's.",
			},
			{
				id: 'noun-tayari-preparation',
				dev: 'तयारी भयो',
				en: "It's ready",
				usage: "तयारी = preparation. तयारी भयो = 'preparation is done / it's ready'.",
			},
			{
				id: 'noun-baithak-meeting',
				dev: 'बैठक छ',
				en: "There's a meeting",
				usage: 'बैठक = meeting.',
			},
			{
				id: 'noun-pratiyogita-competition',
				dev: 'फुटबल प्रतियोगिता',
				en: 'A football competition',
				usage: 'प्रतियोगिता = competition / contest.',
			},
		],
	},
	{
		id: 'noun-ideas-talk',
		title: 'Ideas & Conversation',
		kind: 'phrases',
		goal: 'Explain and discuss — a reason, a question, a topic, an opinion, trust',
		items: [
			{
				id: 'noun-kaaran-reason',
				dev: 'के कारणले?',
				en: 'For what reason?',
				usage: "कारण = reason / cause. के कारणले = 'for what reason / why'.",
			},
			{
				id: 'noun-prashna-question',
				dev: 'मेरो प्रश्न छ',
				en: 'I have a question',
				usage: 'प्रश्न = question.',
			},
			{
				id: 'noun-bishaya-topic',
				dev: 'अर्को विषय',
				en: 'Another topic',
				usage: 'विषय = topic / subject / matter.',
			},
			{
				id: 'noun-chhalphal-discussion',
				dev: 'छलफल गरौं',
				en: "Let's discuss",
				usage: "छलफल = discussion. गरौं = 'let's do'.",
			},
			{
				id: 'noun-bhanai-opinion',
				dev: 'उहाँको भनाइमा',
				en: 'In his opinion',
				usage: "भनाइ = a saying / statement / opinion (from भन्नु 'to say'). उहाँको = his/her (respectful).",
			},
			{
				id: 'noun-bishwaas-trust',
				dev: 'मलाई विश्वास छ',
				en: 'I trust / I believe',
				usage: 'विश्वास = trust / belief / faith.',
			},
			{
				id: 'noun-sambandha-relationship',
				dev: 'राम्रो सम्बन्ध',
				en: 'A good relationship',
				usage: 'सम्बन्ध = relation / relationship / connection.',
			},
			{
				id: 'noun-bare-about',
				dev: 'यसको बारेमा',
				en: 'About this',
				usage: 'बारे = about / regarding (usually बारेमा). यसको = of this.',
			},
			{
				id: 'noun-sambhaavana-possibility',
				dev: 'सम्भावना छ',
				en: "There's a possibility",
				usage: 'सम्भावना = possibility / likelihood.',
			},
		],
	},
	{
		id: 'noun-situations',
		title: 'Problems & Solutions',
		kind: 'phrases',
		goal: 'Handle situations — a problem, a decision, a plan, a change, safety',
		items: [
			{
				id: 'noun-samasya-problem',
				dev: 'ठूलो समस्या',
				en: 'A big problem',
				usage: 'समस्या = problem / issue.',
			},
			{
				id: 'noun-nirnaya-decision',
				dev: 'मेरो निर्णय',
				en: 'My decision',
				usage: 'निर्णय = decision.',
			},
			{
				id: 'noun-yojana-plan',
				dev: 'के योजना छ?',
				en: "What's the plan?",
				usage: 'योजना = plan / scheme.',
			},
			{
				id: 'noun-avastha-situation',
				dev: 'अहिलेको अवस्था',
				en: 'The current situation',
				usage: 'अवस्था = situation / condition / state. अहिलेको = current / of now.',
			},
			{
				id: 'noun-parivartan-change',
				dev: 'ठूलो परिवर्तन',
				en: 'A big change',
				usage: 'परिवर्तन = change.',
			},
			{
				id: 'noun-upachaar-treatment',
				dev: 'राम्रो उपचार',
				en: 'Good treatment',
				usage: 'उपचार = (medical) treatment.',
			},
			{
				id: 'noun-surakshaa-safety',
				dev: 'सुरक्षा छ',
				en: "It's safe",
				usage: 'सुरक्षा = safety / security.',
			},
			{
				id: 'noun-bhaag-part',
				dev: 'मेरो भाग',
				en: 'My share',
				usage: 'भाग = part / share / portion.',
			},
			{
				id: 'noun-prayog-use',
				dev: 'यसको प्रयोग',
				en: 'Its use',
				usage: 'प्रयोग = use / usage / application.',
			},
		],
	},
	{
		id: 'noun-money',
		title: 'Money & Business',
		kind: 'phrases',
		goal: 'Talk money — rupees, cost, an amount, a company, a service',
		items: [
			{
				id: 'noun-rupaiyaan-rupee',
				dev: 'सय रुपैयाँ',
				en: 'A hundred rupees',
				usage: "रुपैयाँ = rupee (Nepal's currency). सय = hundred.",
			},
			{
				id: 'noun-kharcha-expense',
				dev: 'धेरै खर्च भयो',
				en: 'It cost a lot',
				usage: 'खर्च = expense / cost / spending. धेरै = a lot.',
			},
			{
				id: 'noun-rakam-amount',
				dev: 'ठूलो रकम',
				en: 'A big amount',
				usage: 'रकम = amount / sum (of money).',
			},
			{
				id: 'noun-pratishat-percent',
				dev: 'दस प्रतिशत',
				en: 'Ten percent',
				usage: 'प्रतिशत = percent. दस = ten.',
			},
			{
				id: 'noun-nambar-number',
				dev: 'फोन नम्बर',
				en: 'Phone number',
				usage: "नम्बर = number (loanword; e.g. घर नम्बर 'house number').",
			},
			{
				id: 'noun-bikri-sale',
				dev: 'बिक्री भयो',
				en: 'It sold',
				usage: "बिक्री = sale / selling. बिक्री भयो = 'a sale happened / it sold'.",
			},
			{
				id: 'noun-kampani-company',
				dev: 'ठूलो कम्पनी',
				en: 'A big company',
				usage: 'कम्पनी = company / firm (loanword).',
			},
			{
				id: 'noun-sewa-service',
				dev: 'राम्रो सेवा',
				en: 'Good service',
				usage: 'सेवा = service.',
			},
			{
				id: 'noun-bajet-budget',
				dev: 'मेरो बजेट',
				en: 'My budget',
				usage: 'बजेट = budget (loanword).',
			},
		],
	},
	{
		id: 'noun-people-places',
		title: 'People & Places',
		kind: 'phrases',
		goal: 'Name people and places — a person, family, a team, the country, an area',
		items: [
			{
				id: 'noun-byakti-person',
				dev: 'एक व्यक्ति',
				en: 'One person',
				usage: 'व्यक्ति = a person / individual (more formal than मान्छे). एक = one.',
			},
			{
				id: 'noun-mahila-woman',
				dev: 'एउटी महिला',
				en: 'A woman',
				usage: 'महिला = woman (polite / neutral). एउटी = one (for a female).',
			},
			{
				id: 'noun-pariwaar-family',
				dev: 'मेरो परिवार',
				en: 'My family',
				usage: 'परिवार = family (the whole household).',
			},
			{
				id: 'noun-toli-team',
				dev: 'हाम्रो टोली',
				en: 'Our team',
				usage: 'टोली = team / group / squad. हाम्रो = our.',
			},
			{
				id: 'noun-samuha-group',
				dev: 'एउटा समूह',
				en: 'A group',
				usage: 'समूह = group.',
			},
			{
				id: 'noun-sadasya-member',
				dev: 'टोलीको सदस्य',
				en: 'A team member',
				usage: 'सदस्य = member. टोलीको = of the team.',
			},
			{
				id: 'noun-desh-country',
				dev: 'मेरो देश',
				en: 'My country',
				usage: 'देश = country / nation.',
			},
			{
				id: 'noun-kshetra-area',
				dev: 'यो क्षेत्र',
				en: 'This area',
				usage: 'क्षेत्र = area / region / field.',
			},
			{
				id: 'noun-sadak-road',
				dev: 'ठूलो सडक',
				en: 'A big road',
				usage: "सडक = road / street (paved). बाटो is the more general 'way / path'.",
			},
			{
				id: 'noun-jagga-land',
				dev: 'जग्गा किन्नु',
				en: 'To buy land',
				usage: 'जग्गा = land / plot / property. किन्नु = to buy.',
			},
		],
	},
	// --- T11 batch 4: everyday adverbs (frames), appended as an advanced tier at the end of the path. Titles/goals AI-drafted.
	{
		id: 'adv-degree',
		title: 'How Much',
		kind: 'phrases',
		goal: 'Say how much — very, a little, at least, even more, the most',
		items: [
			{
				id: 'adv-nikai-quite',
				dev: 'यो निकै राम्रो छ',
				en: 'This is quite good',
				usage: 'निकै = quite / very / a good deal.',
			},
			{
				id: 'adv-ati-too',
				dev: 'अति गाह्रो छ',
				en: "It's too hard",
				usage: "अति = very / too / extremely (often 'too much').",
			},
			{
				id: 'adv-ekdamai-extremely',
				dev: 'एकदमै राम्रो',
				en: 'Excellent',
				usage: 'एकदमै = extremely / absolutely.',
			},
			{
				id: 'adv-thorai-little',
				dev: 'थोरै दिनुस्',
				en: 'Give a little',
				usage: 'थोरै = a little / a few. दिनुस् = give.',
			},
			{
				id: 'adv-kamtima-atleast',
				dev: 'कम्तीमा एक',
				en: 'At least one',
				usage: 'कम्तीमा = at least.',
			},
			{
				id: 'adv-jhan-evenmore',
				dev: 'झन् राम्रो',
				en: 'Even better',
				usage: 'झन् = even more / all the more.',
			},
			{
				id: 'adv-purai-entirely',
				dev: 'पूरै भयो',
				en: "It's completely done",
				usage: 'पूरै = entirely / completely.',
			},
			{
				id: 'adv-khasai-particularly',
				dev: 'खासै छैन',
				en: 'Not really / not much',
				usage: "खासै = particularly / much — usually with a negative (खासै छैन 'not much').",
			},
			{
				id: 'adv-sabaibhanda-most',
				dev: 'सबैभन्दा राम्रो',
				en: 'The best',
				usage: "सबैभन्दा = most / -est. सबैभन्दा राम्रो = 'the best' (literally 'good more than all').",
			},
			{
				id: 'adv-jati-asmuchas',
				dev: 'जति सक्छ',
				en: 'As much as possible',
				usage: "जति = as much / as many as. जति सक्छ = 'as much as one can'.",
			},
		],
	},
	{
		id: 'adv-time-order',
		title: 'Before & After',
		kind: 'phrases',
		goal: 'Place things in time — earlier, after that, just now, soon, finally, not yet',
		items: [
			{
				id: 'adv-pahile-earlier',
				dev: 'पहिले भन्नुस्',
				en: 'Say it first',
				usage: 'पहिले = before / earlier / first.',
			},
			{
				id: 'adv-tyaspachhi-afterthat',
				dev: 'त्यसपछि के भयो?',
				en: 'What happened after that?',
				usage: 'त्यसपछि = after that / then.',
			},
			{
				id: 'adv-ahilesamma-sofar',
				dev: 'अहिलेसम्म ठीक छ',
				en: 'So far so good',
				usage: 'अहिलेसम्म = so far / until now. ठीक = fine / okay.',
			},
			{
				id: 'adv-bharkharai-justnow',
				dev: 'भर्खरै आयो',
				en: 'It just arrived',
				usage: 'भर्खरै = just now / a moment ago / recently.',
			},
			{
				id: 'adv-chhittai-soon',
				dev: 'म छिट्टै आउँछु',
				en: "I'll come soon",
				usage: 'छिट्टै = soon / quickly.',
			},
			{
				id: 'adv-balla-finally',
				dev: 'बल्ल आयो',
				en: 'It finally came',
				usage: 'बल्ल = finally / only just.',
			},
			{
				id: 'adv-ajhai-yet',
				dev: 'अझै भएको छैन',
				en: "It's not done yet",
				usage: 'अझै = still / yet.',
			},
			{
				id: 'adv-pahilopatak-firsttime',
				dev: 'यो पहिलोपटक हो',
				en: 'This is the first time',
				usage: 'पहिलोपटक = for the first time.',
			},
		],
	},
	{
		id: 'adv-frequency',
		title: 'How Often',
		kind: 'phrases',
		goal: 'Say how often — usually, mostly, again and again, never, once',
		items: [
			{
				id: 'adv-prayah-usually',
				dev: 'म प्रायः जान्छु',
				en: 'I usually go',
				usage: 'प्रायः = usually / often / mostly.',
			},
			{
				id: 'adv-dheraijaso-mostly',
				dev: 'धेरैजसो घरमै',
				en: 'Mostly at home',
				usage: 'धेरैजसो = mostly / most of the time. घरमै = at home.',
			},
			{
				id: 'adv-patakpatak-repeatedly',
				dev: 'पटकपटक हुन्छ',
				en: 'It happens again and again',
				usage: 'पटकपटक = repeatedly / again and again.',
			},
			{
				id: 'adv-kahilyai-never',
				dev: 'कहिल्यै हुँदैन',
				en: 'It never happens',
				usage: 'कहिल्यै = ever; with a negative = never.',
			},
			{
				id: 'adv-ekpatak-once',
				dev: 'एकपटक हेर्नुस्',
				en: 'Take a look',
				usage: 'एकपटक = once / one time. हेर्नुस् = look.',
			},
			{
				id: 'adv-lagataar-continuously',
				dev: 'लगातार तीन दिन',
				en: 'Three days in a row',
				usage: 'लगातार = continuously / non-stop / in a row.',
			},
			{
				id: 'adv-yaspatak-thistime',
				dev: 'यसपटक म जान्छु',
				en: "This time I'll go",
				usage: 'यसपटक = this time.',
			},
			{
				id: 'adv-raati-atnight',
				dev: 'राती सुत्छु',
				en: 'I sleep at night',
				usage: 'राती = at night.',
			},
		],
	},
	{
		id: 'adv-manner-place',
		title: 'How & Where',
		kind: 'phrases',
		goal: 'Say how and where — like this, slowly, together, here, over there, nowhere',
		items: [
			{
				id: 'adv-yasari-thisway',
				dev: 'यसरी गर्नुस्',
				en: 'Do it like this',
				usage: 'यसरी = in this way / like this. गर्नुस् = do.',
			},
			{
				id: 'adv-bistarai-slowly',
				dev: 'विस्तारै बोल्नुस्',
				en: 'Speak slowly',
				usage: 'विस्तारै = slowly / gently. बोल्नुस् = speak.',
			},
			{
				id: 'adv-eklai-alone',
				dev: 'म एक्लै छु',
				en: "I'm alone",
				usage: 'एक्लै = alone / by oneself.',
			},
			{
				id: 'adv-sangai-together',
				dev: 'सँगै जाऔं',
				en: "Let's go together",
				usage: "सँगै = together. जाऔं = let's go.",
			},
			{
				id: 'adv-chhuttai-separately',
				dev: 'छुट्टै राख्नुस्',
				en: 'Keep it separate',
				usage: 'छुट्टै = separately / apart. राख्नुस् = keep / put.',
			},
			{
				id: 'adv-yata-thisway',
				dev: 'यता आउनुस्',
				en: 'Come this way',
				usage: 'यता = here / this way / over here. आउनुस् = come.',
			},
			{
				id: 'adv-uta-overthere',
				dev: 'उता जानुस्',
				en: 'Go over there',
				usage: 'उता = over there / that way.',
			},
			{
				id: 'adv-katai-somewhere',
				dev: 'कतै छैन',
				en: "It's nowhere",
				usage: 'कतै = somewhere; with a negative = nowhere / anywhere.',
			},
			{
				id: 'adv-tyahin-rightthere',
				dev: 'त्यहीँ छ',
				en: "It's right there",
				usage: 'त्यहीँ = right there / in that very place.',
			},
		],
	},
	{
		id: 'adv-linking',
		title: 'Linking & Certainty',
		kind: 'phrases',
		goal: 'Connect ideas — therefore, instead, only, especially, almost, for sure',
		items: [
			{
				id: 'adv-tyasaile-therefore',
				dev: 'त्यसैले गाह्रो छ',
				en: "That's why it's hard",
				usage: "त्यसैले = therefore / that's why.",
			},
			{
				id: 'adv-baru-instead',
				dev: 'बरु यो लिनुस्',
				en: 'Take this instead',
				usage: 'बरु = rather / instead / better. लिनुस् = take.',
			},
			{
				id: 'adv-matrai-only',
				dev: 'यो मात्रै',
				en: 'Only this',
				usage: 'मात्रै = only / just.',
			},
			{
				id: 'adv-khasgari-especially',
				dev: 'खासगरी बिहान',
				en: 'Especially in the morning',
				usage: 'खासगरी = especially / particularly. बिहान = morning.',
			},
			{
				id: 'adv-jastai-like',
				dev: 'यो जस्तै',
				en: 'Like this one',
				usage: 'जस्तै = like / similar to / such as.',
			},
			{
				id: 'adv-tyastai-likewise',
				dev: 'त्यस्तै हो',
				en: "It's about the same",
				usage: 'त्यस्तै = like that / similar / roughly so.',
			},
			{
				id: 'adv-karib-approximately',
				dev: 'करिब सय',
				en: 'About a hundred',
				usage: 'करिब = approximately / about. सय = hundred.',
			},
			{
				id: 'adv-jhandai-almost',
				dev: 'झण्डै एक घण्टा',
				en: 'Almost an hour',
				usage: 'झण्डै = almost / nearly. घण्टा = hour.',
			},
			{
				id: 'adv-pakkai-surely',
				dev: 'पक्कै आउँछु',
				en: "I'll definitely come",
				usage: 'पक्कै = certainly / surely / for sure.',
			},
		],
	},
	// --- T11 batch 5: essential function words (frames), appended as an advanced tier at the end of the path. Titles/goals AI-drafted.
	{
		id: 'fn-pronouns',
		title: 'Pronouns & Self',
		kind: 'phrases',
		goal: "Point precisely — one's own, oneself, any, someone, which, the same",
		items: [
			{
				id: 'fn-aaphno-ones-own',
				dev: 'आफ्नो घर',
				en: "One's own home",
				usage: "आफ्नो = one's own (reflexive possessive — 'my/your/his own'). आफ्नो घर = 'one's own house'.",
			},
			{
				id: 'fn-aaphain-oneself',
				dev: 'म आफैं गर्छु',
				en: "I'll do it myself",
				usage: "आफैं = oneself / by oneself (emphatic). म आफैं = 'I myself'.",
			},
			{
				id: 'fn-kunai-any',
				dev: 'कुनै समस्या छैन',
				en: "There's no problem",
				usage: "कुनै = any / some (before a noun). कुनै … छैन = 'not any / no'.",
			},
			{
				id: 'fn-kohi-someone',
				dev: 'कोही छ?',
				en: 'Is anyone there?',
				usage: 'कोही = someone / anyone (of people). With a negative = no one.',
			},
			{
				id: 'fn-jun-whichthat',
				dev: 'जुन राम्रो छ',
				en: "The one that's good",
				usage: "जुन = which / the one that (relative pronoun): जुन … त्यो = 'whichever … that'.",
			},
			{
				id: 'fn-uhi-samesone',
				dev: 'उही मान्छे',
				en: 'The same person',
				usage: 'उही = the very same / that same one.',
			},
		],
	},
	{
		id: 'fn-conjunctions',
		title: 'If, When & Because',
		kind: 'phrases',
		goal: 'Join clauses — if, when, because, otherwise',
		items: [
			{
				id: 'fn-yadi-if',
				dev: 'यदि सम्भव छ भने',
				en: "If it's possible",
				usage: 'यदि = if — usually paired with भने at the end of the clause. सम्भव = possible.',
			},
			{
				id: 'fn-jaba-when',
				dev: 'जब समय हुन्छ',
				en: "When there's time",
				usage: "जब = when (at the time that): जब … तब = 'when … then'.",
			},
			{
				id: 'fn-kinabhane-because',
				dev: 'किनभने गाह्रो छ',
				en: "Because it's hard",
				usage: 'किनभने = because (gives a reason), usually after the main clause.',
			},
			{
				id: 'fn-natra-otherwise',
				dev: 'नत्र गाह्रो हुन्छ',
				en: "Otherwise it'll be hard",
				usage: 'नत्र = otherwise / or else.',
			},
		],
	},
	{
		id: 'fn-counting',
		title: 'Counting Things',
		kind: 'phrases',
		goal: 'Count with classifiers — how many, two items, one person, and a half',
		items: [
			{
				id: 'fn-wataa-counter',
				dev: 'कति वटा?',
				en: 'How many?',
				usage: "वटा = the counter for things/objects (किताब दुई वटा = 'two books'). कति वटा? = 'how many?'.",
			},
			{
				id: 'fn-duiwataa-twoitems',
				dev: 'दुईवटा कलम',
				en: 'Two pens',
				usage: 'दुईवटा = two (of things) = दुई + वटा. कलम = pen.',
			},
			{
				id: 'fn-tinwataa-threeitems',
				dev: 'तीनवटा आँप',
				en: 'Three mangoes',
				usage: 'तीनवटा = three (of things) = तीन + वटा.',
			},
			{
				id: 'fn-ekjana-oneperson',
				dev: 'एकजना मान्छे',
				en: 'One person',
				usage: "एकजना = one (person). जना is the counter for people (दुई जना = 'two people').",
			},
			{
				id: 'fn-saadhe-andahalf',
				dev: 'साढे तीन बजे',
				en: 'At half past three',
				usage: "साढे = and a half (for time and quantity): साढे तीन = 'three thirty / three and a half'.",
			},
			{
				id: 'fn-dedh-onehalf',
				dev: 'डेढ घण्टा',
				en: 'An hour and a half',
				usage: 'डेढ = one and a half (a special word for 1.5). घण्टा = hour.',
			},
		],
	},
	{
		id: 'fn-big-numbers',
		title: 'Big Numbers',
		kind: 'phrases',
		goal: 'Say big numbers — fifteen, a lakh, a crore, hundreds',
		items: [
			{
				id: 'fn-pandhra-fifteen',
				dev: 'पन्ध्र मिनेट',
				en: 'Fifteen minutes',
				usage: 'पन्ध्र = fifteen (15). मिनेट = minute.',
			},
			{
				id: 'fn-laakh-lakh',
				dev: 'एक लाख',
				en: 'One hundred thousand',
				usage: 'लाख = 100,000 (a lakh) — the South Asian unit above हजार (thousand).',
			},
			{
				id: 'fn-karod-crore',
				dev: 'एक करोड',
				en: 'Ten million',
				usage: 'करोड = 10,000,000 (a crore) — the unit above लाख.',
			},
			{
				id: 'fn-arba-billion',
				dev: 'एक अर्ब',
				en: 'One billion',
				usage: 'अर्ब = one billion (1,000,000,000) — the unit above करोड.',
			},
			{
				id: 'fn-sayaun-hundreds',
				dev: 'सयौं मान्छे',
				en: 'Hundreds of people',
				usage: "सयौं = hundreds (of). Likewise हजारौं = 'thousands'.",
			},
		],
	},
	{
		id: 'fn-little-words',
		title: 'Little Connecting Words',
		kind: 'phrases',
		goal: 'Small but essential — from, except, like, each, every',
		items: [
			{
				id: 'fn-dekhi-from',
				dev: 'आजदेखि',
				en: 'From today',
				usage: "देखि = from / since (a starting point): यहाँदेखि 'from here', आजदेखि 'from today'.",
			},
			{
				id: 'fn-baahek-except',
				dev: 'यो बाहेक',
				en: 'Except this',
				usage: 'बाहेक = except / besides / apart from.',
			},
			{
				id: 'fn-jhain-likeas',
				dev: 'पानी झैं',
				en: 'Like water',
				usage: "झैं = like / as if (a comparison): बच्चा झैं 'like a child'.",
			},
			{
				id: 'fn-chahin-asfor',
				dev: 'म चाहिँ जान्छु',
				en: "As for me, I'll go",
				usage: "चाहिँ = as for / the (a soft topic/contrast marker): यो चाहिँ राम्रो छ 'this one's good'.",
			},
			{
				id: 'fn-harek-each',
				dev: 'हरेक दिन',
				en: 'Every day',
				usage: "हरेक = each / every. हरेक दिन = 'every day'.",
			},
			{
				id: 'fn-pratyek-every',
				dev: 'प्रत्येक व्यक्ति',
				en: 'Each person',
				usage: 'प्रत्येक = each / every (a touch more formal than हरेक). व्यक्ति = person.',
			},
		],
	},
	// --- T11 batch 6: everyday-life nouns, part 2 (frames), appended as an advanced tier at the end of the path. Titles/goals AI-drafted.
	{
		id: 'noun2-travel',
		title: 'Travel & Transport',
		kind: 'phrases',
		goal: 'Get around — vehicles, tickets, the driver, a journey, a flight',
		items: [
			{
				id: 'noun2-sawari-vehicle',
				dev: 'सवारी रोक्नुस्',
				en: 'Stop the vehicle',
				usage: 'सवारी = vehicle. रोक्नुस् = stop.',
			},
			{
				id: 'noun2-yatayat-transport',
				dev: 'यातायात बन्द छ',
				en: 'Transport is shut down',
				usage: 'यातायात = transport / transportation. बन्द = closed / shut.',
			},
			{
				id: 'noun2-motorcycle',
				dev: 'नयाँ मोटरसाइकल',
				en: 'A new motorcycle',
				usage: 'मोटरसाइकल = motorcycle (loanword). नयाँ = new.',
			},
			{
				id: 'noun2-tikat-ticket',
				dev: 'बसको टिकट',
				en: 'A bus ticket',
				usage: 'टिकट = ticket (loanword). बसको = of the bus.',
			},
			{
				id: 'noun2-pul-bridge',
				dev: 'ठूलो पुल',
				en: 'A big bridge',
				usage: 'पुल = bridge.',
			},
			{
				id: 'noun2-chalak-driver',
				dev: 'बस चालक',
				en: 'Bus driver',
				usage: 'चालक = driver. बस = bus.',
			},
			{
				id: 'noun2-traffic',
				dev: 'ट्राफिक जाम',
				en: 'Traffic jam',
				usage: 'ट्राफिक = traffic (loanword). जाम = jam / blockage.',
			},
			{
				id: 'noun2-yatra-journey',
				dev: 'राम्रो यात्रा',
				en: 'A good journey',
				usage: "यात्रा = journey / trip / travel. शुभ यात्रा = 'bon voyage'.",
			},
			{
				id: 'noun2-udaan-flight',
				dev: 'बिहानको उडान',
				en: 'The morning flight',
				usage: 'उडान = flight. बिहानको = of the morning.',
			},
		],
	},
	{
		id: 'noun2-city',
		title: 'City & Country',
		kind: 'phrases',
		goal: 'Places on the map — village, city, capital, temple, border, mountain',
		items: [
			{
				id: 'noun2-gaun-village',
				dev: 'मेरो गाउँ',
				en: 'My village',
				usage: 'गाउँ = village.',
			},
			{
				id: 'noun2-sahar-city',
				dev: 'ठूलो सहर',
				en: 'A big city',
				usage: 'सहर = city / town (also written शहर).',
			},
			{
				id: 'noun2-rajdhani-capital',
				dev: 'नेपालको राजधानी',
				en: "Nepal's capital",
				usage: 'राजधानी = capital city. नेपालको = of Nepal.',
			},
			{
				id: 'noun2-mandir-temple',
				dev: 'पुरानो मन्दिर',
				en: 'An old temple',
				usage: 'मन्दिर = temple. पुरानो = old.',
			},
			{
				id: 'noun2-videsh-abroad',
				dev: 'विदेश जानु',
				en: 'To go abroad',
				usage: 'विदेश = abroad / a foreign country. जानु = to go.',
			},
			{
				id: 'noun2-sima-border',
				dev: 'देशको सीमा',
				en: "The country's border",
				usage: 'सीमा = border / boundary / limit. देशको = of the country.',
			},
			{
				id: 'noun2-himal-mountain',
				dev: 'हिमाल राम्रो छ',
				en: 'The mountains are beautiful',
				usage: 'हिमाल = (snow) mountain; the Himalaya.',
			},
			{
				id: 'noun2-maidan-field',
				dev: 'फुटबल मैदान',
				en: 'A football field',
				usage: 'मैदान = field / ground / open plain.',
			},
			{
				id: 'noun2-basti-settlement',
				dev: 'सानो बस्ती',
				en: 'A small settlement',
				usage: 'बस्ती = settlement / neighborhood. सानो = small.',
			},
		],
	},
	{
		id: 'noun2-money',
		title: 'Money & Commerce',
		kind: 'phrases',
		goal: 'Handle money — a purchase, cash, rent, a loan, income, the total',
		items: [
			{
				id: 'noun2-kharid-purchase',
				dev: 'खरिद गर्नु',
				en: 'To make a purchase',
				usage: "खरिद = purchase. खरिद गर्नु = 'to buy'.",
			},
			{
				id: 'noun2-nagad-cash',
				dev: 'नगद छैन',
				en: 'No cash',
				usage: "नगद = cash. छैन = there isn't.",
			},
			{
				id: 'noun2-bhada-rent',
				dev: 'घरको भाडा',
				en: 'House rent',
				usage: 'भाडा = rent / fare. घरको = of the house.',
			},
			{
				id: 'noun2-rin-loan',
				dev: 'ऋण लिनु',
				en: 'To take a loan',
				usage: 'ऋण = loan / debt. लिनु = to take.',
			},
			{
				id: 'noun2-aamdani-income',
				dev: 'राम्रो आम्दानी',
				en: 'A good income',
				usage: 'आम्दानी = income / earnings.',
			},
			{
				id: 'noun2-phaida-benefit',
				dev: 'के फाइदा?',
				en: "What's the use?",
				usage: 'फाइदा = benefit / profit / advantage.',
			},
			{
				id: 'noun2-hisab-account',
				dev: 'हिसाब गर्नु',
				en: 'To settle the bill',
				usage: "हिसाब = account / calculation / bill. हिसाब गर्नु = 'to do the maths / settle up'.",
			},
			{
				id: 'noun2-jamma-total',
				dev: 'जम्मा कति?',
				en: 'How much in total?',
				usage: 'जम्मा = total / altogether / in all.',
			},
			{
				id: 'noun2-karobar-business',
				dev: 'राम्रो कारोबार',
				en: 'Good business',
				usage: 'कारोबार = business / dealings / transactions.',
			},
		],
	},
	{
		id: 'noun2-school',
		title: 'School & Mind',
		kind: 'phrases',
		goal: 'Learning & ideas — exam, class, language, knowledge, an idea, meaning',
		items: [
			{
				id: 'noun2-pariksha-exam',
				dev: 'परीक्षा दिनु',
				en: 'To take an exam',
				usage: "परीक्षा = exam / test. परीक्षा दिनु = 'to sit an exam' (literally 'give an exam').",
			},
			{
				id: 'noun2-kaksha-class',
				dev: 'पाँचौं कक्षा',
				en: 'Fifth grade',
				usage: 'कक्षा = class / grade / classroom. पाँचौं = fifth.',
			},
			{
				id: 'noun2-bhasha-language',
				dev: 'नेपाली भाषा',
				en: 'The Nepali language',
				usage: 'भाषा = language.',
			},
			{
				id: 'noun2-adhyayan-study',
				dev: 'अध्ययन गर्नु',
				en: 'To study',
				usage: "अध्ययन = study / studies. अध्ययन गर्नु = 'to study'.",
			},
			{
				id: 'noun2-anubhav-experience',
				dev: 'नयाँ अनुभव',
				en: 'A new experience',
				usage: 'अनुभव = experience.',
			},
			{
				id: 'noun2-gyan-knowledge',
				dev: 'धेरै ज्ञान',
				en: 'A lot of knowledge',
				usage: "ज्ञान = knowledge (ज्ञ is pronounced 'gy'). धेरै = a lot.",
			},
			{
				id: 'noun2-vichar-idea',
				dev: 'राम्रो विचार',
				en: 'A good idea',
				usage: 'विचार = thought / idea / opinion.',
			},
			{
				id: 'noun2-artha-meaning',
				dev: 'यसको अर्थ के हो?',
				en: 'What does this mean?',
				usage: "अर्थ = meaning. यसको अर्थ = 'the meaning of this'.",
			},
			{
				id: 'noun2-sapana-dream',
				dev: 'राम्रो सपना',
				en: 'A good dream',
				usage: 'सपना = dream (both while sleeping and an ambition).',
			},
		],
	},
	{
		id: 'noun2-health',
		title: 'Health & Life',
		kind: 'phrases',
		goal: 'Body & wellbeing — the body, health, age, disease, a vaccine, a solution',
		items: [
			{
				id: 'noun2-sharir-body',
				dev: 'मेरो शरीर',
				en: 'My body',
				usage: 'शरीर = body (the whole body; individual parts are in Body Parts).',
			},
			{
				id: 'noun2-swasthya-health',
				dev: 'राम्रो स्वास्थ्य',
				en: 'Good health',
				usage: 'स्वास्थ्य = health.',
			},
			{
				id: 'noun2-umer-age',
				dev: 'तपाईंको उमेर कति?',
				en: 'How old are you?',
				usage: "उमेर = age. तपाईंको उमेर कति? = 'what's your age?'.",
			},
			{
				id: 'noun2-rog-disease',
				dev: 'रोगको उपचार',
				en: 'Treatment for the disease',
				usage: 'रोग = disease / illness. उपचार = treatment.',
			},
			{
				id: 'noun2-khop-vaccine',
				dev: 'खोप लगाउनु',
				en: 'To get vaccinated',
				usage: "खोप = vaccine / vaccination. खोप लगाउनु = 'to get a vaccine'.",
			},
			{
				id: 'noun2-durghatana-accident',
				dev: 'ठूलो दुर्घटना',
				en: 'A big accident',
				usage: 'दुर्घटना = accident.',
			},
			{
				id: 'noun2-jokhim-risk',
				dev: 'ठूलो जोखिम',
				en: 'A big risk',
				usage: 'जोखिम = risk / hazard.',
			},
			{
				id: 'noun2-samadhan-solution',
				dev: 'समस्याको समाधान',
				en: 'The solution to the problem',
				usage: 'समाधान = solution. समस्या = problem.',
			},
			{
				id: 'noun2-sudhar-improvement',
				dev: 'ठूलो सुधार',
				en: 'A big improvement',
				usage: 'सुधार = improvement / reform.',
			},
		],
	},
	// --- T11 batch 7: Feelings & Everyday Things (frames), appended as an advanced tier at the end of the path. Hand-curated themed pocket; titles/goals AI-drafted.
	{
		id: 'feelings-states',
		title: 'Feelings & States',
		kind: 'phrases',
		goal: 'Say how you feel — shy, sad, happy, proud, regretful, hungry, thirsty, tired, alone',
		items: [
			{
				id: 'feel-laaj-shame',
				dev: 'मलाई लाज लाग्यो',
				en: 'I felt shy.',
				usage: "लाज = shame / shyness. मलाई लाज लाग्यो = 'I felt shy/embarrassed'.",
			},
			{
				id: 'feel-dukha-sadness',
				dev: 'मलाई दुःख लाग्यो',
				en: 'I felt sad.',
				usage: "दुःख = sadness / sorrow (also hardship). दुःख लाग्नु = 'to feel sad'.",
			},
			{
				id: 'feel-khusi-happy',
				dev: 'म धेरै खुशी छु',
				en: 'I am very happy.',
				usage: 'खुशी = happy / happiness.',
			},
			{
				id: 'feel-ghamanda-arrogance',
				dev: 'उसलाई घमन्ड छ',
				en: 'He is arrogant.',
				usage: 'घमन्ड = arrogance / conceit (negative pride).',
			},
			{
				id: 'feel-pachhuto-regret',
				dev: 'मलाई पछुतो भयो',
				en: 'I regretted it.',
				usage: "पछुतो = regret. पछुतो हुनु = 'to regret'.",
			},
			{
				id: 'feel-bhok-hunger',
				dev: 'मलाई भोक लाग्यो',
				en: "I'm hungry.",
				usage: "भोक = hunger. भोक लाग्नु = 'to feel hungry'.",
			},
			{
				id: 'feel-tirkha-thirst',
				dev: 'मलाई तिर्खा लाग्यो',
				en: "I'm thirsty.",
				usage: "तिर्खा = thirst. तिर्खा लाग्नु = 'to feel thirsty'.",
			},
			{
				id: 'feel-thakai-tired',
				dev: 'मलाई थकाइ लाग्यो',
				en: 'I feel tired.',
				usage: "थकाइ = tiredness / fatigue. थकाइ लाग्नु = 'to feel tired'.",
			},
			{
				id: 'feel-eklo-lonely',
				dev: 'म एक्लो छु',
				en: 'I am alone.',
				usage: 'एक्लो = alone / lonely.',
			},
		],
	},
	{
		id: 'body-parts-more',
		title: 'More Body Parts',
		kind: 'phrases',
		goal: 'More of the body — tongue, cheek, forehead, knee, nail, skin, bone, blood',
		items: [
			{
				id: 'body-jibro-tongue',
				dev: 'मेरो जिब्रो',
				en: 'My tongue',
				usage: 'जिब्रो = tongue.',
			},
			{
				id: 'body-gaalaa-cheek',
				dev: 'रातो गाला',
				en: 'Red cheeks',
				usage: 'गाला = cheek.',
			},
			{
				id: 'body-nidhaar-forehead',
				dev: 'फराकिलो निधार',
				en: 'A broad forehead',
				usage: 'निधार = forehead. फराकिलो = broad / wide.',
			},
			{
				id: 'body-ghunda-knee',
				dev: 'मेरो घुँडा दुख्यो',
				en: 'My knee hurts.',
				usage: "घुँडा = knee. दुख्नु = 'to hurt / ache'.",
			},
			{
				id: 'body-nang-nail',
				dev: 'नङ काट्नु',
				en: 'To cut nails',
				usage: "नङ = fingernail / toenail. काट्नु = 'to cut'.",
			},
			{
				id: 'body-chhaalaa-skin',
				dev: 'नरम छाला',
				en: 'Soft skin',
				usage: 'छाला = skin (also leather / hide).',
			},
			{
				id: 'body-haddi-bone',
				dev: 'हड्डी भाँचियो',
				en: 'The bone broke.',
				usage: "हड्डी = bone. भाँचिनु = 'to break (snap)'.",
			},
			{
				id: 'body-ragat-blood',
				dev: 'रातो रगत',
				en: 'Red blood',
				usage: 'रगत = blood.',
			},
		],
	},
	{
		id: 'clothes-accessories',
		title: 'Clothes & Accessories',
		kind: 'phrases',
		goal: 'What you wear — socks, a sari, a kurta, a jacket, a sweater, glasses, a scarf',
		items: [
			{
				id: 'wear-mojaa-socks',
				dev: 'नयाँ मोजा',
				en: 'New socks',
				usage: 'मोजा = socks.',
			},
			{
				id: 'wear-saadi-sari',
				dev: 'रातो साडी',
				en: 'A red sari',
				usage: 'साडी = sari (a wrapped garment worn by women).',
			},
			{
				id: 'wear-kurtaa-kurta',
				dev: 'सुती कुर्ता',
				en: 'A cotton kurta',
				usage: 'कुर्ता = kurta (a long tunic). सुती = cotton.',
			},
			{
				id: 'wear-jyaaket-jacket',
				dev: 'न्यानो ज्याकेट',
				en: 'A warm jacket',
				usage: 'ज्याकेट = jacket. न्यानो = warm.',
			},
			{
				id: 'wear-switar-sweater',
				dev: 'बाक्लो स्विटर',
				en: 'A thick sweater',
				usage: 'स्विटर = sweater. बाक्लो = thick.',
			},
			{
				id: 'wear-chasmaa-glasses',
				dev: 'कालो चस्मा',
				en: 'Black glasses',
				usage: 'चस्मा = glasses / spectacles (also sunglasses). कालो = black.',
			},
			{
				id: 'wear-rumaal-handkerchief',
				dev: 'सफा रुमाल',
				en: 'A clean handkerchief',
				usage: 'रुमाल = handkerchief. सफा = clean.',
			},
			{
				id: 'wear-gamchhaa-scarf',
				dev: 'पातलो गम्छा',
				en: 'A thin cotton scarf',
				usage: 'गम्छा = a thin cotton towel-scarf, commonly worn over the shoulder. पातलो = thin.',
			},
		],
	},
	{
		id: 'groceries-more',
		title: 'More Groceries',
		kind: 'phrases',
		goal: 'At the market — rice, eggs, fruit, ghee, mutton, greens, a tomato, legumes',
		items: [
			{
				id: 'food2-chaamal-rice',
				dev: 'एक किलो चामल',
				en: 'One kilo of rice',
				usage: 'चामल = uncooked rice (cooked rice is भात). किलो = kilogram.',
			},
			{
				id: 'food2-andaa-egg',
				dev: 'उमालेको अन्डा',
				en: 'A boiled egg',
				usage: "अन्डा = egg. उमाल्नु = 'to boil'.",
			},
			{
				id: 'food2-phalphul-fruit',
				dev: 'ताजा फलफूल',
				en: 'Fresh fruit',
				usage: 'फलफूल = fruit (collectively). ताजा = fresh.',
			},
			{
				id: 'food2-ghiu-ghee',
				dev: 'भातमा घिउ',
				en: 'Ghee on rice',
				usage: 'घिउ = ghee / clarified butter.',
			},
			{
				id: 'food2-khasi-mutton',
				dev: 'खसीको मासु',
				en: 'Goat meat (mutton)',
				usage: 'खसी = (castrated) goat; खसीको मासु = mutton, the everyday festive meat.',
			},
			{
				id: 'food2-saagpaat-greens',
				dev: 'हरियो सागपात',
				en: 'Green leafy vegetables',
				usage: 'सागपात = leafy greens. हरियो = green.',
			},
			{
				id: 'food2-golbheda-tomato',
				dev: 'पाकेको गोलभेँडा',
				en: 'A ripe tomato',
				usage: "गोलभेँडा = tomato. पाक्नु = 'to ripen / cook'.",
			},
			{
				id: 'food2-gedagudi-legumes',
				dev: 'गेडागुडी खानु',
				en: 'To eat legumes',
				usage: "गेडागुडी = beans & pulses (legumes). खानु = 'to eat'.",
			},
		],
	},
	// --- T11 batch 8: Calendar, Festivals & Directions (frames), appended as an advanced tier at the end of the path. Hand-curated themed pocket; titles/goals AI-drafted.
	{
		id: 'nepali-calendar',
		title: 'Nepali Calendar',
		kind: 'phrases',
		goal: 'The 12 months of the Bikram Sambat year — Baisakh through Chait',
		items: [
			{
				id: 'cal-baishakh',
				dev: 'बैशाख महिना',
				en: 'The month of Baisakh',
				usage: 'बैशाख = Baisakh, the 1st month of the Nepali (Bikram Sambat) year — mid-April to mid-May; the new year starts on 1 Baisakh.',
			},
			{
				id: 'cal-jeth',
				dev: 'जेठ महिना',
				en: 'The month of Jeth',
				usage: "जेठ = Jeth, the 2nd month (May–June); the hot pre-monsoon season. (जेठ also means a husband's elder brother.)",
			},
			{
				id: 'cal-asar',
				dev: 'असार महिना',
				en: 'The month of Asar',
				usage: 'असार = Asar, the 3rd month (June–July); rice-planting time (असार १५ is national paddy day).',
			},
			{
				id: 'cal-saun',
				dev: 'साउन महिना',
				en: 'The month of Saun',
				usage: 'साउन = Saun, the 4th month (July–August); peak monsoon rains.',
			},
			{
				id: 'cal-bhadau',
				dev: 'भदौ महिना',
				en: 'The month of Bhadau',
				usage: 'भदौ = Bhadau, the 5th month (August–September).',
			},
			{
				id: 'cal-asoj',
				dev: 'असोज महिना',
				en: 'The month of Asoj',
				usage: 'असोज = Asoj, the 6th month (September–October); Dashain usually falls here.',
			},
			{
				id: 'cal-kattik',
				dev: 'कात्तिक महिना',
				en: 'The month of Kattik',
				usage: 'कात्तिक = Kattik, the 7th month (October–November); Tihar falls here.',
			},
			{
				id: 'cal-mangsir',
				dev: 'मंसिर महिना',
				en: 'The month of Mangsir',
				usage: 'मंसिर = Mangsir, the 8th month (November–December); the main wedding season.',
			},
			{
				id: 'cal-pus',
				dev: 'पुस महिना',
				en: 'The month of Pus',
				usage: 'पुस = Pus, the 9th month (December–January); the coldest part of winter.',
			},
			{
				id: 'cal-magh',
				dev: 'माघ महिना',
				en: 'The month of Magh',
				usage: 'माघ = Magh, the 10th month (January–February); opens with Maghe Sankranti.',
			},
			{
				id: 'cal-phagun',
				dev: 'फागुन महिना',
				en: 'The month of Phagun',
				usage: 'फागुन = Phagun, the 11th month (February–March); Holi falls here.',
			},
			{
				id: 'cal-chait',
				dev: 'चैत महिना',
				en: 'The month of Chait',
				usage: 'चैत = Chait, the 12th and final month (March–April).',
			},
		],
	},
	{
		id: 'festivals',
		title: 'Festivals & Celebrations',
		kind: 'phrases',
		goal: 'Celebrate — Dashain, Tihar, Teej, Holi, a jatra, a fair, a festival, worship',
		items: [
			{
				id: 'fest-dashain',
				dev: 'दशैं आयो',
				en: 'Dashain has come',
				usage: "दशैं = Dashain, the biggest festival in Nepal (about 15 days, in Asoj/Kattik). आयो = 'came / has arrived'.",
			},
			{
				id: 'fest-tihar',
				dev: 'तिहारको बत्ती',
				en: 'Tihar lights',
				usage: 'तिहार = Tihar, the five-day festival of lights (right after Dashain). बत्ती = light / lamp.',
			},
			{
				id: 'fest-teej',
				dev: 'तीजको नाच',
				en: 'The Teej dance',
				usage: "तीज = Teej, a women's festival (in Bhadau) with fasting, red saris, and dancing. नाच = dance.",
			},
			{
				id: 'fest-holi',
				dev: 'रङको होली',
				en: 'The Holi of colors',
				usage: 'होली = Holi, the spring festival of colors (in Phagun). रङ = color.',
			},
			{
				id: 'fest-jatra',
				dev: 'जात्रा हेर्नु',
				en: 'To watch the jatra',
				usage: "जात्रा = a jatra — a street / chariot festival (e.g. Indra Jatra). हेर्नु = 'to watch'.",
			},
			{
				id: 'fest-mela',
				dev: 'मेलामा जानु',
				en: 'To go to the fair',
				usage: "मेला = a fair / gathering (often at a temple or on a festival day). मेलामा जानु = 'to go to the fair'.",
			},
			{
				id: 'fest-parva',
				dev: 'ठूलो पर्व',
				en: 'A big festival',
				usage: 'पर्व = festival / celebration (a synonym of चाड; often paired as चाडपर्व). ठूलो = big.',
			},
			{
				id: 'fest-puja',
				dev: 'पूजा गर्नु',
				en: 'To worship',
				usage: "पूजा = worship / prayer (a ritual offering). पूजा गर्नु = 'to worship / pray'.",
			},
		],
	},
	{
		id: 'directions',
		title: 'Directions & Places',
		kind: 'phrases',
		goal: 'Find your way — north, south, east, west, direction, a map, around',
		items: [
			{
				id: 'dir-uttar',
				dev: 'उत्तरतिर',
				en: 'Toward the north',
				usage: 'उत्तर = north. -तिर = toward / in the direction of.',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'हिमाल उत्तरतिर छ', en: 'The Himalaya is to the north' },
					{ dev: 'बाटो उत्तरतिर जान्छ', en: 'The road goes north' },
				],
			},
			{
				id: 'dir-dakshin',
				dev: 'दक्षिणतिर',
				en: 'Toward the south',
				usage: 'दक्षिण = south.',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'भारत दक्षिणतिर छ', en: 'India is to the south' },
					{ dev: 'दक्षिणतिर जानुस्', en: 'Head south' },
				],
			},
			{
				id: 'dir-purba',
				dev: 'पूर्वतिर',
				en: 'Toward the east',
				usage: "पूर्व = east. (पूर्व also means 'former / previous'.)",
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'पूर्वतिर हेर्नुस्', en: 'Look east' },
					{ dev: 'घाम बिहान पूर्वतिर हुन्छ', en: 'The sun is in the east in the morning' },
				],
			},
			{
				id: 'dir-paschim',
				dev: 'पश्चिमतिर',
				en: 'Toward the west',
				usage: 'पश्चिम = west.',
				// T29 batch 15 (places, positions & everyday replies; AI-drafted, approved):
				frames: [
					{ dev: 'पोखरा पश्चिमतिर छ', en: 'Pokhara is to the west' },
					{ dev: 'बेलुका घाम पश्चिमतिर हुन्छ', en: 'The evening sun is in the west' },
				],
			},
			{
				id: 'dir-disha',
				dev: 'कुन दिशा?',
				en: 'Which direction?',
				usage: 'दिशा = direction. कुन = which.',
			},
			{
				id: 'dir-naksa',
				dev: 'नेपालको नक्सा',
				en: 'A map of Nepal',
				usage: "नक्सा = map. नेपालको = 'of Nepal'.",
			},
			{
				id: 'dir-waripari',
				dev: 'घर वरिपरि',
				en: 'Around the house',
				usage: 'वरिपरि = around / all around. घर = house.',
			},
		],
	},
	// --- T11 batch 9 (final breadth batch): Linking Words, More Everyday Verbs & Odds & Ends (frames), appended as an advanced tier at the end of the path. Hand-curated highest-value gaps; titles/goals AI-drafted.
	{
		id: 'linking-words',
		title: 'Linking Words',
		kind: 'phrases',
		goal: 'Glue for real sentences — maybe, of course, at least, a little, else, where, the way, as long as, like this',
		items: [
			{
				id: 'link-shaayad',
				dev: 'शायद पानी पर्छ',
				en: 'Maybe it will rain',
				usage: '"Maybe / perhaps" — a hedge at the front of a sentence when you are not certain. Often paired with "hola" for extra softening.',
			},
			{
				id: 'link-taipani',
				dev: 'गाह्रो छ, तैपनि गर्छु',
				en: "It's hard, but I'll still do it",
				usage: '"Even so / still / nevertheless." Links a concession to what you do anyway — "X, taipani Y" = "X, but even so Y."',
			},
			{
				id: 'link-avashya',
				dev: 'अवश्य, म आउँछु',
				en: "Of course, I'll come",
				usage: '"Certainly / of course." A confident yes — a touch more formal than "pakka" but common in polite speech.',
			},
			{
				id: 'link-alikati',
				dev: 'अलिकति पानी दिनुस्',
				en: 'Please give a little water',
				usage: '"A little / a bit" — a small amount. "Alikati" softens a request; "thorai" is the near-synonym you already know.',
			},
			{
				id: 'link-jahaan',
				dev: 'जहाँ मन लाग्छ, त्यहाँ जानुस्',
				en: 'Go wherever you like',
				usage: '"Where" as a linker (not the question "kahaan"). Pairs with "tyahaan" — "jahaan … tyahaan" = "where … there."',
			},
			{
				id: 'link-jasari',
				dev: 'जसरी म गर्छु, त्यसरी गर्नुस्',
				en: 'Do it the way I do',
				usage: '"The way that / how" — the relative of "kasari" (how?). Pairs with "tyasari."',
			},
			{
				id: 'link-jabasamma',
				dev: 'जबसम्म पानी पर्छ, म बस्छु',
				en: "As long as it rains, I'll stay",
				usage: '"As long as / until." "Jabasamma … tabasamma" frames a stretch of time — "as long as X, Y."',
			},
			{
				id: 'link-jasto',
				dev: 'यो जस्तो अर्को छ?',
				en: 'Is there another one like this?',
				usage: '"Like / such as." "Yo jasto" = "like this one" — the fuller form of "jastai," tacked onto the end of a word.',
			},
		],
	},
	{
		id: 'everyday-verbs-more',
		title: 'More Everyday Verbs',
		kind: 'phrases',
		goal: 'Everyday actions — ask, request, remember, forget, choose, return, teach, laugh, cry, dance, touch, throw',
		items: [
			{
				id: 'v9-maagnu',
				dev: 'म पानी माग्छु',
				en: 'I ask for water',
				usage: '"To ask for / request (a thing)" — different from "sodhnu" (to ask a question). "Paani maagnu" = to ask for water.',
				// Depth (T28): everyday-context alternate frames for the same verb, rotated into
				// reviews so the word isn't tied to one sentence. AI-drafted → Ross's review.
				frames: [
					{ dev: 'उसले पैसा माग्यो', en: 'He asked for money' },
					{ dev: 'मद्दत माग्नुस्', en: 'Please ask for help' },
				],
			},
			{
				id: 'v9-samjhanu',
				dev: 'म तिमीलाई सम्झन्छु',
				en: 'I remember you',
				usage: '"To remember / recall" — also "to miss someone." The opposite of "birsanu."',
				// Depth (T28): AI-drafted → Ross's review.
				frames: [
					{ dev: 'मलाई सम्झनुस्', en: 'Remember me' },
					{ dev: 'म घर सम्झन्छु', en: 'I miss home' },
				],
			},
			{
				id: 'v9-birsanu',
				dev: 'नाम बिर्सें',
				en: 'I forgot the name',
				usage: '"To forget." "Birsen" = "I forgot." The opposite of "samjhanu."',
			},
			{
				id: 'v9-rojnu',
				dev: 'एउटा रोज्नुस्',
				en: 'Please choose one',
				usage: '"To choose / pick / select." "Euta rojnus" = "pick one."',
			},
			{
				id: 'v9-padhaaunu',
				dev: 'उहाँले नेपाली पढाउनुहुन्छ',
				en: 'She teaches Nepali',
				usage: '"To teach" — the causative of "padhnu" (to study), literally "to make someone study." "Sikaaunu" is the near-synonym.',
			},
			{
				id: 'v9-haasnu',
				dev: 'मेरो साथी हाँस्यो',
				en: 'My friend laughed',
				usage: '"To laugh / smile." "Haasyo" = "(he/she) laughed."',
			},
			{
				id: 'v9-runu',
				dev: 'बच्चा रोयो',
				en: 'The child cried',
				usage: '"To cry / weep." "Royo" = "(he/she) cried." The stem shifts: run-u → ro-yo.',
			},
			{
				id: 'v9-naachnu',
				dev: 'हामी नाच्यौं',
				en: 'We danced',
				usage: '"To dance." "Naachyaun" = "we danced." Common at festivals and weddings.',
			},
			{
				id: 'v9-chhunu',
				dev: 'नछुनुस्',
				en: "Please don't touch",
				usage: '"To touch." "Na-chhunus" = "don\'t touch" — a useful sign or warning.',
			},
		],
	},
	{
		id: 'odds-ends',
		title: 'Odds & Ends',
		kind: 'phrases',
		goal: 'A closing handful — beside, this side, across, modern, shared',
		items: [
			{
				id: 'oe-chheu',
				dev: 'मेरो छेउमा बस्नुस्',
				en: 'Sit beside me',
				usage: '"Beside / next to / edge." "Mero chheu-maa" = "next to me" — a close, at-your-side near (vs. "najik," nearby).',
				// Depth (T28): AI-drafted → Ross's review.
				frames: [
					{ dev: 'ऊ मेरो छेउमा छ', en: 'He is beside me' },
					{ dev: 'बाटोको छेउमा', en: 'Beside the road' },
				],
			},
			{
				id: 'oe-waari',
				dev: 'नदीको वारि',
				en: 'This side of the river',
				usage: '"This side (the near bank)." Half of the classic pair "waari–paari" — the side you are on.',
			},
			{
				id: 'oe-paari',
				dev: 'नदीको पारि',
				en: 'Across the river',
				usage: '"The far side / across." The other half of "waari–paari." "Nadiiko paari" = "across the river."',
			},
			{
				id: 'oe-aadhunik',
				dev: 'यो आधुनिक सहर हो',
				en: 'This is a modern city',
				usage: '"Modern / up-to-date" — describes cities, gadgets, ideas. Opposite of "puraano" (old).',
			},
			{
				id: 'oe-saajhaa',
				dev: 'यो साझा कोठा हो',
				en: 'This is a shared room',
				usage: '"Shared / common / joint." "Saajhaa kothaa" = "shared room." Also the name of the old public bus service.',
			},
		],
	},
	{
		id: 'sounding-natural',
		title: 'Sounding Natural',
		kind: 'phrases',
		goal: 'React and fill gaps like a local',
		items: [
			{
				id: 'saanchai-really',
				dev: 'साँच्चै?',
				en: 'Really?!',
				usage: 'The back-channel that shows you\'re listening — drop it into any story to keep it going. "Saanchai" means "truly." Say it with a rising tone for surprise, a flat tone for "is that so."',
			},
			{
				id: 'chhodnus-never-mind',
				dev: 'छोड्नुस्',
				en: 'Never mind / forget it',
				usage: 'Politely wave something off — literally "release it," the polite command form of "chhodnu" (to leave/let go). Softer than it looks; use it to drop a topic or tell someone not to bother.',
			},
			{
				id: 'heraun-lets-see',
				dev: 'हेरौं',
				en: "Let's see",
				usage: 'Buys a beat while you think or decide. From "hernu" (to look), which you already know — this is its "let\'s" form. Handy when you\'re not ready to commit to an answer.',
			},
			{
				id: 'kaslai-thaha-who-knows',
				dev: 'कसलाई थाहा',
				en: 'Who knows',
				usage: 'A shrug in words — literally "to whom is it known?" Use it when you genuinely have no idea and want to say so lightly. Builds on "thaha" (knowledge) from "thaha chaina."',
			},
			{
				id: 'bhaihaalcha-itll-work-out',
				dev: 'भइहाल्छ',
				en: "It'll work out",
				usage: 'The reassuring optimist\'s phrase — "it\'ll happen / it\'ll be fine, don\'t worry." An emphatic form of "hunchha." A very Nepali way to wave away worry about something not yet settled.',
			},
			{
				id: 'shabaas-well-done',
				dev: 'शाबास',
				en: 'Well done!',
				usage: 'Cheer someone on — "bravo / nice work." Warm and common, said to kids and adults alike. Pair it with a smile; it always lands well.',
			},
			{
				id: 'kasto-ramro-how-nice',
				dev: 'कस्तो राम्रो!',
				en: 'How nice!',
				usage: 'An admiring exclamation — "how lovely / how wonderful." "Kasto" (how) + "raamro" (good/nice), both familiar. Use it about a view, a meal, a gift, good news.',
			},
			{
				id: 'ke-garne-oh-well',
				dev: 'के गर्ने',
				en: 'Oh well / what can you do',
				usage: 'The quintessential Nepali sigh of acceptance — literally "what to do?" Said when nothing can be done about a delay, the weather, a small misfortune. Learn it and you\'ll sound instantly local.',
			},
		],
	},
	{
		id: 'on-the-move',
		title: 'On the Move',
		kind: 'phrases',
		goal: 'Give and follow directions on the go',
		items: [
			{
				id: 'yahin-roknus-stop-here',
				dev: 'यहीँ रोक्नुस्',
				en: 'Stop here',
				usage: 'To a driver or a walking companion — "yahin" (right here) + "roknus" (please stop, polite command of "roknu"). The exact phrase for getting out of a taxi at the right spot.',
			},
			{
				id: 'kati-tadha-how-far',
				dev: 'कति टाढा छ?',
				en: 'How far is it?',
				usage: '"Kati" (how much) + "taadha" (far). Ask before you set off on foot. The answer often comes as a time: "das minet" (ten minutes) or just "najik" (near).',
			},
			{
				id: 'sidha-janus-go-straight',
				dev: 'सिधा जानुस्',
				en: 'Go straight',
				usage: 'Giving or repeating directions — "sidhaa" (straight) + "jaanus" (please go). The first of the three you need, alongside "turn left" and "turn right."',
			},
			{
				id: 'malai-parkhanus-wait-for-me',
				dev: 'मलाई पर्खनुस्',
				en: 'Wait for me',
				usage: '"Malai" (for me) + "parkhanus" (please wait). Call it out when you\'re a step behind — at a shop counter, a bus door, a trailhead.',
			},
			{
				id: 'baayan-janus-turn-left',
				dev: 'बायाँ जानुस्',
				en: 'Turn left',
				usage: '"Baayan" (left) + "jaanus" (please go) — literally "go left." To a driver, or repeating directions back to check you understood.',
			},
			{
				id: 'daayan-janus-turn-right',
				dev: 'दायाँ जानुस्',
				en: 'Turn right',
				usage: '"Daayan" (right) + "jaanus" (please go). The pair to "baayan jaanus" — learn them together so you never mix them up in the moment.',
			},
			{
				id: 'yo-bato-ho-is-this-the-way',
				dev: 'यो बाटो हो?',
				en: 'Is this the way?',
				usage: 'Checking you\'re on the right path — "yo" (this) + "baato" (road/way) + "ho?" (is it?). Point down a street and ask; a nod or "ho" confirms it.',
			},
			{
				id: 'najikai-cha-its-nearby',
				dev: 'नजिकै छ',
				en: "It's nearby",
				usage: 'The reassuring answer to "kati taadha cha?" — "najikai" (quite near) + "cha" (is). Also useful said back as a question: "najikai cha?" (is it close?).',
			},
		],
	},
	{
		id: 'being-a-guest',
		title: 'Being a Guest',
		kind: 'phrases',
		goal: 'Be a gracious guest at the table',
		items: [
			{
				id: 'ma-aghaen-im-full',
				dev: 'म अघाएँ',
				en: "I'm full",
				usage: 'The polite "stop serving me" — from "aghaunu" (to be satisfied/full). At a Nepali table the host keeps adding rice; this is how you gracefully signal you\'ve had enough.',
			},
			{
				id: 'alikati-matra-just-a-little',
				dev: 'अलिकति मात्र',
				en: 'Just a little',
				usage: 'Decline a big portion without refusing — "alikati" (a little) + "maatra" (only). Hold your hand over the plate and say it as the serving spoon approaches.',
			},
			{
				id: 'piro-nahalnus-no-spice',
				dev: 'पिरो नहाल्नुस्',
				en: "Please don't make it spicy",
				usage: '"Piro" (spicy-hot) + "nahaalnus" (please don\'t put in) — negative polite command of "haalnu" (to put). Say it before the cooking starts; Nepali "piro" can be serious.',
			},
			{
				id: 'khana-mitho-thiyo-food-was-delicious',
				dev: 'खाना मीठो थियो',
				en: 'The food was delicious',
				usage: 'The guest\'s gold-standard compliment, after the meal — "khana" (food) + "mitho" (tasty) + "thiyo" (was). The past-tense partner to "mitho cha." Always appreciated by the cook.',
			},
			{
				id: 'dherai-bhayo-thats-plenty',
				dev: 'धेरै भयो',
				en: "That's plenty",
				usage: 'Wave off an over-generous serving — "dherai" (a lot) + "bhayo" (it\'s become / that\'s it). A gentle "that\'s already too much," said as they reach to add more.',
			},
			{
				id: 'ajha-alikati-a-little-more',
				dev: 'अझ अलिकति',
				en: 'A little more',
				usage: 'When you DO want seconds — "ajha" (still/more) + "alikati" (a little). The opposite of "dherai bhayo"; asking for a bit more is itself a compliment to the cook.',
			},
			{
				id: 'ma-shakahari-hu-im-vegetarian',
				dev: 'म शाकाहारी हुँ',
				en: "I'm vegetarian",
				usage: '"Shaakaahaari" (vegetarian) + "hu" (I am). A key phrase to say early — many Nepali households cook vegetarian daily, so it\'s easily accommodated.',
			},
			{
				id: 'khanako-lagi-dhanyabaad-thank-you-for-the-food',
				dev: 'खानाको लागि धन्यवाद',
				en: 'Thank you for the food',
				usage: 'The warm parting line to a host — "khana" (food) + "ko lagi" (for) + "dhanyabaad" (thank you). More heartfelt than a bare "dhanyabaad"; naming what you\'re thankful for makes it land.',
			},
		],
	},
	{
		id: 'at-the-shop',
		title: 'At the Shop',
		kind: 'phrases',
		goal: 'Bargain and buy like a local',
		items: [
			{
				id: 'dherai-mahango-too-expensive',
				dev: 'धेरै महँगो भयो',
				en: "That's too expensive",
				usage: 'The opening move in any bargain — "dherai" (very) + "mahango" (expensive) + "bhayo" (has become). Say it with a small wince; it invites a lower price without giving offense.',
			},
			{
				id: 'ali-sasto-garnus-make-it-cheaper',
				dev: 'अलि सस्तो गर्नुस्',
				en: 'Make it a bit cheaper',
				usage: 'The direct discount ask — "ali" (a bit) + "sasto" (cheap) + "garnus" (please make/do). Polite and expected at markets; "ali" keeps it friendly rather than demanding.',
			},
			{
				id: 'chhut-dinus-give-a-discount',
				dev: 'छुट दिनुस्',
				en: 'Give me a discount',
				usage: '"Chhut" (discount/reduction) + "dinus" (please give), the request pattern you know from "paani dinus." The single most useful word at a bargaining stall is "chhut."',
			},
			{
				id: 'ma-yo-linchu-ill-take-this',
				dev: 'म यो लिन्छु',
				en: "I'll take this one",
				usage: 'The closer, once you\'ve agreed a price — "ma" (I) + "yo" (this) + "linchu" (I take), from "linu" which you know. Point and say it to seal the deal.',
			},
			{
				id: 'sabai-kati-bhayo-how-much-total',
				dev: 'सबै कति भयो?',
				en: 'How much for everything?',
				usage: 'Totaling up at the counter — "sabai" (all) + "kati" (how much) + "bhayo" (came to). Use it when buying several things at once to get one final number.',
			},
			{
				id: 'sano-size-cha-smaller-size',
				dev: 'सानो साइज छ?',
				en: 'Do you have a smaller size?',
				usage: '"Sano" (small — yes, like the mouse) + "size" (the everyday loanword) + "cha?" (is there?). Swap "sano" for "thulo" (big) to ask the other way.',
			},
			{
				id: 'paisa-firta-dinus-give-me-change',
				dev: 'पैसा फिर्ता दिनुस्',
				en: 'Please give me the change',
				usage: '"Paisa" (money) + "firta" (back/return) + "dinus" (please give). Nudges a shopkeeper who\'s slow with your change, without any edge to it.',
			},
			{
				id: 'malai-chahidaina-i-dont-need-it',
				dev: 'मलाई चाहिँदैन',
				en: "I don't need it / no thanks",
				usage: 'The polite walk-away — "malai" (to me) + "chaahidaina" (is not needed), the negative of "chaahiyo." A firm-but-friendly "no thanks" to a persistent seller.',
			},
		],
	},
];

// T13 · Which companion "owns" each unit's stretch of the path — and therefore voices its
// items in reviews (js/sano.js reviewCompanion) once that companion has a rendered voice
// folder (CHARACTER_VOICES in js/audio.js; clips via tools/tts/synth-app.mjs --units).
// Ten contiguous sections in course order, one per companion, mirrored by buddyOrder in
// renderPath so the pocket art and the quizzing voice agree. Every COURSE unit id must
// appear exactly once (tests/data/unit-voices.test.mjs).
const UNIT_VOICES = {
	// Thulo (rhino, "big") — the big foundations.
	basics: 'thulo',
	'basics-replies': 'thulo',
	numbers: 'thulo',
	'numbers-big': 'thulo',
	pronouns: 'thulo',
	// Pyaro (red panda, "dear") — family and meeting people.
	'family-people': 'pyaro',
	'people-others': 'pyaro',
	introductions: 'pyaro',
	'introductions-origins': 'pyaro',
	comprehension: 'pyaro',
	'comprehension-clarify': 'pyaro',
	connectors: 'pyaro',
	// Shanta (yak, "calm") — the calm of the kitchen and home.
	meals: 'shanta',
	'kitchen-items': 'shanta',
	'food-grocery-items': 'shanta',
	'food-grocery-pantry': 'shanta',
	'patterns-requests': 'shanta',
	'household-living': 'shanta',
	'household-chores': 'shanta',
	'cleaning-items': 'shanta',
	'patterns-doing': 'shanta',
	// Gyani (elephant, "wise") — the verb ladder and grammar patterns.
	'verbs-present': 'gyani',
	'daily-routine': 'gyani',
	'verbs-past': 'gyani',
	'verbs-reactions': 'gyani',
	'verbs-requests': 'gyani',
	'verbs-getting-around': 'gyani',
	'verbs-making-doing': 'gyani',
	'verbs-everyday-actions': 'gyani',
	'modals-can-want-must': 'gyani',
	// Chanchal (langur, "lively") — out and about: town, time, and errands.
	cooking: 'chanchal',
	purchasing: 'chanchal',
	time: 'chanchal',
	'time-week': 'chanchal',
	navigation: 'chanchal',
	'places-getting-around': 'chanchal',
	'place-position': 'chanchal',
	// Rangin (danphe, "colorful") — colors and everything descriptive.
	descriptors: 'rangin',
	colors: 'rangin',
	'comparing-things': 'rangin',
	'adj-size-physical': 'rangin',
	'adj-good-bad-right': 'rangin',
	'adj-order-sequence': 'rangin',
	'adj-same-different': 'rangin',
	'adj-states-conditions': 'rangin',
	weather: 'rangin',
	// Hiun (snow leopard, "snow") — body, health, and around the house.
	'body-parts': 'hiun',
	medical: 'hiun',
	commands: 'hiun',
	'bedroom-items': 'hiun',
	'bathroom-items': 'hiun',
	'living-room-items': 'hiun',
	'laundry-clothing-items': 'hiun',
	'technology-electronics': 'hiun',
	'personal-items': 'hiun',
	miscellaneous: 'hiun',
	// Bahadur (tiger, "brave") — feelings, animals, and wild nature.
	'emotions-feelings': 'bahadur',
	'emotions-more': 'bahadur',
	animals: 'bahadur',
	'animals-wild': 'bahadur',
	'food-drink': 'bahadur',
	'food-veg': 'bahadur',
	'food-dishes': 'bahadur',
	'weather-nature': 'bahadur',
	'weather-land': 'bahadur',
	'weather-water': 'bahadur',
	// Phurtilo (tahr, "nimble") — the quick little words: adverbs and function words.
	'jobs-work': 'phurtilo',
	'duration-frequency': 'phurtilo',
	'noun-time-events': 'phurtilo',
	'noun-ideas-talk': 'phurtilo',
	'noun-situations': 'phurtilo',
	'noun-money': 'phurtilo',
	'noun-people-places': 'phurtilo',
	'adv-degree': 'phurtilo',
	'adv-time-order': 'phurtilo',
	'adv-frequency': 'phurtilo',
	'adv-manner-place': 'phurtilo',
	'adv-linking': 'phurtilo',
	'fn-pronouns': 'phurtilo',
	'fn-conjunctions': 'phurtilo',
	'fn-counting': 'phurtilo',
	'fn-big-numbers': 'phurtilo',
	'fn-little-words': 'phurtilo',
	// Lamo (gharial, "long") — the long tail of the course.
	'noun2-travel': 'lamo',
	'noun2-city': 'lamo',
	'noun2-money': 'lamo',
	'noun2-school': 'lamo',
	'noun2-health': 'lamo',
	'feelings-states': 'lamo',
	'body-parts-more': 'lamo',
	'clothes-accessories': 'lamo',
	'groceries-more': 'lamo',
	'nepali-calendar': 'lamo',
	festivals: 'lamo',
	directions: 'lamo',
	'linking-words': 'lamo',
	'everyday-verbs-more': 'lamo',
	'odds-ends': 'lamo',
	'sounding-natural': 'lamo',
	'on-the-move': 'lamo',
	'being-a-guest': 'lamo',
	'at-the-shop': 'lamo',
};
