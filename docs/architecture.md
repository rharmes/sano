...
11. `js/sounds.js` — **`SOUND_TOPICS`**: pronunciation-drill topics (SR-08).
12. `js/sano.js` — **`window.Sano`**: the lesson engine (functions below). Public API: `state` (getter), `saveState`, `refreshHeader`, `showScreen`, `renderHome`, `applyServerState`, `placeBefore`, `placementOptions`, `resetPathReveal`.
