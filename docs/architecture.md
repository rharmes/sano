...
| `--data` | `node:test` (`tests/data/*.test.mjs`) | `COURSE`/`DIALOGUES`/`SOUND_TOPICS`/`GRAMMAR_TOPICS` integrity — unique ids, required fields, the gloss-join invariant, sound-mark coverage, grammar example parts joining back to a real sentence whose clip is on disk; + romanization coverage over all 638 `dev` (every codepoint mapped, clean output charset, structure preserved); + the ground-truth `dictionary.json` (schema + every COURSE word represented; skips until built); + `WORD_GLOSSES` coverage (every canonical/frame prompt word glossed, no orphan keys); + `CLAUDE.md` ↔ `AGENTS.md` parity (T62: the Codex twin equals the Claude file under a fixed substitution table, and stays under Codex's 32 KiB read budget). |
