...
- **Grammar notes (T64) + verb cards (T66):** `startGrammar` (both kinds; a card sets `#screen-grammar.verb`, hides the contrast block and renders its table), `grammarTable` (one row per form, each a button that plays the form's word clip via `playTileWord`), `finishGrammar`, `grammarSentence` (resolves an example's `clip` — item id or `<id>-fN` frame — to its `{dev, en}`), `grammarRomanizedParts`, `grammarChips`, `grammarWord`; `finishStop` is the shared completion for dialogue / sound / grammar stops (tick the done-map, streak, complete screen).
