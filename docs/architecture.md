...
- **Home / path:** `renderHome`, `renderPath`, `currentUnit`, `unitNewItems`, `unitDueCount`, `unitIsComplete`, `unitIsIntroduced`, `unitMasteredCount`, `unitIsUnlocked`, `placeBefore`, `placementOptions`, `dialogueUnlocked`, `soundUnlocked`, `grammarUnlocked`.
...
- **Grammar notes (T64):** `startGrammar`, `finishGrammar`, `grammarSentence` (resolves an example's `clip` — item id or `<id>-fN` frame — to its `{dev, en}`), `grammarRomanizedParts`, `grammarWord`.
