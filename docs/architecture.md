...
| `--data` | `node:test` (`tests/data/*.test.mjs`) | `COURSE`/`DIALOGUES`/`SOUND_TOPICS` integrity — unique ids, required fields, the gloss-join invariant, sound-mark coverage; + romanization coverage over all 638 `dev` (every codepoint mapped, clean output charset, structure preserved); + the ground-truth `dictionary.json` (schema + every COURSE word represented; skips until built); + `WORD_GLOSSES` coverage (every canonical/frame prompt word glossed, no orphan keys). |
