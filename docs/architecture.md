...
1. `js/data.js` — **`COURSE`**: 83 units / 800 items — the entire course content (the big file). Units >14 items were split into ~8–12-word chunks for the SR-05 mastery gate; item ids are unchanged. The T11 vocabulary expansion adds new frequency-sourced units by batch (batch 1: 50 everyday-verb frames as 5 units after `verbs-past`; batch 2: 45 everyday-adjective frames as 5 units after `comparing-things`; batches 3–5: everyday nouns (46), adverbs (44), and function words (27) as 5 units each appended at the end).
