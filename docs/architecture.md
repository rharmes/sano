...
1. `js/data.js` — **`COURSE`**: 98 units / 927 items — the entire course content (the big file). Units >14 items were split into ~8–12-word chunks for the SR-05 mastery gate; item ids are unchanged. The T11 vocabulary expansion adds new frequency-sourced units by batch (batch 1: 50 everyday-verb frames as 5 units after `verbs-past`; batch 2: 45 everyday-adjective frames as 5 units after `comparing-things`; batches 3–6: everyday nouns (46), adverbs (44), function words (27), and everyday-life nouns pt.2 (45) as 5 units each appended at the end; batch 7: 33 hand-curated feelings/body/clothes/food frames as 4 units appended at the end, once the raw-frequency everyday pool was exhausted; batch 8: 27 hand-curated calendar/festivals/directions frames as 3 units appended at the end; batch 9 (final breadth batch): 22 hand-curated highest-value gaps — linking words, more verbs, odds & ends — as 3 units appended at the end, after which T11 pivots from breadth to depth). **Depth (T28/T29):** items gain an optional `frames: [{dev,en}]` array — alternate example sentences that rotate into an item's reviews over the SAME spaced-repetition record (`itemFrames`/`frameForSeen`/`pickFrame` → `ex.frame` in `js/sano.js`), so known words are practiced in varied contexts without growing the path; live so far are the T28 pilot (3 items) and T29 batch 1 (10 core present-tense verbs), each extra frame's clip named `<id>-fN`. Frames are drafted/reviewed in `design/frames.html` (T31).
