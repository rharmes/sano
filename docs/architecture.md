...
1. `js/data.js` — **`COURSE`**: 104 units / 979 items — the entire course content (the big file). Units >14 items were split into ~8–12-word chunks for the SR-05 mastery gate; item ids are unchanged. The T11 vocabulary expansion adds new frequency-sourced units by batch (batch 1: 50 everyday-verb frames as 5 units after `verbs-past`; batch 2: 45 everyday-adjective frames as 5 units after `comparing-things`; batches 3–6: everyday nouns (46), adverbs (44), function words (27), and everyday-life nouns pt.2 (45) as 5 units each appended at the end; batch 7: 33 hand-curated feelings/body/clothes/food frames as 4 units appended at the end, once the raw-frequency everyday pool was exhausted; batch 8: 27 hand-curated calendar/festivals/directions frames as 3 units appended at the end; batch 9 (final breadth batch): 22 hand-curated highest-value gaps — linking words, more verbs, odds & ends — as 3 units appended at the end, after which T11 pivots from breadth to depth). **Depth — frames (T28/T29):** items gain an optional `frames: [{dev,en}]` array — alternate example sentences that rotate into an item's reviews over the SAME spaced-repetition record (`itemFrames`/`eligibleFrames`/`rotateFrame`/`pickFrame` → `ex.frame` in `js/sano.js`; **gated (T38)**: alternates only once the item has graduated and the frame adds ≤2 never-seen words, and `choice` exercises always show the canonical sentence), so known words are practiced in varied contexts without growing the path; live are the T28 pilot (3 items) + T29 batches 1–6 (85 framed items across present/past verbs, adjectives, position/time, modals, nouns), each extra frame's clip named `<id>-fN`. Frames are drafted/reviewed in `design/frames.html` (T31). **Depth — new units (T30):** four `kind:'phrases'` real-expression units (32 whole utterances built from known vocab — Sounding Natural, On the Move, Being a Guest, At the Shop) appended at the path's end, drafted/reviewed in the T14 `design/expansion.html` pipeline.
...
6. `js/audio.js` — **`SanoAudio`**: `play(id, voiceId?)` (phrase clips `audio/<voice>/<id>.mp3`; a non-default voice **falls back to the default clip** on a load error, so partial companion coverage never goes silent — T13), `playWord(slug)` (`audio/words/<slug>.mp3`), `button(...)`, `voiceForCharacter(id)` (via `CHARACTER_VOICES` — the 6 dialogue-voiced companions map to their own folder, everyone else to `default`). `AUDIO_VERSION` busts caches.
...
9. `js/gloss.js` — **`SanoGloss`**: `renderLine(line, opts?)` (underlined tappable segments; `opts.onWordTap(seg)` fires alongside the popover — the lesson prompts use it to play the word's clip) + a tap-to-translate popover; `closePop()`. Shared by the app, the lesson prompts (T37), and `design/dialogue.html`.
10. `js/glosses.js` — **`WORD_GLOSSES`**: `{ slug: en }` — per-word English behind the tap-a-word lesson glosses (T37). **Generated** by `tools/build-glosses.mjs` — do not hand-edit (shape + sources: `docs/data-model.md`).
11. `js/en-glosses.js` — **`EN_GLOSSES`**: `{ audioId: [[en] | [en, np], …] }` — the reverse lookup behind the tap-a-word hints on **English** prompts (T59), keyed per prompt rather than per word. **Generated** by `tools/build-en-glosses.mjs` — do not hand-edit (shape + why: `docs/data-model.md`).
12. `js/sounds.js` — **`SOUND_TOPICS`**: pronunciation-drill topics (SR-08).
12b. `js/grammar.js` — **`GRAMMAR_TOPICS`** + **`GRAMMAR_ROLES`**: grammar notes (T64) — one-page explainers on the path, each built from real course sentences — and, in the same array with `kind: 'verb'`, the verb cards (T66): a conjugation `table` per workhorse verb (shape: `docs/data-model.md`).
13. `js/sano.js` — **`window.Sano`**: the lesson engine (functions below). Public API: `state` (getter), `saveState`, `refreshHeader`, `showScreen`, `renderHome`, `applyServerState`, `placeBefore`, `placementOptions`, `resetPathReveal`.
...
- **Home / path:** `renderHome`, `renderPath`, `currentUnit`, `unitNewItems`, `unitDueCount`, `unitIsComplete`, `unitIsIntroduced`, `unitMasteredCount`, `unitIsUnlocked`, `placeBefore`, `placementOptions`, `stopUnlocked` (one unlock rule for dialogue / sound / grammar stops: the `after` unit is complete).
- **Numerals (T68):** `isNumeral` (an item whose `dev` is Devanagari digits only) and `itemClip` (the clip an item plays — its own id, a numeral's borrowed `says`, or null = silent); both live inside the lifted SR-05 depth block because `itemFrames` needs them, and a frame carries `numeral` so the renderers can keep a glyph prompt silent.
...
- **Exercise renderers:** `renderExercise` (dispatch) → `renderChoice`, `renderMatch`, `renderListenMatch`, `renderWordbank`, `renderType`, `renderSpeak`; helpers `setPrompt`, `setListenPrompt`, `glossedPrompt` (T37 tap-a-word Nepali prompts), `glossedEnglishPrompt` (T59 — the same underline on an English prompt, popping the romanized Nepali; silent, and `null` when the prompt has no alignment so it renders plain), `getDistractors`, `wordbankDistractors`, `uniquePairItems`, `uniqueClipItems`, `fitMatchTiles`, `hashId`/`waveformSvg`/`listenTile`, `playTileWord`, `cleanTileText`, `stripParens`.
- **Grading:** `answerExercise`, `checkExercise`, `acceptedEnglish`, `applyAnswer`, `showFeedback`, `selectMatchTile`, `finishMatch`, `normalize`, `lenientEquals`, `editDistance`.
- **Dedup invariant:** the tap-the-pairs grids and `choice` dedupe each bundle by display text (`uniquePairItems`, plus `getDistractors`' used-text set) so two items with identical romanized/English text never appear as two tiles — which would let a correct pairing grade as wrong. The listening grid's tiles are clips, so it also dedupes **by clip** (`uniqueClipItems`, T68): a numeral borrows the clip of the word that says it, and the pair collides on neither text.
- **Dialogue player (SR-01):** `openLinkedDialogue` (boot: `/?dialogue=<id>` opens a story directly — the way in while no story node is on the path, T65), `startDialogue`, `renderDialogueConvo`, `dialogueBubble`, `revealNextLine`, `advanceDialogue`, `startDialogueQuiz`, `renderDialogueQuestion`, `answerDialogueQuestion`, `continueDialogue`, `finishDialogue`, `courseItem`.
- **Sounds (SR-08):** `startSoundDrill`, `renderSoundCard`, `highlightDev`, `advanceSound`, `finishSound`, `soundExamples`.
- **Grammar notes (T64) + verb cards (T66):** `startGrammar` (all kinds; a card sets `#screen-grammar.verb`, hides the contrast block and renders its table; the T68 numerals note sets `.numerals` and renders both, its rows led by a large glyph), `grammarTable` (one row per form, each a button that plays the form's word clip via `playTileWord`), `finishGrammar`, `grammarSentence` (resolves an example's `clip` — item id or `<id>-fN` frame — to its `{dev, en}`), `grammarRomanizedParts`, `grammarChips`, `grammarWord`; `finishStop` is the shared completion for dialogue / sound / grammar stops (tick the done-map, streak, complete screen).
...
- **Home / path:** `renderHome` → `renderPath` draws the winding path — units in order, with dialogue (gold — only a story flagged `onPath`; none is today, T65), sound (lavender), grammar-note (teal, T64) and verb-card (clay, T66 — a `.grammar.verb` stop) stops woven in after their `after` unit (one `addStop` helper draws all four), and decorative companions in the pockets (SR-07). `currentUnit` = first unlocked, incomplete unit; **a unit is complete (unlocks the next) only when every item has `graduated` — the SR-05 mastery gate** (`unitIsIntroduced` = the weaker "all met"; the current node's ring is two-tone — a faint `--accent-soft` arc for words *introduced* (`unitNewItems`) under a solid `--accent` arc for words *mastered* (`unitMasteredCount`), so it moves as you practice but fills only at unlock). The daily-lesson button (`dailyPlan`) is review-dominant and throttles new words by review debt.
- **Lesson:** `startDailyLesson` (via `dailyPlan`) / `startUnitLesson` → `buildExercises` makes the queue. Each new item: 2× `choice` + an in-session tap-based `wordbank` recall (ordered after a recognition drill) + a `speak`. Review items **escalate with maturity** — recall-strength (interval ≥ `RECALL_INTERVAL` = 2) gets a `wordbank` while still learning and free `type` once `graduated`; ~50% of recall reviews audio-only; `listenMatch`/`match` bundles for eligible vocab. `renderExercise` dispatches by type → answering routes through `applyAnswer` → `scheduleReview` (auto-graded via `exerciseGrade`; EASY grades accrue `recalls` toward graduation) → `registerActivity`. `finishLesson` shows the complete screen. **Numeral items (T68) route differently:** new = 2× `choice` + `type` ("Type the number": glyph prompt, number answer, numeric keypad) + a `speak` only when the item has `says`; reviews = `type` once at recall strength, else `choice` either way or — with `says` — the listening choice, whose answers are glyphs. Glyph prompts and glyph match tiles never play audio or show `pron`; the reveal does.
- **Dialogue (SR-01):** a gold path node (only an `onPath` story — none today, T65) or `openLinkedDialogue` at boot (`/?dialogue=<id>`; drops the param from the URL, a first run stays with onboarding, an unknown id is ignored) → `startDialogue` → `renderDialogueConvo` reveals lines one at a time (`dialogueBubble` builds a head + bubble for a speaker, or a full-width narrator line; **romanized-only** via `SanoGloss.renderLine`, with autoplayed `SanoAudio`) → `startDialogueQuiz` (comprehension) → `finishDialogue` marks `dialoguesDone`.
...
| `logout.php` | Deletes the session row and clears the `__Host-sano_session` cookie. |
...
| `ingest-traffic.php` | Server-only nightly cron (T40): parse the Apache access logs → the `traffic_*` aggregate tables. Dreamhost keeps ~7 days of logs, so this is what accumulates history. Visitors are salted `sha256(ip + UA)` hashes — no raw IP is stored; countries come from a locally compiled CC0 IP→country index (`--update-geo`, pinned to a version line; a compiled index only replaces the one on disk if it clears an absolute floor and 90% of the existing range count, so a truncated download or an HTML error page can't silently blank every country — T50). `--json` prints the parse with no DB or config (what `tests/data/traffic-parse.test.mjs` asserts on). Not in the rsync. |
...
| `tts/synth-app.mjs` | Render phrase / word / dialogue-line / companion-review clips through the ElevenLabs API — `--phrases` (→ `audio/default/<id>.mp3`, Sano's clone) / `--words` (→ `audio/words/<slug>.mp3`) / `--dialogues` (→ `audio/<voice>/<clipId>.mp3`, per speaker) / `--units [id,id,…]` (T13: each mapped unit's phrase+frame clips in its path companion's voice → `audio/<companion>/<clipId>.mp3`; un-voiced companions skipped+counted). Add `--new` for only clips missing on disk, `--only` for one, `--sample` to preview. Bump `AUDIO_VERSION` (js/audio.js) after. |
| `build-numeral-font.py` | T68: builds `fonts/nepali-numerals-{300,400,700}.woff2` (each from a Noto weight one step lighter than its CSS slot — Light / Regular / Medium — since Noto runs heavier than Lato) from Noto Sans Devanagari (OFL; source TTFs not committed) — applies the font's Nepali `locl` digit forms (5 and 8) to the cmap, then cuts it to U+0966–096F (~3 KB each). Python + fonttools in a throwaway venv; re-run only to change the source font. |
| `tts/build-words.mjs` | Build `tts/words.json` (per-word Devanagari; every canonical + frame sentence across all units, incl. single-word items — plus every verb-card table cell in `js/grammar.js`, T66, listed under the pseudo-id `grammar:<card>`) for the word-bank clips. |
| `build-glosses.mjs` | Generate `js/glosses.js` (T37): one English gloss per promptable word — single-word item `en` → dictionary gloss → hand-drafted surface-form FILLS (AI-drafted, Ross-reviewed). Fails on any un-glossed word; run after `build-words.mjs` when content changes. |
| `build-en-glosses.mjs` | Generate `js/en-glosses.js` (T59): align each frame's own English to its own Nepali *through* the T37 glosses, so an English prompt word can hint the Nepali it stands for. Reads `js/glosses.js`, so run it **after** `build-glosses.mjs`. Leaves a word unhinted rather than guess; `--report` writes `design/en-gloss-report.json` for the `design/en-gloss.html` review tool, whose rulings come back as `OVERRIDES`. |
...
| `--data` | `node:test` (`tests/data/*.test.mjs`) | `COURSE`/`DIALOGUES`/`SOUND_TOPICS`/`GRAMMAR_TOPICS` integrity — unique ids, required fields, the gloss-join invariant, sound-mark coverage, grammar example parts joining back to a real sentence whose clip is on disk; + romanization coverage over all 638 `dev` (every codepoint mapped, clean output charset, structure preserved); + the ground-truth `dictionary.json` (schema + every COURSE word represented; skips until built); + `WORD_GLOSSES` coverage (every canonical/frame prompt word glossed, no orphan keys); + `CLAUDE.md` ↔ `AGENTS.md` parity (T62: the Codex twin equals the Claude file under a fixed substitution table, and stays under Codex's 32 KiB read budget); + reviewer-pair parity (T70: `.codex/agents/pr-antagonist.toml`'s brief equals `.claude/agents/pr-antagonist.md`'s under its own substitution table, and each pins its harness's reviewer model at xhigh — the procedure is `docs/pr-review.md`). |
...
- **`tests/e2e/_helpers.mjs`** — `boot()` (seed + load + inline animation-freeze + a seeded
  `Math.random` stub so every run draws the identical lesson — T39) and
