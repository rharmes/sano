...
| `build-numeral-font.py` | T68: builds `fonts/nepali-numerals-{400,700}.woff2` from Noto Sans Devanagari (OFL; source TTFs not committed) — applies the font's Nepali `locl` digit forms (5 and 8) to the cmap, then cuts it to U+0966–096F (~3 KB each). Python + fonttools in a throwaway venv; re-run only to change the source font. |
