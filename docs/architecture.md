...
1. `js/data.js` — **`COURSE`**: 78 units / 773 items — the entire course content (the big file). Units >14 items were split into ~8–12-word chunks for the SR-05 mastery gate; item ids are unchanged. The T11 vocabulary expansion adds new frequency-sourced units by batch (batch 1: 50 everyday-verb frames as 5 units after `verbs-past`; batch 2: 45 everyday-adjective frames as 5 units after `comparing-things`; batch 3: 46 everyday-noun frames + batch 4: 44 everyday-adverb frames as 5 units each appended at the end).
