...
<img src="docs/sano-idle.gif" alt="Sano, the paper-cut mouse mascot, idling" width="130" align="left" />
...
- **Guard order:** every endpoint runs its stateless checks — HTTP method, the CSRF header, JSON parse, field validation — _before_ authentication and any DB call, so a malformed request fails fast and the whole validation surface is testable without a database (the no-cookie path returns 401 before `db()` is ever touched). Checks that need a row (revision conflict, `no_such_user`, the admin check) necessarily follow auth. The canonical order is documented above the guards in `lib.php` and exercised by `tests/api/guards.spec.mjs`.
...
- **`test.sh [--static|--unit|--data|--api|--ui]`** — the **single test-suite entry point** (= **`npm test`** / **`npm run check`**); with no flag it runs every locally-runnable tier. Tiers: `--static` (the static preflight below), `--unit`/`--data` (`node:test` over `tests/unit`/`tests/data`), `--api` (Playwright `request` guard specs — no browser, no DB), `--ui` (Playwright Chromium + WebKit over `tests/e2e`). Full detail under "Testing & verification".
- **`check.sh`** — the **static preflight** (= **`npm run lint`**, and what `test.sh --static` runs): `format.sh --check`, `stamp-version.mjs --check`, `php -l` over `api/` + `tools/`, `node --check` over the JS/MJS. No browser; exits non-zero on the first failure.
...
- **`deploy.sh [-n] [--allow-dirty]`** — rsync the site to the live Apache host. **To deploy:** finish the normal change workflow first (`format.sh` → `stamp-version.mjs` → `tools/test.sh` → task branch → PR → merge on the gauntlet's approve), switch to the merged `main`, and on Ross's go — a merge doesn't authorize a deploy — run `tools/deploy.sh -n` to preview the itemized transfer and `tools/deploy.sh` to upload. It ships the working-tree files **as they are on disk** (committed or not), so as a safety net it **refuses to run on a dirty tree** — commit first, or pass `--allow-dirty` to ship uncommitted changes on purpose (`-n` dry-runs skip the guard, and untracked files don't trip it; only tracked modifications block). It no longer stamps (that's a pre-commit step, verified by `npm run check`) — it just rsyncs the allowlist — `index.html .htaccess favicon.svg apple-touch-icon.png icon-192.png icon-512.png icon-512-maskable.png manifest.json sw.js css js fonts audio api admin` — with `--checksum --no-times` (the host resets mtimes) and `--delete-after`, which prunes only inside the listed paths: a file removed from the repo disappears from the site, anything placed there by hand is deleted, and files outside them (`~/sano-config.php`, `~/sano-tools/`, the Composer vendor dir, the host's own top-level files) are untouched. Anything off that list never ships, so `tools/`, `design/`, `docs/`, `CLAUDE.md`, and `README.md` stay local. Connection details come from the `sano-deploy` SSH alias in `~/.ssh/config` (key auth, HostName namastesano.com) — nothing sensitive in the repo. **Verify after:** `curl -sI https://namastesano.com/ | grep -i cache-control` should report `no-cache`, and the live `css/sano.css?v=…` stamp should match the one in your local `index.html`.
...
- The old `check-viewports.mjs` (9-width overflow sweep) and `check-webkit.mjs` (real-Safari animation probe) are **retired** — both are now Playwright e2e specs in `tests/e2e/`, where the `webkit` project catches the WebKit-only rendering the Chromium tooling can't. Real iOS-device Safari stays a manual check.
...
One suite, one entry point — **`tools/test.sh`** (= `npm test`); pass a tier flag to scope it. CI runs the same tiers. The runners are `node:test` (built into Node, zero new dep) for pure logic + data, and `@playwright/test` for everything HTTP or browser.

| Tier | Flag | Runner | Covers |
| --- | --- | --- | --- |
| Static | `--static` | `check.sh` | Prettier `--check`, stamp check, `php -l`, `node --check`. (= `npm run lint`.) |
| Unit | `--unit` | `node:test` (`tests/unit/`) | Pure logic lifted from `js/sano.js`: SR-05 scheduler, answer matching, dates/streak/freeze, state migration, exercise dedup. |
| Data | `--data` | `node:test` (`tests/data/`) | `COURSE`/`DIALOGUES`/`SOUND_TOPICS` integrity — unique ids, required fields, the gloss-join invariant, sound-mark coverage. |
| API | `--api` | Playwright `request` (`tests/api/`) | Pre-DB guard specs (method/CSRF/JSON/validation, no DB) + a PHP pure-helper test. `integration.spec.mjs` adds the full request cycle — auth, revision conflict, lockout, admin — gated on `SANO_TEST_DB`. |
| UI e2e | `--ui` | Playwright Chromium + WebKit (`tests/e2e/`) | Onboarding, home/path + the 9-width overflow sweep, every lesson exercise type, the dialogue player + tap-gloss, dictionary, reminder modal, admin demo, idle / reduced-motion. |

- Pure helpers are lifted out of the classic (non-module) browser scripts by `tests/lift.mjs` — no app-code change needed to test them. Fixtures come from `tests/seed.mjs`, the same builders `tools/dev-seed.html` uses.
- **DB integration** (`tests/api/integration.spec.mjs`) needs MySQL and runs only when `SANO_TEST_DB` is set; the `tests/api` guard specs run with **no** `sano-config.php`, so they assert only the pre-DB guards. Both halves run under `--api` (integration skips locally).
- **Flaky tests are defects**, not noise — a test that passes only sometimes gets root-caused (wait on the real condition, freeze animations, fix the app race), not retried away.
- **CI** (`.github/workflows/ci.yml`) runs three jobs on every push to `main` and every PR: `static + unit + data + api guards` (no browser/DB), `e2e` (Chromium + WebKit), and `api integration` (against a `mysql:8` service). Results show in the repo's **Actions** tab. Real iOS-device Safari stays a manual check.
- **Visual**: serve locally and screenshot via a temp harness page (`.shot-harness.html` that seeds `sano.state.v1`, iframes the app, clicks into a screen); delete it before committing. Headless Chrome follows the system theme — to force light mode, strip the dark `@media` blocks into temp CSS copies. A pixel screenshot-diff tier is scoped but **deferred** — see `docs/visual-regression.md`.
- **Live checks** after deploy: `curl -sI https://namastesano.com/ | grep -i cache-control` — HTML `no-cache`, css/js `max-age=2592000` (busted by `?v=`), `api/` `no-store`.
...
**npm scripts** — `npm run` with no name prints the full menu. `start` / `stop` run and kill the dev server on :8000; `format` formats in place (the only script that writes); `lint` runs the static preflight (`tools/check.sh` — format + stamps + `php -l` + `node --check`, no browser); `test` and `check` both run the full suite (`tools/test.sh`), with `test:unit` / `test:data` / `test:api` / `test:e2e` for a single tier; `stamp` rewrites the `?v=` hashes; `deploy:preview` / `deploy` dry-run / ship. (`start`, `stop`, and `test` are npm lifecycle names, so they also work without `run`.)
