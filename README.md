...
- **`check.sh [--no-viewports]`** — the **preflight**: every static check in one command — `format.sh --check`, `php -l` over `api/` + `tools/`, `node --check` over the JS/MJS, then `check-viewports.mjs`. Exits non-zero on the first failure. `--no-viewports` (= **`npm run lint`**, what CI runs) skips the headless-Chrome step. The full thing is **`npm run check`**.
...
- **Preflight (one command)**: `tools/check.sh` (or `npm run check`) bundles format check + stamp check + PHP lint + JS syntax + the viewport scenarios; `npm run lint` is the browser-less subset. The individual checks below are what it wraps.
