...
- **`check.sh [--no-viewports]`** — the **preflight**: every static check in one command — `format.sh --check`, `php -l` over `api/` + `tools/`, `node --check` over the JS/MJS, then `check-viewports.mjs`. Exits non-zero on the first failure. `--no-viewports` skips the headless-Chrome step (used by CI). Also wired as **`npm run check`**.
...
- **Preflight (one command)**: `tools/check.sh` (or `npm run check`) bundles format check + PHP lint + JS syntax + the viewport scenarios; `--no-viewports` runs the browser-less subset. The individual checks below are what it wraps.
...
- **CI**: `.github/workflows/ci.yml` runs the preflight (`--no-viewports`) on every push to `main` and every PR — Prettier + `php -l` + `node --check` on a GitHub Ubuntu runner (built-in PHP, `actions/setup-node`, `npm install`). The headless-Chrome layout check stays local. Results show in the repo's **Actions** tab.
