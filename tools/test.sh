...
#
#   tools/test.sh            # static + unit + data + ui  (the full local suite)
...
#   tools/test.sh --ui       # headless-Chrome viewport layout
#
# Tiers added in later phases: --api / --e2e (Playwright: HTTP + real Chromium/WebKit).
...
# locally) — explicit file paths behave identically everywhere.
set -eu
cd "$(dirname "$0")/.."

want_static=0 want_unit=0 want_data=0 want_ui=0 explicit=0
for arg in "$@"; do
	case "$arg" in
